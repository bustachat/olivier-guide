...
- Current version: **v44.46 (August 2026)** — always verify with `git log --oneline -1` and `athletes/olivier.json` guideVersion; treat any hardcoded version in prose as a hint, not truth (this line itself sat stale at v42.18 for 13 versions until v44.31 — the §6 state snapshot is the more reliable of the two)
...
- **🚩 OPEN (v44.46) — `index.html` Glossary, Minutes Outlook lens: "Barry D2 consistently ranks #1 under this lens" is FALSE, and pre-existing.** Barry's `lensScores.minutes` is **59**; the lens is led by `iowa_western` **79**, then lsu_eunice 77, dodge_city_cc/daytona_state 75, and five more JUCOs at 73. Barry has never been #1 on the stored numbers, so this was **not** introduced by the v44.46 refresh — found by the §3a Type 3 prose sweep and deliberately **not** fixed, to keep a roster commit free of unrelated copy edits. The same sentence's *"7 of 13 midfielders clear"* is currently accurate against Barry's stored 2025-26 data and passes the `PROSE` check — **it will go stale the moment Barry is refreshed in Wave 2, so fix both halves in that commit.**

- **🚩 OPEN (v44.46) — Keiser is still described as "Fort Lauderdale" in two `js/app.js` strings.** v40.6 corrected Keiser's location to **West Palm Beach** across 12 text occurrences plus mapX/mapY, but missed the `CONF_SECTIONS` NAIA intro (`js/app.js` ~line 528, *"Keiser University in Fort Lauderdale has clinical simulation labs…"*) and the Coaches-tab NAIA tier intro (~line 2555, *"Keiser in Fort Lauderdale has clinical simulation labs"*). Cosmetic and no scoring impact, and **no validator can see it** — the `PROSE` check covers counts, roster claims, scrape-season phrasing and phantom schools, not a wrong *city*. Left for a copy pass rather than fixed inside a Type 3 roster commit.
