...
- **✅ RESOLVED v45.10 — `tyler_jc` uses `apacheathletics.com` on all 3 surfaces + a new `ICON_OVERRIDES` map, not a DOMAINS/domain swap.** v45.09 tried swapping the two fields (see that version's now-superseded reasoning) to fix a "blends into the background" complaint, but that put `tjc.edu`'s icon in front of users everywhere — and `tjc.edu/favicon.ico` turns out to **redirect to WordPress's own generic default site icon** (`wp-includes/images/w-logo-blue-white-bg.png`), not a TJC mark at all. Owner caught it immediately via screenshot ("wtf is that?"). Investigated properly with a VPN (`apacheathletics.com` CloudFront-blocks every automated/datacenter path this project has, including a real Chrome browser without one — long-documented in this project's history) via Claude-in-Chrome: confirmed `apacheathletics.com` genuinely has no `/favicon.ico` (a real 403 past the CloudFront block, not a proxy artifact); its actual declared `<link rel="icon">` is a real, current "TJC" athletics wordmark at a media-hash path — just shaped as a wide 512×121 banner, which is why it rendered as a near-invisible sliver through Google's/DuckDuckGo's square-icon-shaped proxies. **Fix:** `juco.json` `domain` reverted to `'apacheathletics.com'` (`tjc.edu` is never used for Tyler JC again anywhere); new `ICON_OVERRIDES` map in `js/app.js` (general mechanism — several other audited schools hit this same "real icon isn't at `/favicon.ico`" problem, see §D's icon-audit note above) lets a school's confirmed-real icon URL be used directly, wired as first-priority on the card, modal, and Dashboard, with the existing Google/initials fallback chain preserved if it ever fails. **Do not touch the other 169 schools' DOMAINS/domain split** — that convention is still correct and by design; this is a Tyler-specific override layered on top, not a change to the convention.
