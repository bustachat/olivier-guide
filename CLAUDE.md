...
**Current version: v45.41 (2026-09-15).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
| **7F — Region 6 (KS)** | `seward_county_cc`, `garden_city_cc`, `dodge_city_cc` | Garden City and Seward had empty 2026-27 pages on Aug 17. Seward has the largest MF group in the batch (20). |
