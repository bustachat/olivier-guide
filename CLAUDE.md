...
- Current version: **v44.48 (August 2026)** — always verify with `git log --oneline -1` and `athletes/olivier.json` guideVersion; treat any hardcoded version in prose as a hint, not truth (this line itself sat stale at v42.18 for 13 versions until v44.31 — the §6 state snapshot is the more reliable of the two)
...
- **🚩 OPEN (v44.48) — the "9.9 equivalencies for D1 soccer" figure in `index.html` may be stale post-*House*.** Both the Financial Model explainer and the Glossary equivalency block state D1 = 9.9. §5c records that **NCAA D1 post-*House* (July 1 2025) replaced sport-specific scholarship limits with a 28-player roster cap, all fundable, at opt-in schools.** So 9.9 still holds at non-opt-in schools but not at opt-in ones. **Deliberately not "fixed" in v44.48** — a correct sentence depends on per-school opt-in status, which the guide does not track for any of its 67 D1 programs, and §5c explicitly says that status needn't be researched *because `full` carries a zero penalty*. Resolve either by writing opt-in-aware copy ("9.9 at schools that have not opted into the House settlement; opt-in schools instead run a 28-player fully-fundable roster") or by deciding the distinction is too fine for the Financial Model card. Do not silently drop the 9.9 — it is still correct for part of the field.
