...
- **✅ RESOLVED v45.09 — `tyler_jc`'s inverted DOMAINS/domain fields swapped to match every other school's split.** `DOMAINS` in `js/app.js` holds the ATHLETICS host (modal-header logo) and the school object's `domain` holds the UNIVERSITY host (Explore card + Dashboard logo) for 169 of 170 schools — Tyler JC alone had it backwards (`DOMAINS.tyler_jc: 'tjc.edu'`, `domain: 'apacheathletics.com'`). Owner-reported the Dashboard/Explore icon "blends into the background"; traced to more than the v45.08 contrast fix could solve — `apacheathletics.com`'s actual favicon is ~77% transparent with only a small gold accent (pixel-sampled directly), while `tjc.edu`'s is a solid white-canvas logo with a real blue mark. **Swapped both fields** (`DOMAINS.tyler_jc` → `'apacheathletics.com'`, `juco.json` `domain` → `'tjc.edu'`) rather than the one-token fix this note used to suggest, since that partial fix would have dropped the good `tjc.edu` icon from the app entirely. Net effect: cards + Dashboard (high-traffic) now show the solid `tjc.edu` logo; the modal (one school at a time) shows the sparser `apacheathletics.com` one — the better trade. **Do not "fix" the other 169** — the split is by design.
