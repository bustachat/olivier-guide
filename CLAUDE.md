...
**Current version: v44.86 (2026-08-16).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
- **`roster_season` doubles as the roster-refresh ledger** — the Minutes Outlook tab shows at a glance which schools are on 2026-27 data and which are still on 2025-26. Prior baseline **64 on 2026-27, 51 on 2025-26, 7 `available:false`** (v44.55, scoped to the pre-gap-fill-campaign ~122 schools). **v44.87 moved 10 JUCOs from 2025-26 to 2026-27** (miami_dade, northeast_cc, daytona_state, iowa_lakes_cc, angelina_college, johnson_county_cc, glendale_cc_az, arizona_western, cowley_cc, coastal_bend_cc) → **74 on 2026-27, 41 on 2025-26** within that same scope, 7 `available:false` unchanged. **A fresh guide-wide recount (`python3` script over all 170 schools' `minutesOutlook.roster_season`) done for this update reads 83 on 2026-27 / 71 on 2025-26 / 16 `available:false`** — the gap between that and the incremented 74/41/7 figure above is the ~48 gap-fill-campaign JUCOs (Batches 1–14, v44.70–v44.85) added after this ledger's original scope was defined, most of which arrived already on fresh data. Treat the 83/71/16 figure as the authoritative current total; the incremented 74/41/7 figure is kept only to show this session's delta against the legacy baseline.
...
Campaign plan and session-by-session detail live in the `roster_refresh_campaign_2026` memory. **Next action (Batch 2): the coach-verification follow-up below, then the 7 "published but empty" JUCOs, deferred to late August, plus a `tyler_jc` re-check.**
...
- **⏳ JUCO Session 4 SURVEY (2026-08-06) — the session ran too early: 7 of 19 probed JUCOs publish a 2026-27 page with ZERO players.** Probe method: real Chrome, season-scoped URL (`/sports/msoc/2026-27/roster` — `/sports/msoc/roster` **404s** on these `/index` hosts, per v42.5), recording title + `tbody tr` count + `body.innerText.length`.
  ...
  - **🚩 NEW — coach-verification follow-up for v44.87's Batch 1, queued for Batch 2.** The Change Type 2 spot-check for this batch was only genuinely done for `miami_dade` (Giuseppe DePalo confirmed live on the roster page's own Coaching Staff section, matches `coaches.json`). For the other 9 refreshed schools it was NOT completed despite the session's task list marking it done — a bookkeeping error caught when the owner asked directly: `northeast_cc`'s roster page only showed an assistant (Austin Backman), not the stored head coach "Adam Potter"; `daytona_state` only had a "View Coaches" link, never followed; `cowley_cc`'s table showed a team manager, not a head coach; `angelina_college`'s Coaching Staff section rendered empty; `iowa_lakes_cc`, `johnson_county_cc`, `glendale_cc_az`, `arizona_western`, `coastal_bend_cc` were never checked at all (only their roster tables were pulled). `efsc` also untouched (deferred entirely). **No coach data was written anywhere in v44.87** — this is a verification gap, not a known error — but it means the stored coach for these 9 schools is unconfirmed against a live source as of this date. Batch 2: visit each school's `/coaches` (or equivalent) page via Claude-in-Chrome and cross-check name/title against `coaches.json`; only touch the file if a genuine mismatch is found (§3a Change Type 2).
...
- **🚩 NEW 2026-08-16 — `tyler_jc` may be a structurally broken site, not just an unread one.** Owner reports the athletics host required a VPN to reach at all (direct IP request was blocked — geo-block or similar, distinct from the CloudFront datacenter-IP block seen elsewhere), and once reached, the site still has no usable roster content — reads as a rebuild that isn't finished yet. This supersedes the old "SPA shell, needs a different read" framing: the problem isn't the read method, it's that the site itself may not be populated. **Do not keep retrying this with the sandboxed browser pane or Claude-in-Chrome from a datacenter IP** — it will fail for a different reason (block, not emptiness) and produce a misleading signal. Re-check periodically via a real residential browser; no ETA. `coaches.json.url` is also one of only 3 schools guide-wide with no stored URL at all (§6D) — same underlying cause.
...
- **🚩 §14's Opportunity Score table CANNOT reproduce a single stored JUCO trajectory — there is no written rule that regenerates JUCO minutes projections (found Session 4, 2026-08-06).** The table caps **Yr1 at 40–50%**, while **every** stored JUCO Yr1 sits at **56–72** (`murray_state_ok` 62, `tyler_jc` 68, `lsu_eunice` 72, `neosho_county_cc` 56). §14's documented *"JUCO adjustment (v26): ×1.2 multiplier"* does not close the gap either — murray's own shape (4 cleared, 10 returning ⇒ opp 4.5, ×1.2 = 5.4) lands in the **15–25% / 30–40%** row against its stored **62/78**. The JUCO values were hand-set in the v26 era and the §14 JUCO line is a remnant that no longer reconciles.
  - **Why it stayed hidden:** `trajectory[].pct` is **stored-only judgment data** — nothing recomputes it at runtime and no validator checks it against §14, the same silent-drift class as the Wake Forest `lensScores.value` bug (v44.30). Sessions 1–3 refreshed 4-year schools, where the table *is* the calibration and `apply_roster_refresh.py` reproduced results exactly; Session 4 was the first time the campaign met a **2-year college**.
  - **Measured impact of applying §14 as written:** `lsu_eunice` fit **64 → 43** (minutes 77 → 19) and `neosho_county_cc` **44 → 33** (minutes 62 → 31), with ~28 more schools similarly — JUCOs would fall from the top of the Minutes lens to the bottom, contradicting the guide's core JUCO thesis. **This was tried, measured, and reverted; do not "just apply the table".**
  - **Owner decision (2026-08-06): facts now, calibration later.** Session 4 refreshes only the factual fields (`mf_total`, `roster_season`, `cleared_*`, `recruit_*`) via `apply_roster_refresh.py`'s **`facts_only`** branch, which skips the trajectory and the whole cascade. Because `scores.js` reads **only** `trajectory[].pct`, that provably moves no score — verified by diffing `data/juco.json` for any changed scoring field (0 hits). Every refreshed JUCO note carries a **⚠ MIXED VINTAGE** disclosure saying the counts are 2026-27 while the trajectory is prior-season.
  - **To resolve:** build a JUCO opportunity→trajectory table calibrated so the existing anchors reproduce (murray 4.5 ⇒ 62/78, tyler ⇒ 68/80), add it to §14 beside the 4-year table, then re-derive all 30 in one campaign and re-rank once — the §5a/§5d pattern. Needs owner sign-off on the anchors, because it is a scoring-model change.
