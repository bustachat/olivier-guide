...
**Current version: v45.16 (2026-09-03).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
- **🚩 OPEN CAMPAIGN, started v45.15 — degreeTitle/acuAlign fabrication audit. Handover for a fresh session below; do not re-derive the scope, it's already computed.**
  ...
  **Scope, computed by diffing every current school id against the data files as they stood at the RULE 0 commit (`97218e9`, 2026-07-10) — 110 of 170 schools existed before RULE 0 and have never had their `degreeTitle` claim checked against a live source:**
  ...
  | Status | Count | Ids |
  |---|---|---|
  | ✅ Fixed (fabricated) | 2 | `indian_hills`, `iowa_western` |
  ...
  **✅ TIER 1 CLOSED (v45.16) — all 22 JUCOs checked: 21 confirmed real + 1 fixed (`iowa_western`).** Next up: Tier 2 (12 schools), then Tier 3 (69 schools).
  ...
  | 🚩 **Tier 2 — medium risk, check next** (D2/NAIA/D3, smaller programs) | 12 | `barry`, `chapman`, `columbia_college`, `csula`, `georgian_court`, `keiser`, `lynn`, `nova`, `ocu`, `pba`, `stedwards`, `uc_charleston` |
