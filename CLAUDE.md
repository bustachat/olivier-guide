...
**Current version: v45.15 (2026-09-03).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
- **🚩 OPEN CAMPAIGN, started v45.15 — degreeTitle/acuAlign fabrication audit. Handover for a fresh session below; do not re-derive the scope, it's already computed.**
  ...
  **Scope, computed by diffing every current school id against the data files as they stood at the RULE 0 commit (`97218e9`, 2026-07-10) — 110 of 170 schools existed before RULE 0 and have never had their `degreeTitle` claim checked against a live source:**
  ...
  | Status | Count | Ids |
  |---|---|---|
  | ✅ Fixed (fabricated) | 1 | `indian_hills` |
  | ✅ Spot-checked, confirmed real | 5 | `smc`, `tyler_jc`, `arizona_western`, `cowley_cc`, `angelina_college` |
  ...
  | 🚩 **Tier 1 — highest risk, check first** (JUCO, same risk class as Indian Hills: small schools, sparse public info) | 22 | `barton_cc`, `blinn_college`, `coastal_bend_cc`, `daytona_state`, `dodge_city_cc`, `efsc`, `iowa_lakes_cc`, `iowa_western`, `johnson_county_cc`, `lsu_eunice`, `miami_dade`, `mohave_cc`, `monroe_college`, `nassau_cc`, `neosho_county_cc`, `northeast_cc`, `phoenix_college`, `pima_cc`, `southeastern_cc_ia`, `suffolk_cc`, `ulster_cc`, `westchester_cc` |
