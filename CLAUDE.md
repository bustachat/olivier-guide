...
**Current version: v45.42 (2026-09-15).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
| **7G — Region 9/18 (West)** | `otero_college`, `northwest_college`, `trinidad_state`, `slcc`, `north_idaho_college`, `pacific_northwest_christian_college` | Watch the redshirt class-year trap at `slcc` (§15). |
