...
**1 — `negtest.py` was reported as 8/8; it was printing 7/8.** The `MAXAID / missing field` case's `find` string carried a **CRLF** while `data/conferences.json` is LF-only (0 CRLF, 903 LF), so `str.replace()` was a silent no-op and the tool reported **VOID / MUTATION-NOOP — "this test proved NOTHING"**. Confirmed pre-existing (7/8 at parent commit `a4f888e` too; `conferences.json` untouched since v44.85), so v45.13 did not break it — but reporting it as a pass is precisely the v44.50 self-deception `negtest.py` exists to prevent, recurring one level up: the script caught the no-op and the session's summary discarded the catch. Fixed the `find` string to `\n`; the suite now reports **8/8 proven · all branches proven**, and the MAXAID missing-field branch genuinely fires for the first time since it was written. Trap and rule recorded in §15.

**2 — the counts never reconciled.** v45.13's entry said a 26-school sweep was "cut to 14 genuinely broken", then "resolved 6 of the 14", then listed **15 still broken** — and 7 fixed + 15 broken = **22**, matching neither 14 nor 26. This is CLAUDE.md §15's own "never close an item that names a count without re-deriving the count" rule being broken inside the document that states it. Corrected to 22 in the v45.13 entry, with the correction shown rather than silently overwritten. Honest caveat recorded there: 22 is the reconciliation of that entry's own two lists, not a re-measured figure — every favicon fetch is unreachable from a sandboxed session (agent proxy returns `403 CONNECT` for `www.google.com` and every school domain), so the underlying hash/fetch findings could not be independently re-run.

**3 — §4 described a single icon chain "on all three render surfaces". That was false in two directions.** v45.13 updated §6 and left §4 stating `ICON_OVERRIDES → own /favicon.ico → Google proxy → initials` for card, modal and shortlist alike. The modal had just become a **4-stage** chain starting with `DOMAINS[id]` — a map that sentence never mentioned — and the Dashboard shortlist had never followed it at all. §4 now carries a per-surface table, states plainly that the modal is a deliberate strict superset (the only surface trying `DOMAINS[id]`, which is exactly where the UCLA bug lived), and documents `virginia`'s override as the one sanctioned case of a proxy URL used as an override *value*.

**4 — a third render surface was never examined, and it had the project's only third-party logo dependency.** `js/dashboard.js`'s shortlist cards led with `https://logo.clearbit.com/{domain}` — undocumented in §4, unique in the codebase, and enough to make one school render a Clearbit logo in the shortlist and a favicon on its own card. Removed; the shortlist now follows the same chain as the Explore card, so all three surfaces agree modulo the modal's extra `DOMAINS[]` stage. **Whether that endpoint was still alive could not be verified from this session** (network blocked), so the decision was made on consistency grounds, not on it being dead: if real non-favicon logos are wanted, that is an `ICON_OVERRIDES`/`assets/logos` decision applied uniformly, not one surface reaching out to an unpinned external service on its own.

**Two open items added to §6D, both from this audit.** (a) **The coloured-initials fallback is unreachable on every surface** — v45.13 correctly diagnosed that Google's proxy answers with a decodable ~726B globe under an HTTP 404, so `onerror` never fires again, and then routed *around* it rather than remediating it; the terminal initials stage in all three chains is therefore dead code, and the 15 broken schools show a meaningless globe where legible initials would be strictly better and free. A deterministic fix needing no research is noted there (skip the proxy for the already-hash-confirmed placeholder domains). (b) **The mirror-image case is unswept** — since v45.13 the modal is a strict superset, so a school with a broken own-`domain` favicon but a working `DOMAINS[]` one now renders in the modal and stays broken on card and shortlist; **83 of 170 schools have distinct values**, and nobody has checked how many land in that class.

**Also settled from the code what v45.13 could only hedge:** its §6 entry said the card was "likely" affected too for the 15 schools. It is — and so is the shortlist. Both read only the school's own `domain`, which for those 15 *is* the same broken host the modal fails on, so all three surfaces land on the same globe and one `ICON_OVERRIDES` entry per school fixes all three at once. Noted in §6D.

Files: `negtests/checks.json`, `js/dashboard.js`, `CLAUDE.md` (§4 rewritten, §6D +2 items and 1 extended, §15 negtest trap), `CHANGELOG.md`, `athletes/olivier.json` (guideVersion v45.13→v45.14). No data file, no scoring field, and no school record touched.
...
**A naive "which DOMAINS[] entries return ~726 bytes from Google's proxy" sweep produced a false-positive-heavy list of 26.** Re-verifying against what the app *actually* tries first (the direct `/favicon.ico` fetch, not the proxy) cut it down, because many of the 26 have a perfectly good *direct* athletics-domain favicon that the proxy check never exercised. **[Corrected v45.14 — the figure originally written here was "14", which contradicts this entry's own two lists and was never reconciled.** The entry names 7 schools fixed (`ucla`, `smc`, `northwestern`, `william_mary`, `ucsb`, `calpoly`, `virginia`) and 15 still broken; 7 + 15 = **22** genuinely broken end-to-end, not 14. The 15-school list was structurally re-verified in v45.14 (all 15 do have `DOMAINS[]` === `domain`, so no fallback exists) and the 7 fixed schools all do have a distinct `.edu` domain — but the underlying favicon fetches could not be re-run, so 22 is the reconciliation of this entry's own lists rather than a re-measured figure.**) This is the same "verify against the real thing, don't trust the intermediate signal" lesson this session's mascot-name re-audit had already surfaced once — it recurred immediately in a completely different part of the codebase.

**Fix — `js/app.js`, `buildModalHeader()` / `handleModalLogoError()`:** the modal now tries, in order: (1) `DOMAINS[u.id]` direct favicon, (2) the school's own `domain` direct favicon (skipped if identical to stage 1), (3) Google's proxy for the **school domain**, not the athletics one, (4) text initials. This resolved 6 of the 22 confirmed-broken schools for free, with zero research needed, because their university `.edu` domain already had a working icon the modal simply never tried: **`ucla`, `smc`, `northwestern`, `william_mary`** (all fixed via stage 2, direct `.edu` favicon) and **`ucsb`, `calpoly`** (fixed via stage 3 — their `.edu` domain's direct favicon also 404s, but Google's proxy for the `.edu` domain returns a real, hash-confirmed-non-generic icon). `virginia` needed a 7th `ICON_OVERRIDES` entry pointing straight at Google's proxy URL for `virginia.edu`, because the bare domain's TLS certificate is registered to a different hostname and hangs rather than failing promptly in a real browser (confirmed: `www.virginia.edu` also 404s directly, `uva.edu` doesn't resolve at all) — the override skips the slow/hanging direct attempt and goes straight to the confirmed-working (hash-verified, real Rotunda icon) proxy URL.
