...
- **✏️ RE-SCOPED (audited 2026-07-20) — `recruit_pathway` / `recruit_pathway_note`.** Previously logged as "populated only for the 4 v35 JUCO adds, full 93-school pass deferred." Live check (all `data/*.json` school entries) finds **0 of 220 school records have either key present at all** — not even the 4 originally claimed. Either the field was silently dropped in a later data rewrite, or the original "4 populated" count was wrong. Worth a quick git-blame/history check before resuming the full pass, since the pass may need to start from zero rather than 4/93. Still informational-only, no scoring cascade either way.
