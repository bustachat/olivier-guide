...
**Current version: v45.38 (2026-09-15).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
| **7C — Region 5/14 (TX)** | `blinn_college`, `hill_college`, `jacksonville_college`, `texas_southmost`, `ranger_college` | NJCAA already showed Blinn's 2026-27 roster; **still start on Blinn's own site** (§15). |
