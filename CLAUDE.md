...
**Current version: v45.20 (2026-09-03).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
- **🚩 OPEN CAMPAIGN, started v45.15 — degreeTitle/acuAlign fabrication audit. Handover for a fresh session below; do not re-derive the scope, it's already computed.**
  ...
  **Scope, computed by diffing every current school id against the data files as they stood at the RULE 0 commit (`97218e9`, 2026-07-10) — 110 of 170 schools existed before RULE 0 and have never had their `degreeTitle` claim checked against a live source:**
  ...
  | Status | Count | Ids |
  |---|---|---|
  | ✅ Fixed (fabricated/false claim) | 4 | `indian_hills`, `iowa_western`, `lynn` (all three fully fabricated degrees — the school has no such program at all), `chapman` (partial — real degree, renamed Kinesiology→Applied Human Physiology, but fabricated course codes attached to it) |
  | ✏️ Minor correction (real program, degree-type/course-code accuracy only) | 5 | `stedwards` — real, extensive Kinesiology program (6 tracks incl. Pre-PT), but it's a B.A. not the stored B.S.; specific course codes unverifiable, replaced with confirmed real course titles. `butler` — real degree is "Kinesiology, B.S.", not "Exercise Science"; real courses use `KIN` prefix, not `ES`. `cal` — no "Integrative Human Physiology" or Kinesiology major exists at any UC; real closest match is "Integrative Biology, B.A." (not B.S.); `acuAlign` corrected 12→3, `lensScores.academic` 88→31 (fitOlivier untouched). `charleston` — real degree is "Exercise Science, B.S.", not "Kinesiology"; real courses use `EXSC` prefix, not `KIN`. `charlotte` — real UNDERGRAD degree is "Exercise Science, B.S." ("Kinesiology" is the grad-only name); real courses use `EXER` prefix, not `KINE`. None of these five are fabrications — real, substantive programs with a wrong name and/or fabricated course-code prefix. |
  | ✅ Spot-checked/verified, confirmed real | 37 | `smc`, `tyler_jc`, `arizona_western`, `cowley_cc`, `angelina_college` (5 pre-campaign, checked during the Indian Hills investigation), `barton_cc`, `blinn_college`, `coastal_bend_cc`, `daytona_state`, `dodge_city_cc`, `efsc`, `iowa_lakes_cc`, `johnson_county_cc`, `lsu_eunice`, `miami_dade`, `mohave_cc`, `monroe_college`, `nassau_cc`, `neosho_county_cc`, `northeast_cc`, `phoenix_college`, `pima_cc`, `southeastern_cc_ia`, `suffolk_cc`, `ulster_cc`, `westchester_cc` (21, the rest of Tier 1), `barry`, `columbia_college` (its stored "⚠ No Exercise Science" disclosure is accurate — real offerings are Healthcare Management / Health Sciences), `csula`, `georgian_court`, `keiser`, `nova`, `ocu`, `pba`, `uc_charleston` (9, the rest of Tier 2), `akron`, `army`, `calpoly` (3, Tier 3 first batch) |
  ...
  **Tier 3 IN PROGRESS (v45.20) — 8/69 checked: 3 confirmed real (`akron`, `army`, `calpoly`) + 5 minor corrections (`butler`, `cal`, `charleston`, `charlotte`, plus `stedwards` counted above from Tier 2's same finding class).** Pattern so far: Tier 3 is producing mostly "real program, wrong degree name/course prefix" findings rather than full fabrications — worth watching whether that holds across the remaining 61.

  | 🚩 **Tier 3 remaining — lower risk, check last** (major D1 conferences — large, well-documented programs, but still technically unverified) | 61 | `clemson`, `creighton`, `csuf`, `delaware`, `denver`, `depaul`, `drexel`, `duke`, `elon`, `fau`, `fiu`, `gcu`, `georgetown`, `hofstra`, `indiana`, `louisville`, `marquette`, `maryland`, `memphis`, `mercyhurst`, `michigan`, `michiganstate`, `monmouth`, `navy`, `ncstate`, `northeastern`, `northwestern`, `notredame`, `ohiostate`, `pennstate`, `pittsburgh`, `princeton`, `providence`, `rutgers`, `setonhall`, `smu`, `stanford`, `stjohns`, `stonybrook`, `syracuse`, `temple`, `tulsa`, `uab`, `uca`, `ucdavis`, `ucirvine`, `ucla`, `uconn`, `ucriverside`, `ucsb`, `ucsd`, `unc`, `usf`, `vermont`, `villanova`, `virginia`, `wakeforest`, `washington`, `william_mary`, `wisconsin`, `xavier`, `yale` |
