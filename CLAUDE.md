...
- **🚩 OPEN CAMPAIGN, started v45.15 — degreeTitle/acuAlign fabrication audit. Handover for a fresh session below; do not re-derive the scope, it's already computed.**
  ...
  **Tier 3 IN PROGRESS (v45.28) — 65/69 checked: 43 confirmed real + 14 minor corrections + 8 real fabrications (`duke`, `georgetown`, `northwestern`, `stjohns`, `ucsb`, `ucsd`, `villanova`, `washington`).** Don't assume prestige or thoroughness of documentation protects against fabrication — most of this campaign's worst finds came from selective, extremely well-documented schools. (`stedwards`'s minor correction is counted under Tier 2, not here.)

  | 🚩 **Tier 3 remaining — lower risk, final stretch** | 4 | `william_mary`, `wisconsin`, `xavier`, `yale` |
