...
- Current version: **v40.10 (July 2026)** — always verify with `git log --oneline -1` and `athletes/olivier.json` guideVersion; treat any hardcoded version in prose as a hint, not truth
...
**Current version: v40.10 (July 2026).** Always confirm with `git log --oneline -1` and `guideVersion` in `athletes/olivier.json`. All v39 work is committed and pushed (`c456259` = v39.1–v39.6 squashed, `09c2ab7` = v39.7, `69cfc55` = failures summary); v40.1/v40.2 followed. See `v39_session_failures_summary.md` for the v39 incident log.
...
- **Social media verification for 4 of the 17 v39.1-v39.4 JUCO adds** — Region 1 completed v40.7; Region 6 v40.8; Region 11 v40.9; Region 14 (Blinn — dept FB only; Coastal Bend — college FB only; Angelina — program IG+X found; LSU Eunice — program X+FB found) completed v40.10. All via Chrome MCP: harvested from each official athletics site; IG/X accounts navigated and confirmed active per §7 Phase 1I. The remaining 4 (Region 15: Nassau, Ulster, Suffolk, Westchester) still have all four SOCIAL fields `null`. One final batch.
- **Coach email/phone for the remaining v39.1-v39.4 JUCO adds** — Tier-1 confirmed as of v40.10: Johnson County CC (Jeff Cole, v39), Phoenix College (David Cameron), Pima CC (David Cosgrove), Glendale CC (Jeff Perry), Dodge City CC (Juan Espinal), Iowa Lakes CC (Ben MacRae), Blinn (Michael McBride), Coastal Bend (Justin Rodriguez), LSU Eunice (John Plumbar — all v40.10). Genuinely not published, stay `null`: Mohave CC, Southeastern CC (Henrique Vieira confirmed still head coach), Angelina (Martin Melchor confirmed still head coach, names only). Neosho: see the coach-change item above. The remaining 4 (Region 15) are `null` from the v39 research; re-check each staff directory in the final batch. JUCO staff pages change contact info more often than D1 programs.
