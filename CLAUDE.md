...
**Current version: v44.93 (2026-08-17).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
| **5** | The NJCAA gap-fill campaign JUCOs (56 schools added v44.71–v44.85, region by region — the "39" figure in the `project-roster-refresh-campaign` memory is stale/wrong, 56 is the real count against `data/juco.json`). They carried accurate data from add time; the purpose here is re-verification, not a fix. Working sub-batches in add order, oldest first. | **🔄 IN PROGRESS.** Sub-batch A (Region 1, added v44.71) done v44.93 — `yavapai_college`, `eastern_arizona`, `csn_college` re-verified with real churn (new 2026-27 rosters now fully published; all 3 scores moved). `community_christian_college` re-checked, genuinely still unpublished (no 2026-27 roster yet). ~52 schools remain across Regions 2, 4–6, 9, 10, 14, 16–20, 24 — see CHANGELOG v44.93 for the full sub-batch order plan. |
