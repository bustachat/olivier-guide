...
**Access:** the first request returned a CloudFront 403. Appending a throwaway query string (`&_=1`) cleared it with no VPN, and the same thing worked on Tyler's own site. As on every site in this family, `navigate` returns before the table renders, so wait ~5s before reading.
