...
- Current version: **v44.34 (August 2026)** — always verify with `git log --oneline -1` and `athletes/olivier.json` guideVersion; treat any hardcoded version in prose as a hint, not truth (this line itself sat stale at v42.18 for 13 versions until v44.31 — the §6 state snapshot is the more reliable of the two)
...
- **🚩 NEW (v44.34) — 2 coach-contact discrepancies found live, NOT applied (Change Type 2).** Surfaced while Tier-1 verifying the URLs above; a contact edit is Change Type 2 and that commit was scoped to URLs.
  - **`stedwards` / Brian Young — BOTH fields differ and this one may be costing real outreach.** Live 2026 staff page: **`briany@stedwards.edu` / 512-448-8507**. Guide stores: `byoung@stedwards.edu` / 512-448-8415. If the live page is right, mail sent from the Outreach tab is bouncing today.
  - **`setonhall` / Andreas Lindberg — missing phone.** Live coaches page lists **(973) 275-6429**; the guide stores an empty string. The live page shows no email, so the stored `alindberg@shu.edu` is not contradicted.

  Both were read directly off each school's own staff page, so this is Tier-1 evidence, not a guess — apply whenever coach contacts are next touched, and note `ocu` already sits on the separate v44.27 conflict list below.
