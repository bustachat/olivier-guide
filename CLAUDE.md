...
**Current version: v45.47 (2026-09-16).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
- **"this batch" / "this campaign" appear in ~180 rendered strings** (139 + 63 in `juco.json` alone), e.g. "the best 2025-26 raw win total of this batch". Same class; `check_no_jargon.py` reports them as WARN. Re-derive with `python sweep.py "this batch" "this campaign"`, rewrite each comparison against something a reader can see (or drop it), then move both patterns from `WARN_PATTERNS` to `PATTERNS`.
