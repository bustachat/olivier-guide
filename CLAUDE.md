...
**Current version: v45.01 (2026-08-17).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
| **5** | The NJCAA gap-fill campaign JUCOs (56 schools added v44.71–v44.85, region by region — the "39" figure in the `project-roster-refresh-campaign` memory is stale/wrong, 56 is the real count against `data/juco.json`). They carried accurate data from add time; the purpose here is re-verification, not a fix. Working sub-batches in add order, oldest first. | **🔄 IN PROGRESS.** Sub-batch A (Region 1, v44.93) — 3 schools with real churn, 1 unchanged. Sub-batch B (Region 2, v44.94) — 4 schools unchanged, 1 coach vacancy filled, 1 coach interim-tag dropped. Sub-batch C (Region 4 Chicago + Region 5, v44.95) — `truman_college`, `daley_college`, `malcolm_x_college`, `kennedy_king_college`, `wilbur_wright_college`, `hill_college` all genuinely unchanged (exact name-for-name roster matches); `western_texas` had real churn (2 of last year's freshmen returned as sophomores exactly as projected, mf_total 7→8). Sub-batch D (Region 6 new schools, v44.96) — `coffeyville_cc`, `garden_city_cc`, `seward_county_cc` all genuinely unchanged, exact matches (including a 20-of-20 exact-set match at `seward_county_cc`). Sub-batch E (Region 9, v44.97, all 9 schools) — `northwest_college`, `trinidad_state`, `lamar_cc` unchanged; `central_wyoming` re-confirmed still genuinely blank-position; `casper_college`, `laramie_county_cc`, `gillette_college`, `northeastern_junior_college` had real churn; `western_nebraska_cc` flipped `available:false`→`true` (first real position data ever captured) — caught and fixed a stray-key validator error (MO-KEYS) from the flip leaving orphaned `note`/`reason` fields. Sub-batch F (Region 10 USC campuses, v44.98) — `usc_salkehatchie`, `usc_union` unchanged; `usc_sumter` had real churn (8→10 MFs); `usc_lancaster` is a second `available:false`→`true` flip (8 MFs) — the same stray-key cleanup applied proactively this time, caught on the first validator run. Sub-batch G (Region 14 Texas, v44.99) — all 4 (`paris_jc`, `jacksonville_college`, `northeast_texas_cc`, `texas_southmost`) genuinely unchanged, no patches needed at all this round. Sub-batch H (Regions 16/17/19/20 grouped, v45.00) — `jefferson_college_mo`, `harcum_college`, `hagerstown_cc`, `central_georgia_tech` unchanged; `crowder_college` (6→8 MFs) and `harford_cc` (10→5 MFs, a genuinely new turned-over roster class) had real churn. Sub-batch I (Region 18, v45.01, all 8 schools) — `snow_college`, `north_idaho_college`, `pacific_northwest_christian_college` unchanged (the latter's new roster is too sparse to use); `slcc` (10→12 MFs), `college_of_southern_idaho` (10→8, entirely fresh freshman group), `truckee_meadows_cc` (8→11) all had real churn; `colorado_northwestern_cc` and `usu_eastern` both flipped `available:false`→`true` (2 more first-ever real-data captures). ~5 schools remain: Region 24 (3, Illinois) plus `ranger_college` and `noc_enid`. |
