...
**Current version: v45.27 (2026-09-03).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
- **🚩 OPEN CAMPAIGN, started v45.15 — degreeTitle/acuAlign fabrication audit. Handover for a fresh session below; do not re-derive the scope, it's already computed.**
  ...
  **Scope, computed by diffing every current school id against the data files as they stood at the RULE 0 commit (`97218e9`, 2026-07-10) — 110 of 170 schools existed before RULE 0 and have never had their `degreeTitle` claim checked against a live source:**
  ...
  | Status | Count | Ids |
  |---|---|---|
  | ✅ Fixed (fabricated/false claim) | 10 | `indian_hills`, `iowa_western`, `lynn`, `duke`, `georgetown`, `northwestern`, `stjohns`, `ucsb`, `ucsd` (all nine fully fabricated degrees — the school has no such program at all; `northwestern` and `ucsd` both distinctively reused a *real* course prefix from an unrelated source — Michigan's `MOVESCI` and UCSD's own Education Studies `EDS`, respectively — rather than inventing one from nothing; `stjohns` is the only one with no real adjacent major found at all, `acuAlign` corrected to 0), `chapman` (partial — real degree, renamed Kinesiology→Applied Human Physiology, but fabricated course codes attached to it) |
  | ✏️ Minor correction (real program, degree-type/course-code accuracy only) | 14 | `stedwards`, `butler`, `cal`, `charleston`, `charlotte`, `delaware`, `gcu`, `louisville`, `setonhall`, `stonybrook`, `uab`, `uca`, `ucdavis` (see prior entries — all real programs, wrong name/prefix), `vermont` — real degree is "Exercise Science, B.S.", not "Kinesiology"; the stored course codes matched a different university's (UBC) numbering, not UVM's. None of these fourteen are fabrications — real, substantive programs with a wrong name and/or fabricated course-code prefix. |
  | ✅ Spot-checked/verified, confirmed real | 77 | `smc`, `tyler_jc`, `arizona_western`, `cowley_cc`, `angelina_college` (5 pre-campaign, checked during the Indian Hills investigation), `barton_cc`, `blinn_college`, `coastal_bend_cc`, `daytona_state`, `dodge_city_cc`, `efsc`, `iowa_lakes_cc`, `johnson_county_cc`, `lsu_eunice`, `miami_dade`, `mohave_cc`, `monroe_college`, `nassau_cc`, `neosho_county_cc`, `northeast_cc`, `phoenix_college`, `pima_cc`, `southeastern_cc_ia`, `suffolk_cc`, `ulster_cc`, `westchester_cc` (21, the rest of Tier 1), `barry`, `columbia_college` (its stored "⚠ No Exercise Science" disclosure is accurate — real offerings are Healthcare Management / Health Sciences), `csula`, `georgian_court`, `keiser`, `nova`, `ocu`, `pba`, `uc_charleston` (9, the rest of Tier 2), `akron`, `army`, `calpoly`, `clemson`, `creighton`, `csuf`, `denver`, `depaul`, `drexel`, `elon`, `fau`, `fiu`, `hofstra`, `indiana`, `marquette`, `maryland`, `memphis`, `mercyhurst`, `michigan`, `michiganstate`, `monmouth`, `navy`, `ncstate`, `northeastern`, `notredame`, `ohiostate`, `pennstate`, `pittsburgh`, `princeton`, `providence`, `rutgers`, `smu`, `stanford`, `syracuse`, `temple`, `tulsa`, `ucirvine`, `uconn`, `ucriverside`, `ucla`, `unc`, `usf` (42, Tier 3 so far) |
  ...
  **Tier 3 IN PROGRESS (v45.27) — 61/69 checked: 42 confirmed real + 13 minor corrections + 6 real fabrications (`duke`, `georgetown`, `northwestern`, `stjohns`, `ucsb`, `ucsd`).** Don't assume prestige or thoroughness of documentation protects against fabrication — most of this campaign's worst finds came from selective, extremely well-documented schools. (`stedwards`'s minor correction is counted under Tier 2, not here.)

  | 🚩 **Tier 3 remaining — lower risk, check last** (major D1 conferences — large, well-documented programs, but still technically unverified) | 8 | `villanova`, `virginia`, `wakeforest`, `washington`, `william_mary`, `wisconsin`, `xavier`, `yale` |
