...
**Current version: v45.36 (2026-09-14).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
|---|---|---|
| **7A — 4-year schools** | `clemson` (D1), `nova`, `csula` (D2), `chapman` (D3) | Done first: **full 4-year cascade**, and Clemson carries the highest fit (67). No NJCAA fallback exists for these. If a school still has no 2026-27 roster, it stays deferred. |
