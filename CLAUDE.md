...
- Current version: **v44.33 (August 2026)** — always verify with `git log --oneline -1` and `athletes/olivier.json` guideVersion; treat any hardcoded version in prose as a hint, not truth (this line itself sat stale at v42.18 for 13 versions until v44.31 — the §6 state snapshot is the more reliable of the two)
...
- **🚩 NEW (v44.33) — 4 broken `coaches.json` URLs, found by the first-ever sweep of that field. Candidates verified, corrections NOT applied.** v44.31's 333-link sweep covered school `url` + `SITE_URLS` + `DOMAINS` but **never `coaches.json.url`** — 108 entries that no sweep had ever checked. Probing all of them found 5 broken; `ocu` was fixed in v44.33 (it was the school being worked on). These 4 remain:

  | schoolId | stored (broken) | verified-live candidate | confidence |
  |---|---|---|---|
  | `setonhall` | `shupiratesl.com/sports/mens-soccer` — NXDOMAIN | `shupirates.com/sports/mens-soccer` → 200 | **high** — same stray-`l` typo v44.31 fixed in `DOMAINS`, never mirrored here |
  | `virginia` | `virginiasports.com/sports/mens-soccer` — 404 | `virginiasports.com/sports/msoc` → 200 | **high** — Virginia is the known non-JUCO `msoc` school (v44.31) |
  | `ncstate` | `gopack.com/sports/mens-soccer/roster/coaches/marc-hubbard/5258` — 404 | `gopack.com/sports/mens-soccer/coaches` → 200 | medium — the per-coach bio id rotated; re-pull Hubbard's current bio href |
  | `stedwards` | `sehawks.com/sports/mens-soccer` — NXDOMAIN | none — `stedwardshilltoppers.com` also does not resolve | **needs real research** |

  Not applied because the ask that surfaced them was a one-line override fix; batching four unrelated URL corrections into it would have buried the actual change. **Per-coach bio URLs rot** (the NC State case) — prefer the program `/coaches` page over a `/roster/coaches/<slug>/<id>` deep link when re-storing. **Also worth doing: fold `coaches.json.url` into §15's sweep method**, since it is currently the only stored-link field with no sweep coverage.
