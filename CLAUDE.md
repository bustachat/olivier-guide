...
**Current version: v45.39 (2026-09-15).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
| **7D — Region 1/8 + Santa Monica** | `efsc`, `phoenix_college`, `smc` | `efsc` published 2026-27 with **no positions** (Aug), so if its own page still lacks them, this is a legitimate NJCAA fallback case. `phoenix_college` is NJCAA **DII** (use `div2` in the NJCAA URL). `smc` is CCCAA, so it has **no NJCAA fallback**. |
