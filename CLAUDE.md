...
- Current version: **v44.47 (August 2026)** — always verify with `git log --oneline -1` and `athletes/olivier.json` guideVersion; treat any hardcoded version in prose as a hint, not truth (this line itself sat stale at v42.18 for 13 versions until v44.31 — the §6 state snapshot is the more reliable of the two)
...
- **🚩 OPEN (v44.47) — the Coaches-tab NAIA tier intro says "No scholarship maximum in NAIA — full packages possible", which contradicts §5c.** Found while fixing the Keiser string on the same line (`js/app.js`, NAIA tier intro). NAIA men's soccer is limited to **12 scholarship equivalencies**, which is exactly why §5c classifies NAIA as `fundingPathway: "capped"` and applies a **−3** Fit penalty. So the UI tells the user there is no cap while the scoring model penalises them for one. Deliberately **not** fixed in the v44.47 copy commit — it is a different error class (a scoring-model contradiction, not a stale fact) and worth a deliberate rewrite alongside the D2/NAIA aid strings. Check the Explore NAIA intro and the Glossary funding text for the same claim when fixing.
