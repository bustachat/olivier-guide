...
- Current version: **v44.53 (August 2026)** — always verify with `git log --oneline -1` and `athletes/olivier.json` guideVersion; treat any hardcoded version in prose as a hint, not truth (this line itself sat stale at v42.18 for 13 versions until v44.31 — the §6 state snapshot is the more reliable of the two)
...
**Current version: v44.53 (August 2026) — `negtest.py` added, and an audit for other prose-parsed display values found a REAL one: two cards showed a school name where a GPA belongs.** `negtest.py` mechanises the v44.50 lesson — **a validator's silence is only evidence if you proved the mutation landed.** It asserts the file changed and refuses to run the validator otherwise (reporting `VOID` rather than a pass), separates `CHECK-SILENT` from `PASS`, and always restores. `negtests/checks.json` holds **8 proven cases across MAXAID/TIER/FIT/SCORES-SRC**; the FIT case is a **permanent regression test for v44.51** — reintroduce a formula mirror and it goes silent. The audit ran every prose extraction in the renderers across all 111 schools: **`Target: Notre` / `Target: Georgetown`** was live on those two cards (`gpa.minSchol.split(' ')[0]` against a need-blind school's full sentence) — **FIXED** with a guarded extraction, 4 of 111 cards change. Two cosmetic instances logged not fixed (`soccerLevel` shows a bare "D1"/"JUCO" for 6 schools and a 59-char sentence for `denver`; `prePT` overflows for keiser/chapman), and the GPA *filter* extraction was verified sound across all 111 so it need not be re-audited. Issues: 0. PRIOR: **v44.52 (August 2026) — `sweep.py` added at the repo root: the exhaustive-search discipline is now a command, not a virtue.** Three consecutive sessions closed a copy/data item on a count that was a LOWER BOUND (v44.47 said 2/was 8 · v44.48 said 1/was 8 · v44.49's brief said 2/was **29**), because **a truncated search read as exhaustive is indistinguishable from a clean result.** The walker that got two of those right was hand-written and thrown away twice; it is now committed. Attributes every hit in `data/`+`athletes/` to its **owning record + JSON path**, never truncates, takes **several patterns at once** (a claim has many phrasings — v44.48 needed three), case-insensitive by default, docs excluded unless `--docs`. **Control-tested on creation:** 42 hits for `9\.9` matched an independent grep exactly, and the delta from v44.49's 29 reconciled line-by-line. **Its first real run also taught its own lesson** — `no cap|uncapped` returned 2 hits, both false (`"no capacity"`, `"uncapacitied"`), so read the rows, not the total. Issues: 0. PRIOR: **v44.51 (August 2026) — the validator stopped reimplementing the Fit Score; it now loads and calls the real `js/scores.js`.** The FIT check reconciled all 111 stored scores against **its own copy** of `calculateFitScore()` and five helpers, so a formula change that missed the mirror would be blessed with `Issues: 0` while every ranking was wrong. **Proven, not theorised:** with `housingPenalty` 6→10 in `scores.js`, the OLD validator reported `Issues: 0` and the new one 10 drifted schools. All 48 lines of duplicated formula deleted; `scores.js` byte-identical to HEAD (SHA-verified). Loaded via `vm` — **do NOT add `module.exports` to `scores.js`** (§4: no build step) — and the loader **throws rather than falling back**, since a silent fallback to a mirror is the bug removed. Four negative tests, each asserting the mutation landed first (v44.50 lesson), which also caught a broken harness of my own. Mirror had NOT yet drifted, so no scores were wrong — the exposure was latent. Issues: 0. PRIOR: **v44.50 (August 2026) — the Conferences card's "Max Aid" stat no longer parses prose; `maxAid` is now a stored field on all 25 conferences.** The tile was derived with `c.scholarships.split('Up to')[1]…`, rendering a WORD for **10 of 25** conferences ("NCAA" ×3, "Army", "NAIA", "equivalent" ×2, "Athletic", "ZERO" ×2). New **`MAXAID`** validator check enforces the field's presence and ≤12-char length **and greps `js/app.js`** so the parse cannot return — the data checks alone would not see the renderer regress. All five branches negative-tested, and **the first negative test passed while proving nothing** (patched the wrong indent, so the mutation never applied) — assert the mutation landed before trusting a validator's silence. Also: `README.md` version header bumped. Issues: 0. PRIOR: **v44.49 (August 2026) — the D1 "9.9 equivalencies" figure is now opt-in-aware; the sweep found TWENTY-NINE strings, not the 2 the owner's brief named.** *House v. NCAA* (1 July 2025) made a flat 9.9 a partial truth: non-opt-in schools still run the cap, opt-in schools carry a 28-player fully-fundable roster. **9.9 was qualified, never dropped.** The distinction was **sized to its surface** — Glossary gets a new canonical *"The House Settlement"* entry (including the explicit admission that the guide does not track opt-in status, and the recruit-facing point that *permission to fund 28 is not the same as funding 28*), the Financial Model card gets one clause, and the 25 one-line table cells get a compact suffix. The **comparative** strings were the genuinely misleading ones — the NAIA intro and OCU's note both argued "more than D1 (9.9)", so the D1 half was dropped and the D2 half kept (9.0 is untouched by the settlement). **Text only; no score moved and none should** (§5c: D1 ⇒ `full` ⇒ 0 penalty regardless of opt-in). Newly logged, NOT fixed: the Conferences-card **"Max Aid" stat is parsed out of the prose** and renders "NCAA"/"Army"/"NAIA"/"equivalent" for 10 of 25 conferences — pre-existing, measured before and after, and a *renderer* bug that must not be "fixed" by reshaping copy. Issues: 0. PRIOR: **v44.48 (August 2026) — the UI told the user NAIA has no scholarship cap while §5c penalises it −3 for exactly that cap; EIGHT strings fixed.** The lost distinction: NAIA men's soccer has a **12-equivalency TEAM cap** (more generous than D1's 9.9 and D2's 9.0, but a cap) and **no PER-PLAYER cap**, so a coach can concentrate the pool into a full ride. All eight collapsed the second into "no cap". **The Glossary already said it correctly** (*"NAIA has 12 with no per-player cap"*), so this was making the rest match, not inventing a framing. Fixed across `index.html`, `js/app.js`, `conferences.json` ×2, `conf-prestige.json`, `coaches.json`, `d2.json` ×2. **Text only — no `fundingPathway`/`maxAthletic`/`costNum`/score moved, and none should: the −3 was always right, the copy was wrong.** Strings like *"Up to equivalent of full ride"* were deliberately KEPT — a full ride genuinely is attainable individually, and `ocu`/`keiser` store `maxAthletic: 1.0` **with** `capped`, which is that split exactly. **Method: search SEVERAL PHRASINGS** — `no cap|no maximum` found 5, the equivalency angle found 2 more, and *"not limited by equivalency caps"* needed a third pattern. Newly logged, not fixed: the **D1 "9.9 equivalencies"** figure may be stale post-*House* (opt-in schools run a 28-player fully-fundable roster), but correct copy needs per-school opt-in status the guide deliberately doesn't track. Issues: 0. PRIOR: **v44.47 (August 2026) — two pre-existing UI copy errors fixed, both surfaced by v44.46's Change Type 3 prose sweep and neither catchable by any validator.** (1) The Glossary's Minutes Outlook entry claimed *"Barry D2 consistently ranks #1 under this lens — 7 of 13 midfielders clear"*. Barry's minutes score is **59**; `iowa_western` leads at **79**. It was never true. **Rewritten to describe what the lens rewards rather than name a winner** — naming a #1 is the same defect class `PROSE` exists for, and Session 4 refreshes all 30 JUCOs, i.e. the very schools atop this lens. (2) **Keiser was still "Fort Lauderdale" in EIGHT places, not the two logged** — 2 × `js/app.js` NAIA intros, `data/aac.json` FAU `olivierMatch` (wrong direction as well as city), `data/conferences.json` ×2 (including the Conferences-tab school chip), `data/coaches.json` ×2 (Oldham bio + strengths), and `data/pipeline.json`'s Keiser row. **Two lessons: re-grep when closing a "N known instances" item** (the note came from a js/html glob and never swept `data/`), **and never conclude "none remain" from a head-limited grep** — the first fix pass asserted the repo was clean on the strength of a search truncated to 12 results, and the remaining five were caught only because a *cached* `js/app.js` (v37.3 gotcha) kept rendering the old strings. Also newly logged, NOT fixed: the Coaches NAIA intro says *"No scholarship maximum in NAIA"*, contradicting §5c's `capped` classification and its −3 penalty. Issues: 0. PRIOR: **v44.46 (August 2026) — Wave 1 Session 3 of the 2026-27 roster refresh: `d2`, 4 of 6 flipped schools refreshed (Change Type 3, full cascade each), 2 deferred, and `ivy` dropped from the campaign entirely.** Survey re-run first; 8 of 14 flipped, matching the 2026-08-04 list with **zero churn**. **`ivy` is OUT OF SCOPE by owner ruling** — *"I dont care about Yale and Princeton. They are unattainable as they dont offer scholarships."* Both parse cleanly but stay `available:false`; **this is a research-effort ruling, not a removal** — do not delete them, and skip `ivy.json` in future campaigns. Refreshed: `columbia_college` 35→36, `ocu` 59→**50**, `pba` 61→**62**, `stedwards` 61→**54** (OCU and St. Edward's fall because their stored trajectories were flat and unsupported — OCU's Yr1–Yr3 were all 50; PBA rises because **7 of its 10 MFs clear before 2027**, a graduate-transfer roster model). **`recruit_pathway`: columbia_college reclassified Portal/JUCO-heavy → Mixed** (it publishes Hometown / High School / Previous School as three separate columns, so the split is *readable* — only 3 of 12 MFs are transfers); ocu + pba confirmed; stedwards retained (no previous-school column). **No coach changed at any of the 6 schools** — no Change Type 2. **Two deferred to Wave 2 as published-but-not-populated (instances 4 and 5): `keiser`** (full coaching staff, **zero players**; its own 2025 page returns 34 through the identical read) and **`barry`** (21 players vs 34, and the shape gives it away — **ONE goalkeeper** and 12 of 21 midfield-capable). **NEW DIAGNOSTIC: check the goalkeeper count.** Squad-size ratio alone was ambiguous for Barry (62% vs Pittsburgh's deferred 50%); a positional breakdown showing 1 GK is not. **NEW CAVEAT CLASS — a blank *position* cell is a field-level gap, not a roster-level one:** `ocu` publishes 3 of 25 and `stedwards` 6 of 39 players with an empty position cell (St. Edward's bio pages have no position field either), so `mf_total` counts only confirmed midfielders. Materiality was *checked, not assumed* — OCU's worst case stays inside its trajectory row, St. Edward's would cross one, so only that note flags a re-check. Tooling: `roster_survey.py` is now argv-driven, and `roster_extract.py` learned **`Fy.`** (the Ivy first-year label) which had been silently falling into `unknown` — same class as Session 2's `4th` bug, and again caught by the standing control test. Issues: 0. **Refresh ledger: 60 on 2026-27, 45 on 2025-26, 6 available:false.** PRIOR: **v44.45 (August 2026) — three UI-integrity fixes found by the OWNER looking at the app, not by any check; two new mechanical checks now cover that ground.** **v44.43** fixed two panels the roster refresh had outdated: the Minutes Outlook key was written in a 2025 roster's class years and **inverted** once schools moved to 2026-27 (a 2026-27 junior is a 2027 senior, not cleared), and seven `CONF_SECTIONS` intros were stale — one of them (`asun`) claiming UCA had "6 of 9 MFs clearing" when v44.42 had just made it 0 of 9, one (`big-ten`) naming **USC**, and the Academic-First lens naming **UF** — neither of which fields a team here. **v44.44** added the **`PROSE`** validator check (section program counts, `"N of M MFs"` roster claims, scrape-season class-year phrasing, phantom school anchors), which found three more real errors on its first run. **v44.45** fixed the conference filter chips: **six schools had no chip at all** (army, navy, delaware, mercyhurst, uc_charleston, columbia_college — the row summed to 105 of 111) and **UCA sat in the wrong chip** because the alias scan used bare `.includes()` and `"sun conference"` matched inside `"asun conference"`, filing a D1 school under the NAIA Sun Conference. Added the **`CHIPS`** check. Both new checks were **negative-tested** — and that test caught a real gap: the CHIPS mirror reimplements the matching, so it was blind to the resolver itself regressing until a code-shape guard was added. **Standing lesson: prose and UI wiring are the two areas no validator could previously see.** PRIOR: v44.42 (August 2026) — Wave 1 Session 2 of the 2026-27 roster refresh: 28 of 33 schools in `big-ten`+`big-west`+`caa`+`d1-other` refreshed to 2026-27 (Change Type 3), full cascade each, one conference file per commit, Issues: 0 at every gate.** **Method change: these rosters were PARSED, not browsed** — on all four files' hosts the roster table is server-rendered (position, academic year AND previous-school column all in raw HTML; only the `<title>` year is client-injected), so a Python extractor replaced Session 1's per-school browser work. It was **control-tested against 8 committed Session-1 schools and reproduced all 8 exactly** before being trusted, and it caught two of its own bugs in the process: a substring header match missed the class column at Louisville (`CL`) and Duke (`Yr.`), and **ordinal eligibility labels (`1st`–`5th`, used by Indiana and Washington) dropped every `4th`-year graduating senior into "unknown"**. **A fourth roster template is now on record (§15): WMT list view** (`.roster-list-item` with semantic per-field classes), plus Penn State's `.player-list-item` variant. **`pennstate` DEFERRED to Wave 2 — the third "published but not populated" instance**: 8 players and ZERO midfielders beside a full coaching staff, while its 2025 page returns a complete squad. **`stonybrook` populated for the FIRST time**, closing a gap open since v21; `available` flips false→true and its Fit falls 43→34 because the real outlook is worse than the neutral 0.5 it had been carrying (the neutral working as designed). **`recruit_pathway`: indiana → Transfer-preferred** (4 of 7 MFs are 4-year transfers and the midfield has no 1st- or 2nd-year player at all) and **northwestern → Mixed**; 13 schools publish no previous-school column and retained their value with a lower-confidence note. **New trap: the previous-school column can hold CLUBS, not colleges** — calpoly, northeastern and uca would all have been misclassified by counting entries rather than reading them. **No coach changed at any of the 29 schools researched** — no Change Type 2, no re-rank. `recruit_risk` was deliberately RETAINED, not re-derived (unscored, out of scope). Refresh ledger: **56 schools on 2026-27, 49 on 2025-26**. Full detail: CHANGELOG.md v44.39–v44.42. PRIOR: v44.38 (August 2026) — Wave 1 Session 1 of the 2026-27 roster refresh: 27 of 31 schools in `aac`+`acc`+`big-east` refreshed to the 2026-27 season (Change Type 3), full cascade each.** `mf_total`+`roster_season` written together every time (v44.32 rule). **Both `MO_MISSING_OK` whitelist entries closed** — notredame (3 rising seniors) and georgetown (4); the Set is now empty but kept as the documented escape hatch for a real research gap. **2 deferred to Wave 2, same failure class: a published season page is not a populated one** — `tulsa`'s 2026 page has coaches/staff and zero players (control-tested: its 2025 page returns 29 players through the same extractor), `pittsburgh`'s lists 13 players against 26 in 2025. Both keep their 2025-26 data and label. **Method fix (v44.36b): §14's Opportunity Score ranges were being resolved by feel** — two schools with different opp scores had landed on identical trajectories — so trajectory is now *derived* from the researched counts by linear interpolation within the row, in `apply_roster_refresh.py`. **`recruit_pathway` reclassified for 2** (memphis → Portal/JUCO-heavy, xavier → Mixed); **11 schools publish no previous-school column**, so their classification was retained rather than re-derived and each note says so. **No coach changed at any of the 29 schools researched** — no Change Type 2, no re-rank. Issues: 0. Full detail: CHANGELOG.md v44.36–v44.38. PRIOR: v44.35 (August 2026) — St. Edward's coach contact corrected + Seton Hall phone added (Change Type 2, contact only, NO re-rank since `overallScore` didn't move).** `stedwards` email `byoung@`→`briany@stedwards.edu` + phone `512-448-8415`→`512-448-8507`; `setonhall` phone `""`→`973-275-6429`. Tier-1 re-verified immediately before writing (`mailto:`/`tel:` hrefs on the 2026 staff pages, not display text). **Lesson: a coach's email can be hardcoded in the `bio` string as well as `contact{}`** — St. Edward's bio ended with `"Email: byoung@stedwards.edu"`; grep bios too on any contact change. **Seton Hall's EMAIL was deliberately left alone** — the live page publishes no email, the stored `alindberg@shu.edu` appears nowhere on the site and doesn't match the `firstname.lastname@shu.edu` pattern every colleague uses, but deriving it would be a guess (§6). Also logged: **13 coach cards render a literal `"null"` for Yrs HC** (pre-existing, confirmed against HEAD — same class as the v43.12 null-contact guard), and **the Outreach tracker renders only the 10 shortlisted schools**, so it can't be used to spot-check contact data. Issues: 0. Full detail: CHANGELOG.md v44.35. PRIOR: v44.34 (August 2026) — the last 4 broken `coaches.json` URLs cleared; that field now sweeps 108/108 live (0 × 404, 0 × NXDOMAIN).** All Tier-1 browser-verified: `stedwards` → `gohilltoppers.com/sports/mens-soccer` (the dead `sehawks.com` replacement — and note `goseu.com` returned HTTP 200 while serving a Chinese streaming site, so content had to be read, not just status codes), `setonhall` → `shupirates.com/…`, `virginia` → `virginiasports.com/sports/msoc`, `ncstate` → `gopack.com/sports/mens-soccer`. **NC State proved the rot hypothesis:** its stored bio id `5258` now serves as `5017` — same coach, rotated id. All four stored as **program pages** per the 81-of-108 convention; never store `/roster/coaches/<slug>/<id>`. **Cross-check: every corrected URL already existed correct in the school object, and the dead hosts appeared nowhere else in the repo** — confirming `coaches.json.url` was the single stored-link field no sweep covered. No UI change is possible: that field has **no renderer consumer**. Two coach-contact discrepancies found live and logged in §6, NOT applied (Change Type 2) — St. Edward's stores an email AND phone that both differ from its live staff page. Issues: 0. Full detail: CHANGELOG.md v44.34. PRIOR: v44.33 (August 2026) — `rosterUrl()`'s per-school `overrides` map DELETED; OCU coach URL fixed.** Found by the 2026-27 availability survey, not a bug report. All 4 overrides audited: `lynn`/`csula`/`keiser` were **byte-identical** to what the function's own rule derives (pure no-ops), and `ocu` — the only one that changed anything — pointed at `okcu.edu/athletics/soccer/roster`, a **404**. OCU's school object already held the correct `www.ocusports.com/sports/mens-soccer`, so deleting the map IS the fix (Miami Dade precedent, v42.13). **The lesson: a hardcoded override silently masks the school object's `url` and is invisible to both validators AND to §15's URL sweep, which checks `url`/`SITE_URLS`/`DOMAINS` only — do not re-add per-school roster URLs.** Also fixed `coaches.json` OCU `url` (same dead `okcu.edu` host, 404) → Billy Martin's live bio page, Tier-1 browser-verified. A sweep of all 108 coaches.json URLs found **4 more broken, logged in §6 not fixed** (`virginia`, `ncstate`, `stedwards`, and `setonhall` still carrying the `shupiratesl.com` typo v44.31 fixed in DOMAINS but never mirrored here). No scoring impact. Issues: 0. Browser-verified: all 7 derived roster URLs 200, lynn/csula/keiser unchanged, the v42.5 JUCO `/index` fallback intact. Full detail: CHANGELOG.md v44.33. PRIOR: v44.32 (August 2026) — `minutesOutlook.mf_total_2025` renamed to `mf_total` + new required `roster_season` (2026-27 roster refresh campaign, Session 0). The old key baked a season into its NAME while the value is just "midfielders on whichever roster was last scraped" — Murray State (v44.29) was already researched off its 2026-27 roster and displayed a 2026-27 count under a hardcoded "MFs (2025)" label. Done FIRST because the campaign writes this field for all 111 schools; renaming after would migrate every record twice. 104 schools migrated (every `available:true` school): 103 → `"2025-26"`, 1 (`murray_state_ok`) → `"2026-27"`. Seasons were sourced, not guessed — §5 documented the field as the 2025 count, `MO-KEYS` has enforced that key name since v40.2, and `smc`/`tulsa` notes state their season explicitly. Both renderers (app.js:1381 modal, app.js:3304 card) now derive the label from `roster_season`, falling back to "Midfielders" rather than "MFs (undefined)". Validator: `roster_season` required on `available:true`, plus a `YYYY-YY` format check — because athletics sites label fall 2026 as "2026" at calendar-year schools and "2026-27" at academic-year ones, and this string renders verbatim. Both new checks negative-tested before being trusted. NO scoring cascade — `scores.js` reads only `trajectory[].pct`, never `mf_total`. Issues: 0. Browser-verified across all 104 cards (103 × "MFs (2025-26)", 1 × "MFs (2026-27)", zero `undefined`) plus both modals. Bonus: `roster_season` doubles as the campaign's refresh ledger. Full detail: CHANGELOG.md v44.32. PRIOR: v44.31 (August 2026) — Notre Dame's dead `url` fixed + a full 333-link sweep found two more dead hosts. `notredame.url` was `/sports/mens-soccer` (HTTP 404); Notre Dame uses `/sports/msoc`. Corrected in BOTH `data/acc.json` and `data/coaches.json` (the coach entry carried the same dead string). Stored as the **bare** path, matching Virginia — the only other non-JUCO msoc school — because the 20 schools storing `/sports/msoc/index` are all JUCO/Sidearm, where `rosterUrl()` deliberately stops at the program page (v42.5); the bare path lets it build the working `/sports/msoc/roster`. Note `u.url` does NOT drive "Visit Site" (that's `SITE_URLS`) — it drives the modal's "Men's Soccer →" link, the Compare tab's "Visit →", and `rosterUrl()`. The sweep (111 `url` + 111 `SITE_URLS` + 111 `DOMAINS`) found two further NXDOMAIN hosts: `tyler_jc` `www.apacheathletics.com` → bare `apacheathletics.com`, and `setonhall` `DOMAINS` `shupiratesl.com` (stray trailing `l`) → `shupirates.com` — the latter silently broken, since Google's favicon service returns a generic globe rather than an error. **Only NXDOMAIN was treated as proof of death**: ~15 live hosts answer 202/403 (Cloudflare) or throw SSL/timeout errors to scripted clients. `DOMAINS.gcu = 'lopes.com'` is suspected wrong but resolves, so it was logged in §6 rather than guessed. No scoring change (`url`/`DOMAINS` aren't read by scores.js); ND holds fit 45 / value 27. Validator Issues: 0 against v44.30's new VALUE check. Full detail: CHANGELOG.md v44.31. PRIOR: v44.30 (August 2026) — Wake Forest value-lens drift fixed + a new `VALUE` check in validate_consistency.js (Change Type 4 + 11): `wakeforest.lensScores.value` was stored 50 against a formula value of 29 (costNum $91k caps costRatio at 1.0, so affordability floors at 0 and value should equal `0.6 × fitOlivier` exactly). It ranked 30th of 111 on the Value lens with 71 cheaper schools below it; now 81st. Root cause is structural: unlike `fitOlivier`, which `scores.js` recomputes on every page load, **`lensScores.value` is stored-only and nothing recomputes it** — it is hand-written during the §3a Type 4/12 cascades and thereafter only read, so it was the one derived school score that could drift silently and indefinitely. The new VALUE check closes that gap (mirrors the FIT check's structure and `>1` tolerance; proven to fire on the known bug). Army and Navy were flagged by the same sweep (stored 47/45 vs formula 66/65) and deliberately NOT changed — §4 zeroes service-academy `fin{}` by rule, which saturates affordability and would credit them +40 for tuition actually paid via a 5-year service commitment; the check exempts `costNum === 0` and the design question is logged in §6 deferred items. Full detail: CHANGELOG.md v44.30. PRIOR: v44.29 (August 2026) — Added Murray State College (OK) as the 111th school and 30th JUCO (Change Type 1 + 14); full detail: CHANGELOG.md v44.29. PRIOR: v44.28 (July 2026) — Removed athlete-specific personalization from all coach bios (Change Type 2): a Drexel bio factual error ("Belgian" instead of "Australian") led to a broader check — 15 of 110 coach bios named "Olivier" or referenced his specific August 2027 arrival date, which is wrong architecture for an athlete-agnostic file. All 15 genericized (substance kept, name/date dropped); new standing rule added to §5 so this doesn't recur. Full detail: CHANGELOG.md v44.28. PRIOR: v44.27 (July 2026) — Coach data architecture consolidation: `coach{}` removed from all 110 school objects; `coaches.json` is now the SOLE source, looked up live via `getCoach(schoolId)` (js/app.js). Prompted by the owner spotting mismatched coach profile text between the Details modal and the Coaches tab ranking card — confirmed as real drift (21 schools had disagreeing email/phone between the two sources), not a display bug. Added `title` to coaches.json's schema (migrated from the removed field, no data loss). Two owner-verified live corrections applied (Drexel email, Elon title/email/phone). 10 schools have an unresolved contact conflict where both sides had different real values — logged in §6 for future Tier-1 re-verification rather than guessed. Modal's Coach & Contact tab now also shows the coach's rank badge. `COACH-SYNC` validator check repurposed to catch a resurrected school-level `coach` key. Full detail: CHANGELOG.md v44.27. PRIOR: v44.26 (July 2026) — Pathways tab: new "Recruiting Pathway by School" section (Change Type 11), the first UI consumer of the `recruit_pathway` data completed last session. Groups all 103 populated schools into 4 cards (Freshman-friendly/Mixed/Transfer-preferred/Portal-JUCO-heavy) computed live from the `unis` array — no hardcoded list, stays correct as schools change. Added a `RECRUIT_PATHWAY_VALUES` enum check to validate_consistency.js so a bad value errors instead of silently disappearing from the tab. Full detail: CHANGELOG.md v44.26. PRIOR: v44.25 (July 2026) — Somoano (UNC) coach bio/record factual fix (Change Type 2, two-file rule): `coaches.json` record/bio/strengths + `acc.json` coach.profile both corrected the "Dorrance dynasty legacy program" conflation (UNC women's coach, separate program) to state Somoano's own real men's-title record — 2001 (Bolowich) and 2011 (Somoano's first year as HC). No overallScore/rank change (already correctly anchored at 88 per §5d); this closes the item deferred since v44.14. Full detail: CHANGELOG.md v44.25. Also completed this session: the `recruit_pathway` full-population campaign (v44.16–v44.24) — 103/110 schools now populated, remaining 7 structurally blocked (`available:false`); see §6 deferred items and CHANGELOG.md for full per-batch detail. PRIOR: v44.15 (July 2026) — Coaches tab label fix: coach-card stat "MLS Picks" relabelled "MLS Players Dev" (Change Type 11, display-only). Owner spotted the confusing juxtaposition of Steve Clements' "10 MLS Picks" coach-card stat against Tyler JC's 0 in the Pro Pipeline `mlsDraft` table — not a bug, two different metrics (coach's career-long developed-players count vs. school's 5yr direct-draft-picks count, see §5b) sharing a misleading label. Relabelled to match the Rankings table's already-correct wording. Full detail: CHANGELOG.md v44.15. PRIOR: v44.14 (July 2026) — Pro Pipeline tab reconciliation (Change Type 7, display-only): added 9 missing NCAA D1 titles (Clemson, Vermont, Georgetown, Syracuse, Notre Dame, Stanford, Maryland, Duke, Navy) and 2 JUCO titles (Phoenix College, Pima CC) to `pipeline.json`'s ranked tables — all were already correctly documented in each school's own `titles[]` but never surfaced in the shared Pro Pipeline tab tables. Added Akron and NC State to the `mlsDraft` table (both previously absent despite guide-documented pipelines). Fixed a standalone factual error in UNC's `titles[]`: wrongly credited women's coach Anson Dorrance with years 1978/1979 — corrected to the real men's titles, 2001 (Bolowich) and 2011 (Somoano's first year as HC), verified via NCAA.com/Wikipedia. No fitOlivier cascade (scores.js doesn't read titles/pipeline.json). Full detail: CHANGELOG.md v44.14. Deferred: `coaches.json`'s Somoano `record` field repeats the same wrong "Dorrance dynasty" framing — Change Type 2 territory (two-file rule + mandatory full re-rank), out of this session's scope. See CHANGELOG.md for the v44.11–v44.13 UX sessions between this and the v44.10 conference-reclassification work below. PRIOR: v44.10 (July 2026) — men's-soccer conference reclassification (Delaware→Summit precedent): Akron MAC→Big East (confKey, from 2023 when the MAC dropped men's soccer) and Army & Navy AAC→Patriot League (new confKey + CONF_SECTIONS/conferences.json/conf-prestige.json entries). Fixed Akron's confRecord (it wrongly credited MAC titles for 2023-25 Big East seasons + a fabricated 2021 MAC title/College Cup). New `confMoveNote` field renders a note in the Standings & Titles tab explaining each move. Schools stay in their original conf FILE (d1-other.json / aac.json) but group by men's-soccer confKey. conf-prestige.json: MAC removed, Patriot added (rank 23), Big East +Akron, AAC −Army/Navy. PRIOR: v44.9 — confRecord backlog campaign COMPLETE (Change Type 6, display-only). The validate_consistency.js confRecord counter is now 0 (was 38). All 110 schools' 2020–2025 conference history is researched & sourced; verification also fixed many wrong (not just thin) entries — hidden titles, fabricated results, and men's-soccer-conference-vs-guide-grouping mismatches (Army/Navy→Patriot, Akron→Big East, both deferred as structural questions). Batch 1 (v44.2): all 11 Big East schools rewritten from bigeast.com (counter 38→33). Batch 2 (v44.3): all 9 flagged AAC-file schools rewritten (counter 33→24) from THREE sources — theamerican.org (AAC), conferenceusa.com (C-USA 2020-21 for Charlotte/FAU/UAB), patriotleague.org (Army & Navy, all years). Fixed real errors: Tulsa's hidden 2021 AAC title, Charlotte 2nd-2023/1st-2025, Charlotte/FAU/UAB were AAC-soccer from 2022 not 2023, and Army/Navy play PATRIOT LEAGUE men's soccer not the AAC (Army won the 2022 Patriot League). Batch 3 (v44.4): all 5 flagged Big West schools (+ UCSB/Cal Poly enriched) rewritten (counter 24→19) from bigwest.org (fetched via curl — the site bot-blocks the in-app browser) + Wikipedia tournament table. Fixed CS Fullerton's hidden 2023 Big West title, UC Riverside's 2022 double (regular-season + tournament champions), UC Irvine's 2023/2025 tournament titles, and the fabricated 2020 rows (the entire Big West 2020-21 season was COVID-cancelled). Batch 4 (v44.5): all 5 flagged Big Ten schools rewritten (counter 19→14) from bigten.org (new Next.js site — parse embedded __NEXT_DATA__ JSON, only has 2022-25) + Wikipedia (Big Ten 2020/2021 + Washington's Pac-12 years). Fixed Penn State's hidden 2021 Big Ten title (reg season + tournament), Washington's 2022 Pac-12 title + 18-2-2 2021 (Washington was Pac-12 2020-23, Big Ten 2024-25), Northwestern 3rd-2023, Rutgers 3rd-2022. Batch 5 (v44.6): all 4 flagged ACC schools rewritten (counter 14→10) from theacc.com (Atlantic/Coastal divisions 2020-23, single table 2024-25) + Wikipedia Pac-12 templates for Cal/Stanford's Pac-12 years (Cal/Stanford = Pac-12 2020-23 → ACC 2024-25). Fixed Stanford's hidden 2020 Pac-12 title, Louisville's real ACC Atlantic finishes (2nd-2021, 3rd-2020/22 — not "Lower ACC"), NC State's 2025 NCAA national-final run; removed misplaced Stanford title claims. Batch 6 (v44.7): 4 flagged schools across 4 conferences — UCA (ASUN), GCU (WAC), Delaware (CAA→Summit), Drexel (CAA, lone caa.json flag, folded in) rewritten (counter 10→6); MCP-browser-first (asunsports/caasports/thesummitleague loaded; only wacsports.com bot-blocked → Wikipedia WAC templates). Fixed GCU's back-to-back WAC titles (2020 undefeated + 2021), UCA's consistent 2nd-place ASUN finishes (not "Mid"), Drexel 2nd-CAA-2024, Delaware's real CAA→Summit history (winless 2021, 12-2-3 first Summit season). Batch 7 (v44.8): 3 flagged d2 schools — Nova SE (SSC), Keiser (Sun/NAIA), Georgian Court (CACC) rewritten (counter 6→3). Batch 8/8 (v44.9, FINAL): the last 3 JUCO schools — Indian Hills (ICCAC 2021-23 filled, 2024/2025 title rows preserved), LSU Eunice (an NJCAA DI INDEPENDENT not a new program — 2021-23 = independent overall records), Miami Dade (men's soccer BEGAN 2024-25 per FCSAA standings — trimmed the fabricated 2020-23 rows to the 2 real seasons). **Counter 3→0 — campaign COMPLETE across 8 batches (v44.2-v44.9).** Prior batch fixes: UConn's fabricated 2023 reg-season title, Providence's mislabeled 2024 East title, Seton Hall's mis-dated tournament title. Deferred STRUCTURAL finds (men's-soccer conf ≠ guide grouping — Delaware/v44.0 class): Akron→Big East (filed MAC), Army & Navy→Patriot League (filed AAC). Prior: v44.1 coach-card staff render fix; v44.0 Delaware reclassified CAA → Summit League (CUSA sponsors no men's soccer; new single-school Summit League conference, moved caa.json → d1-other.json, display/grouping only, Delaware Fit holds 38, validator Issues 0). Prior series: v43.x — coach `overallScore` rubric (§5d), Step 2 re-score campaign 100% COMPLETE: all 110 coaches re-scored + re-ranked, deferred backlog cleared.** Always confirm with `git log --oneline -1` and `guideVersion` in `athletes/olivier.json`. (v42.19–v42.28 = the §5a devScore re-baseline batches + the v42.22 CAA coach fix; v42.29 = the 8-coach Change-Type-2 reconciliation; v42.30–v42.31 = Vermont HC timeline correction; v42.32 = Navy/Hackworth coach re-score; v42.33–v42.34 = dead-host fixes + Stony Brook coach → validator Issues 0. **v43.0 = §5d Coach Rubric written (doc-only, Step 0) + `ptPathScore` "PT Path" card badge deprecated. v43.1 = §5d Step 1: `overallScoreNote` field + gated `COACH-RUBRIC` validator check + global rankClass↔band coherence check (Issues held at 0, `0/110 re-scored`). v43.2 = §5d Step 2 Batch 1/10: ACC 12/13 coaches re-scored (Rhythm B — scores committed per batch, single global re-rank deferred to Batch 10; stored `rank` left stale meanwhile). Gunn 81→95 & McIntyre 79→83 (under-scored national champions corrected up); Riley/Hudson/Kerr halos stripped down; Vidovich held 76 + flagged upward (bio would not render); NC State's Findley departed→Hubbard, deferred as a Change Type 2 fix (12/110 re-scored). v43.3 = §5d Step 2 Batch 2/10: Big Ten 11 coaches re-scored (23/110 total). Clark (Washington) 68→92 — reigning 2025 NCAA national champion badly under-scored (the Gunn pattern); Rensing (Michigan State) 61→72 (2018 College Cup semi + 16 MLS draftees, recent-slump inverse halo); Jorden (UCLA) 93→84 (crest halo stripped — no NCAA title); Cirovski/Yeagley/Dow anchors held. Deferred: Clark's stale bio (Change Type 2 — add the 2025 title). v43.4 = §5d Step 2 Batch 3/10: Big East 11 coaches re-scored (34/110 total). Stewart (Providence) 63→74 — THE find: a College Cup semifinalist (2014) with 4 deep NCAA runs sat in the solid band on an empty data-gap bio (Gunn/Clark pattern); Masur (St. John's) 80→85 (1996 champion, 468 wins, floored by recent form); Higgins (Xavier) 63→67 & Lindberg (Seton Hall) 64→65 (D2-transplant coaches with a Big East tournament title each, solid→strong); Gbandi 73→74, Fetrow 48→52; Wiese 95 & DePaul/Plotkin 60 anchors held. No coach-change deferrals. v43.5 = §5d Step 2 Batch 4/10: AAC 10 coaches re-scored (44/110 total). Russell (FIU) 91→73 (prestige/results halo stripped — Sweet-16 ceiling at a mid-major, "14 MLS draftees" was program-cumulative not his); Kiefer (USF) 88→83 (deep 24-yr multi-program résumé but Elite-Eight ceiling, below national-title anchors); Worthen (FAU) 72→62 (corrected a stored error — he's a 9-yr HC not "debut season", thin pro output); Langan (Charlotte) 65→74 & McIntosh (Tulsa) 62→68 (under-scored mid-major standouts — 11 & 12 NCAA apps, 16 pros / 2 Elite Eights); Mulrooney (Memphis) 58→64 (elite USMNT/MLS playing CV + rebuild); Green (Temple) 58→56; Hackworth/Navy 74 & Plotkin/Army 70 anchors held. No coach-change deferrals. Deferred data fixes: Worthen yearsHC 1→9, UAB dead host blazerssports→uabsports, Russell draftee-count note. v43.6 = §5d Step 2 Batch 5/10: Big West 7 coaches re-scored (51/110 total). Kuntz (CSUF) 50→68 & Cupello (UCR) 48→65 — biggest corrections, both under-scored off data-gap/recent-form (Kuntz built UCI: 7 Big West tourney titles/7 NCAA/4× CoY/USSF A; Cupello developed Aaron Long = USMNT captain + 2022 World Cup); Shaffer (UC Davis) 62→68 & Raz (UCI) 64→66 under-scored veterans up; Kennedy (Cal Poly) 73→67 (no D1 NCAA berth — trimmed); Vom Steeg (UCSB) 90 held (2006 national champ, 32 MLS draftees) & Pascale (UCSD) 58 held (strong D2, poor D1). No coach-change deferrals. Deferred: Kuntz licence None→USSF A, Cupello yearsHC 8→14, TWO more dead hosts (ucirvineantares→ucirvinesports, ucrhighlanders→gohighlanders — now 5 total, full url sweep overdue). v43.7 = §5d Step 2 Batch 6/10: CAA 9 coaches re-scored (60/110 total). McCourt (Monmouth) 52→72 — THE find: a 23-yr/.592, 19-conference-title, 9× CoY veteran with ~22 pros (10+ MLS) sat at rank 107/near-last on a data-gap bio (Kuntz/Gunn pattern); Reeves (Elon) 65→71 (16-yr multi-conference — 6 titles, 4× CoY, USSF A, St. John's Final-Four assistant pedigree); Norris (W&M) 63→68 (2 CAA titles + Sweet 16 + 4 NCAA, dated-peak halo); Castellanos (Drexel) 60→66 (data-gap — 2000 UConn College Cup champion + USSF A + D3 titles); Anatol (Stony Brook) 58→63 (2023 CAA CoY + USF Elite-Eight pedigree; v42.34 had held 58); Wiggans (Charleston) 76→61 (city/trajectory halo stripped — losing HC record, no titles/NCAA as HC); Roche (Hofstra) 72→65 (first-year HC — program's 4 titles were his assistant work; USSF D); McMenemy (Delaware) 65→66 & Bonomo (Northeastern) 66 held. No coach-change deferrals. Deferred: four CAA coaches store licence null but hold USSF 'A' (Anatol/Castellanos/Reeves/McCourt), Monmouth mlsPlayers 0→10+, Delaware bio "Conference USA"→Summit League, Northeastern host gonu.com→nuhuskies.com (6th dead-alias); Stony Brook minutesOutlook now scrapable. v43.8 = §5d Step 2 Batch 7/10: d1-other 6 coaches re-scored (66/110 total). Franks (Denver) 84→86 (under-anchored — two College Cups 2016/2024, 2016 Nat'l CoY, six first-round MLS picks incl. 2019 MLS ROY); Davies (GCU) 79→73 (program/facilities-momentum halo trimmed — only ~2 yrs as a D1 HC atop a strong UCF assoc pedigree); Segebart (UCA) 72→59 (rk-strong→rk-solid — first-year HC named Dec 2024, the ASUN runner-ups were his assistant work); Embick (Akron) 88, Dubois (Vermont) 66 & Solomon (Mercyhurst) 62 held. Vermont watch resolved — Dubois is the confirmed current HC (2025 bucket = Sidearm season-trap; Dow coached 2025), scored as a first-year HC not swapped; no coach-change deferrals. Deferred: Segebart yearsHC 4→1 + licence null→USSF A-Senior. v43.9 = §5d Step 2 Batch 8/10: Ivy 2 coaches re-scored (68/110 total). Barlow (Princeton) 87→75 (rk-elite→rk-strong — Ivy-prestige/longevity halo stripped: 29-yr/7 Ivy titles/Bob Bradley coaching-tree successor/ex-US U-15 NT HC/USSF A, but ~5 MLS in 29 yrs, no internationals, first-round NCAA ceiling → below the 80 elite floor); Stannard (Yale) 78→70 (rk-strong holds — strong MSU associate-HC pedigree [2 Elite Eights, developed Chapman/Alashe] + 11-yr HC turnaround/2 Ivy titles/USSF A, but marquee development was as an assistant and his own HC pipeline is thin [Downs→USL]; corrected to sit below his former boss Rensing 72). No coach-change deferrals; no new deferred data fixes. Ivy context: Princeton/Yale offer no athletic scholarships but §5d has no division/program ceiling — scored on CV/development, not the recruiting constraint. v43.10 = §5d Step 2 Batch 9/10: D2/NAIA/D3 11 of 12 coaches re-scored (79/110 total). The predicted D2/NAIA halo correction landed — the five baseline 82–86 rk-elite coaches all came down: Martorana (PBA) 86→60, Young (St. Edward's) 85→73, Rootes (Lynn) 84→79, Ivanovic (Barry) 83→63, Billy Martin (OCU) 82→60; Rootes (3 D2 national titles as HC, 2× D2 National CoY, 512 wins) held at the TOP of strong not elite because the elite band pairs winning with a clear pro pipeline he lacks. Under-scored corrected up: Erush (Cal State LA) 70→74 (MLS playing career + Galaxy/US-Soccer pedigree + .735 HC record), Smee (UC Charleston) 68→72 (2024 D2 national runner-up as HC + SPL pro career; the program's 2 titles were his assistant work — score the coach not the program), Mason (Columbia College) 55→62 (131-win winningest William Woods HC, not a first-year coach), Oldham (Keiser) 68→71 (2021 NAIA national champion as HC). McArthur (Nova) 74→54 (~2 HC seasons); Raso (Georgian Court) 64 held (sub-.500 caps the USSF-A/D1-assistant pedigree). Chapman DEFERRED as Change Type 2 — current HC is Eddie Carrillo '90, not the stored Dustin Johnson (batch ships 11/12). Deferred data fixes: four dead host aliases (lagoldeneagles.com / kuseahawks.com / gculions.com / columbiacougars.com), four yearsHC errors (mcarthur 6→2, oldham 3→10, smee 2→6, mason 1→~15). v43.11 = §5d Step 2 Batch 10/10 (FINAL): JUCO 28 of 29 coaches re-scored + the single global re-rank of all 110 (107/110 re-scored; campaign COMPLETE — the §5d yardstick is now applied to every coach and the live ranking is truthful). Dominant data-gap pattern: stored JUCO bios omitted national titles/pipelines — Fisher (Nassau) 58→76 (2015 champion + 10 Final Fours + 15 MLS/pro), Cosgrove (Pima) 64→76 (2 DI titles + NJCAA HoF + 451 wins), MacRae (Iowa Lakes) 62→73 (2023 DII champion + MLS/USL alumni), Brown (Iowa Western) 65→75 (2 DI titles + D1 pedigree), Potter (Northeast) 58→68 (2024 DII champion), McBride (Blinn) 66→71 (18 pros incl. a full Canadian international), DiBernardo (Monroe) 72→77 (3-DI-title dynasty), Cameron (Phoenix) 68→72, Dale (AWC) 64→68. Clements (Tyler JC) 79→80 (rk-strong→rk-elite) — the single JUCO coach in the elite band (rank 22, at the floor below all D1): 7 titles/554 wins/NJCAA HoF + a real 100+-pro/10-MLS pipeline satisfies the Rootes test. Avallone (Daytona State) 74→66 (halo trimmed — ~3 college HC seasons). Santa Monica DEFERRED as Change Type 2 — current HC is Tim Pierce, not the stored Lee Avery (batch ships 28/29). 3 legacy remain, all deferred coach-swaps (NC State/Findley, Chapman/Johnson, Santa Monica/Avery). Deferred data fixes: dead hosts (daytona_state→dscfalcons.com, blinn coach.url→buccaneersports.com bio), yearsHC (dibernardo 5→16, brown 5→7, dale/fisher null), mlsPlayers/licence backfills (fisher 0→15 + USSF National, mcbride 0→18). v43.12 = deferred-backlog closeout (3 commits): (1) Tyler JC staffing — added Jake Carney (S&C) + guarded null coach-contacts to render "—" not "null" (15 cards/12 modals); (2) data-quality backfills — 10 dead/wrong athletics hosts fixed across coaches.json/conf-JSON/app.js (incl. Chapman chapmanathletics→athletics.chapman.edu), 11 yearsHC + 11 licence + 3 mlsPlayers backfills, Washington-bio 2025-title + Delaware-Summit-League corrections; (3) the 3 deferred coach swaps (Change Type 2) — NC State Findley→Marc Hubbard 83/rk-elite (2 seasons: first-ever national final + Hermann winner + No.1 MLS pick), Chapman Johnson→Eddie Carrillo '90 61 (31-yr winningest D3 HC), Santa Monica Avery→Tim Pierce 63 (HC since 2012, UCLA All-American/national-champ player) — then a final global re-rank. **§5d coach rubric now 110/110 re-scored, 0 legacy — campaign 100% complete; every coach on the same yardstick.**)** All v39 work is committed and pushed (`c456259` = v39.1–v39.6 squashed, `09c2ab7` = v39.7, `69cfc55` = failures summary); v40.1/v40.2 followed. See `v39_session_failures_summary.md` for the v39 incident log.
...
- `recruit_pathway` / `recruit_pathway_note` schema added v34; **full pass COMPLETE as of v44.24** — 103/110 schools populated (all with `minutesOutlook.available:true`; the remaining 7 are `available:false` and structurally excluded). Carries no scoring weight and was never folded into fitOlivier (see CHANGELOG.md v34 + v44.16–v44.24 notes) — doubly moot since GPA-adjacent factors are out of the Fit Score entirely.
...
The full 174-issue baseline from the v35.1 code review (previously listed here) was cleared across v36.1–v36.8 — see CHANGELOG.md's v36 entry for the batch-by-batch detail. `node validate_consistency.js` now reports **Issues: 0** — the Stony Brook coach-name gap that held the count at 1 was resolved in v42.34 (Ryan Anatol, see CHANGELOG.md).

Lower-priority (code quality, still deferred — none were in v36's named scope): `atarToGpa` defined in both scores.js and app.js (app.js wins by script load order — do not reorder the script tags); `DATA_BASE_URL` means `./data/` in app.js but site root in dashboard.js; olivier.json fetched twice per page load; `selectSchoolFromBar()` button-highlight matcher can never match (arrow-fn toString); dashboard `filterToConf('other')` scrolls to the Ivy section (5 Explore sections share `data-confkey="other"`, plus 5 duplicate `id="grid-other"` elements); search keyword echoed unescaped into the filter-summary HTML (self-XSS); ~~stale Explore section intro texts in CONF_SECTIONS~~ (**✅ FIXED v44.43** — owner spotted the ACC panel. All 7 stale intros rewritten: the full-vs-listed framing is gone everywhere (everything has been full-profile since v25), **`big-ten` named "USC", which fields no team in this guide** — the same phantom-anchor error §5a flags for "UF" in the Glossary — and **`asun` claimed UCA had "6 of 9 MFs clearing before Olivier arrives", which the v44.42 refresh had just made false (it is now 0 of 9)**. Lesson: a conference intro can hard-code a *roster fact*, so §3a Type 3 should sweep CONF_SECTIONS intros whenever a refresh materially moves a school's opportunity); Glossary Minutes Score text says Yr1 45/Yr2 30/Yr3 15/Yr4 10 but code is Yr1 60/Yr2 40; FX slider sublabels say 1.30–1.80 but the range is 1.20–1.70. (The old `costScore()` falsy-zero-for-service-academies issue is now moot — cost was removed from fitOlivier entirely in v37.1.)

### Deferred items (carried forward)

**[Backlog audit — 2026-07-20]** Every item in this section was re-verified against live data/coaches.json/validate_consistency.js this session (not assumed from the list). Several entries had already been fixed in later sessions without this list being updated; those are now marked ✅ RESOLVED and trimmed. Two entries turned out to need re-scoping rather than either close or stay-open — see the ✏️ RE-SCOPED items below. Everything else was confirmed still genuinely open. See memory `deferred_backlog_refresh_queued.md` for the audit method.

- **✅ RESOLVED v44.34 — the 4 broken `coaches.json` URLs are fixed; that field now sweeps 108/108 live (0 × 404, 0 × NXDOMAIN).** All researched Tier-1 in a real browser: `stedwards` → `gohilltoppers.com/sports/mens-soccer`, `setonhall` → `shupirates.com/…`, `virginia` → `virginiasports.com/sports/msoc`, `ncstate` → `gopack.com/sports/mens-soccer`. Three durable lessons:
  - **Per-coach bio deep links rot.** NC State's stored bio id was `5258`; the live page now serves the same coach at `5017`. All four are stored as **program pages**, matching the dominant convention (81 of 108). **Never store `/roster/coaches/<slug>/<id>`.**
  - **A resolving host is not a live host.** Hunting St. Edward's, `goseu.com` returned **HTTP 200** while serving a Chinese video-streaming site. Status codes alone would have stored it; the content had to be read (Monroe parked-lander lesson, v42.13).
  - **The school objects were right all along.** Each corrected URL already existed, correct, in the school object, and the two dead hosts appeared **nowhere else in the repo**. That is v44.33's diagnosis confirmed from the other side: `coaches.json.url` was the one stored-link field no sweep ever covered, so it alone drifted. **§15's sweep method should now include `coaches.json.url`** — 3 entries (`tyler_jc`, `indian_hills`, `murray_state_ok`) still have no `url` at all, a gap rather than breakage.
  - Note `coaches.json.url` has **no UI consumer** — no renderer reads it (verified by enumerating every field read off a coach object). It is stored reference data, so its correctness cannot be caught by looking at the app.

- **✅ RESOLVED v44.35 — the 2 coach-contact discrepancies applied (Change Type 2, contact only, no re-rank).** `stedwards` email `byoung@`→**`briany@stedwards.edu`** and phone `512-448-8415`→**`512-448-8507`**; `setonhall` phone `""`→**`973-275-6429`**. Evidence was `mailto:`/`tel:` hrefs on the 2026 staff pages, not display text. **A third location also carried the old address** — the St. Edward's `bio` string ends with a hardcoded `"Email: byoung@stedwards.edu"`; when changing a coach contact, **grep the bio strings too**, not just `contact{}`. `overallScore` unmoved, so the "re-rank ALL" trigger did not fire.

- **✅ RESOLVED v44.53 — the `Target:` GPA stat showed a SCHOOL NAME on two cards.** `js/app.js`'s card GPA strip did `u.gpa.minSchol.split(' ')[0]`, and `minSchol` is free prose: usually it opens with the threshold (*"3.5+ for academic merit"*), but at need-blind schools it is a whole sentence — so **Notre Dame rendered `Target: Notre` and Georgetown `Target: Georgetown`** (plus `Target: N/A` on smc/miami_dade). Found by the v44.53 audit for other instances of the v44.50 Max Aid defect class, not by a bug report. Fixed with a guarded extraction: take a GPA only when the string actually opens with one, else show `—`, because at those schools there IS no GPA scholarship threshold to display. Exactly **4 of 111** cards change; verified live that all 111 now render a GPA number or `—`.

- **🚩 OPEN (v44.53) — the card's "Soccer Level" stat is parsed out of prose and is inconsistent for 7 schools.** `js/app.js:728` does `u.soccerLevel.split('—')[0]`, so the stat shows whatever precedes the em dash. Because the field's format is inconsistent, **6 schools show a bare division token** where the others show an actual level — `mercyhurst`/`georgian_court` → `"D1"`/`"D2"`, `columbia_college` → `"NAIA"`, and `northeast_cc`/`monroe_college`/`indian_hills` → `"JUCO"`, against `tyler_jc`'s correct `"NJCAA Division I"`. Separately **`denver` renders a 59-character sentence** into a compact stat: *"Summit League (2025), moving to West Coast Conference (2026)"*. **This is the same defect class as Max Aid (v44.50) and the `Target:` bug above** — a display value parsed out of free prose. It is also the mechanism behind the long-deferred cosmetic `soccerLevel` format item (UX backlog D1, owner-deferred — *do not fix that formatting without being asked*), but the **Denver overflow is new and is a rendering problem rather than a formatting preference**. Fix the same way as `maxAid`: a short stored field read directly, not a split. Low priority, cosmetic, no scoring impact.

- **🚩 OPEN (v44.53) — the "Pre-PT Path" stat overflows for 2 schools.** Same `split('—')[0]` pattern at `js/app.js:723` and `js/dashboard.js:647`: `keiser` renders *"Strong (clinical simulation labs)"* (33 chars) and `chapman` *"Excellent (KIN 405 Pre-PT Prep required)"* (40 chars) into a compact stat slot. Cosmetic, no scoring impact, same fix shape.

- **Reference (verified v44.53, do NOT re-audit) — the GPA *filter* extraction is sound.** `js/dashboard.js:59/378/428` derive a number for GPA filtering via `parseFloat(u.gpa.minEntry.match(/[\d.]+/))`. That was checked against **all 111 schools** and every result is either a plausible GPA (1.0–4.0) or 0 for an open-admission school, which is the intended reading. This one is prose-parsed *and* correct — recorded so a future audit of this defect class doesn't re-derive it.

- **🚩 OPEN (v44.35) — `setonhall` / Lindberg's stored email `alindberg@shu.edu` is suspicious but UNPROVABLE; deliberately not changed.** The live coaches page publishes his phone but **no email**, and the stored address appears nowhere on the site. It is the only `shu.edu` address in the file and does **not** match the `firstname.lastname@shu.edu` pattern every colleague uses (`jeffrey.matteo@`, `nicolai.andersen@`). Under that pattern it would be `andreas.lindberg@shu.edu` — but deriving it would be exactly the guess §7 forbids. Resolve by asking the program directly, or leave; do not pattern-match it into the file.

- **🚩 OPEN (v44.35) — 13 coach cards render a literal `"null"` for Yrs HC (pre-existing, cosmetic).** `yearsHC: null` on 13 `coaches.json` entries prints as the string `null` in the Coaches → Profiles stat block. Confirmed pre-existing (13 nulls in `HEAD` before the v44.35 edit, exactly matching the 13 rendered). **Same defect class as the v43.12 null-contact guard, which guarded email/phone but not this stat** — the fix is the same shape: guard the render with `?? '—'`. Either fix the renderer or backfill the 13 `yearsHC` values (several are already listed as deferred data fixes elsewhere in this section).

- **Reference — the Outreach tracker renders ONLY the 10 shortlisted schools** (confirmed live v44.35: FIU, PBA, Lynn, UCSB, USF, Barry, Clemson, UNC, FAU, SMU). A coach-contact error on a non-shortlisted school is therefore visible in Coaches → Profiles and the school modal, but never in the Outreach list — don't use that tab to spot-check contact data.

- **NEW (v44.46) — `keiser` and `barry` deferred to Wave 2: instances 4 and 5 of "published but not populated".** Neither is a scraping failure; do not retry before late August. **`keiser`**'s 2026 roster renders its full coaching staff and **zero players** — browser-confirmed, with its own 2025 page returning 34 players/17 MFs through the identical read (raw HTML agrees: 62 `sidearm-roster-player` hits and **0** position classes vs 1615/136 on the 2025 page). **`barry`** publishes **21 players against 34 in 2025** with **one goalkeeper**, 5 defenders and 12 of 21 listed midfield-capable — browser-confirmed at 21. Both retain their 2025-26 `minutesOutlook` and `roster_season`. **Barry is the case that produced the new goalkeeper-count diagnostic** (see §15): its 62% squad ratio was ambiguous against Pittsburgh's deferred 50%, but a squad with a single keeper is not a squad. Had it been refreshed as-is it would have stored 12 MFs with **zero** clearing before 2027 — a plausible number and a fabricated opportunity score.

- **NEW (v44.46) — `ivy.json` is out of the roster campaign by owner ruling; both schools stay `available:false`.** *"I dont care about Yale and Princeton. They are unattainable as they dont offer scholarships."* This is a **research-effort** decision, **not a Change Type 10** — `ivy.json` is unchanged, both schools remain in the guide, and their `fundingPathway:"none"` −8 penalty already expresses the constraint. Do not delete them without an explicit instruction. Both 2026-27 rosters were fetched and parse cleanly (princeton 15 MFs of 31, yale 10 of 27) if this is ever reversed. **Accepted consequence:** `available:false` scores the neutral 0.5 minutes value, so Princeton's stored fit 41 and Yale's 47 are mildly *flattering* — the same neutral that cost Stony Brook 43→34 on population (v44.41).

- **NEW (v44.46) — `ocu` and `stedwards` carry a disclosed BLANK-POSITION caveat; re-check when the schools complete their data.** `ocu` publishes 3 of 25 players and `stedwards` 6 of 39 with an **empty position cell**. This is genuine, not a parser artifact: the rendered card view shows no position, and all six St. Edward's **player bio pages carry no position field either**. `mf_total` counts only confirmed midfielders and both `recruit_pathway_note`s say so. Materiality was measured, not assumed: **OCU is immaterial** (worst case moves opportunity 6.0 → 7.0, same trajectory row), **St. Edward's is not** (8.5 → 5.5 would cross a row). Re-derive St. Edward's when positions are published. **This is a field-level gap and must not be confused with the roster-level gap above** — the squad shape is healthy in both cases (OCU 3 GK, St. Edward's 5 GK).

- **✅ RESOLVED v44.47 — the Glossary's "Barry D2 consistently ranks #1 under this lens" claim is gone.** It was false and pre-existing: Barry's `lensScores.minutes` is **59**, while the lens is led by `iowa_western` **79**, then lsu_eunice 77, dodge_city_cc/daytona_state 75, and five more JUCOs at 73 — Barry was never #1 on the stored numbers. **Rewritten to describe the mechanism rather than name a winner.** That choice is the durable part: naming a #1 school is the same defect class the `PROSE` check exists for, and **Session 4 refreshes all 30 JUCOs — precisely the schools at the top of this lens** — so any named school would be stale within one session. The rewrite also drops the *"7 of 13 midfielders"* figure, which was accurate against Barry's stored 2025-26 data and passing `PROSE`, but would have gone stale the moment Barry is refreshed in Wave 2. **Rule of thumb this establishes: lens/ranking copy should explain what the lens rewards, never which school currently wins it.**

- **✅ RESOLVED v44.47 + v44.47b — Keiser's "Fort Lauderdale" references fixed; there were EIGHT, not two.** v40.6 corrected Keiser to **West Palm Beach** across 12 occurrences plus mapX/mapY but missed: 2 × `js/app.js` NAIA intros (Explore + Coaches), `data/aac.json` FAU `culture.olivierMatch` (*"adjacent to Fort Lauderdale (Keiser) and Miami"* — wrong city **and wrong direction**; WPB is ~30 min *north* of Boca Raton, Fort Lauderdale is south), `data/conferences.json` `sun.guideSchools[0]` (**the Conferences-tab school chip**) and `sun.olivierNote`, `data/coaches.json` Oldham's `bio` and `strengths[2]`, and `data/pipeline.json`'s Keiser `ncaaD2` row.

  **Two lessons. The first: re-grep when closing a "N known instances" copy item** — the §6 note said "two `js/app.js` strings", was written from a js/html glob, and `data/` had never been swept. **The second, recorded deliberately: the first fix pass claimed "swept every remaining Fort Lauderdale: all are legitimate" and that claim was FALSE** — it rested on a grep truncated to its first 12 results, so most of `data/` was never examined. The other five surfaced only because the live DOM was still running a **cached `js/app.js`** (the v37.3 gotcha) whose old strings exposed them. **A truncated search read as exhaustive is indistinguishable from a clean result — never conclude "none remain" from a head-limited grep.** The corrected method: a script that walks every string in `data/`+`athletes/` and attributes each hit to its owning record, so all 39 get judged individually.

  The **5 remaining Keiser mentions are correct**: two cite Fort Lauderdale as a 45-min destination, one is the housing note recording the v40.6 correction, two are the rewritten FAU/Sun strings. All other repo occurrences belong to **Nova Southeastern** (genuinely in Fort Lauderdale) or to FAU/Lynn/PBA/FIU distances. Still true that **no validator can see this class of error** — `PROSE` covers counts, roster claims, scrape-season phrasing and phantom schools, not a wrong *city*.

- **✅ RESOLVED v44.48 — the "NAIA has no scholarship cap" contradiction; there were EIGHT strings, not one.** The UI told the user NAIA is uncapped while §5c classifies it `fundingPathway: "capped"` and docks **−3** for that cap — the guide contradicting its own scoring model, in the athlete's favour, on a recruiting-decision fact.

  **The distinction being lost:** NAIA men's soccer has a **12-equivalency TEAM cap** (more generous than D1's 9.9 and D2's 9.0 — but a cap). What it lacks is a **PER-PLAYER** cap, so a coach may concentrate the pool into a full ride for one athlete. All eight strings collapsed "no per-player cap" into "no cap". **The guide already said it correctly in two places** — `index.html` Glossary *"NAIA has 12 with no per-player cap"* and the funding-penalty block listing NAIA as capped — so the fix was making the other eight match, not inventing a framing.

  Fixed: `index.html` Financial Model explainer · `js/app.js` Coaches NAIA tier intro · `conferences.json` SAC `desc` + AMC `scholarships` · `conf-prestige.json` AMC `scholarships` · `coaches.json` Mason bio · `d2.json` OCU `fin.internationalNote` + `facilityDetails.note`.

  **Deliberately left alone as accurate:** the *"Up to equivalent of full ride (NAIA)"* strings, Oldham's *"NAIA full scholarship potential"*, and Keiser's *"NAIA full ride possible"* — a full ride genuinely is attainable for an individual, and the stored data agrees (`ocu`/`keiser` carry `maxAthletic: 1.0` **with** `fundingPathway: "capped"`, which is exactly the team-cap/per-player-cap split). **Text only — no `fundingPathway`, `maxAthletic`, `costNum` or score moved, and none should: the −3 was always right; the copy was wrong.**

  **Method note, reinforcing v44.47b: search SEVERAL PHRASINGS, not one.** A `no cap|no maximum` pattern found 5; querying the *equivalency* angle surfaced a 6th and 7th; and the SAC `desc`'s *"not limited by equivalency caps"* only fell out of a third pattern. **One regex read as exhaustive would have left three live.** See [[feedback-exhaustive-search]].

- **✅ RESOLVED v44.49 — the D1 "9.9 equivalencies" figure is now opt-in-aware; there were TWENTY-NINE strings, not the 2 logged.** 9.9 was **not** dropped (still correct at non-opt-in schools, per v44.48's instruction); it is now qualified everywhere it appears. **Sized to its surface, which was the delegated judgment call:** the **Glossary** carries the canonical explanation in a new *"The House Settlement (from 1 July 2025)"* entry — both models, the explicit admission that **this guide deliberately does not track opt-in status**, why that is defensible (§5c: it cannot move a Fit Score), and the point that matters to a recruit, *permission to fund 28 is not the same as funding 28*; the **Financial Model card** gets one clause plus a pointer (the owner's instinct that a card can't carry a settlement caveat was right); the **25 one-line table cells** get a compact `"— 28 funded roster spots if House opt-in"` suffix. Counts: `conferences.json` 14 (**invisible** — only the parsed number reaches the UI), `conf-prestige.json` 11 (prestige table), `index.html` 2, `js/app.js` NAIA intro 1, `d2.json` OCU note 1.

  **The two genuinely misleading strings were the COMPARATIVE ones**, not the flat statements: the NAIA tier intro and OCU's `internationalNote` both argued *"12 equivalencies — more than D1 (9.9) or D2 (9.0)"*, which post-*House* is unreliable against an opt-in D1 school. **The D1 half was dropped, the D2 half kept** — 9.0 is untouched by the settlement. Same defect class as v44.48: copy resting on a cap that had moved.

  **No score moved and none should** — §5c's D1 ⇒ `full` ⇒ 0 penalty is unaffected by opt-in status, exactly as it says. No new research: §5c has carried the ncaa.org-sourced fact since v42.18, so this was propagation, not a lookup.

- **✅ RESOLVED v44.50 — the Max Aid tile now reads a stored `maxAid` field; the parse is gone.** `maxAid` added to **all 25** conferences as a short authored token (`"9.9"` D1 · `"9.0"` D2 · `"12"` NAIA · `"None"` Ivy/D3 · `"Varies"` JUCO — a single value genuinely cannot express NJCAA DI vs DII vs CCCAA), renderer is now `${c.maxAid||'—'}`, and a **`MAXAID`** check enforces presence + a ≤12-char length AND **greps `js/app.js` to fail if `scholarships.split()` ever returns** — the data checks alone would be blind to the renderer regressing. **All five branches negative-tested** (missing / empty-string / wrong-type / over-length / renderer-regressed). Two currently-*working* values were deliberately changed as part of this: D2 `"9"` → `"9.0"` (matches the 9.9 format, reads as an equivalency rather than a count) and Ivy/SCIAC `"ZERO"` → `"None"` (`"ZERO"` was never authored — it was the parser grabbing the first word of *"ZERO athletic scholarships…"*). **Test-quality note worth keeping: the first negative test PASSED and proved nothing** — it patched a 6-space indent when the file uses 4, so the mutation silently never applied and "Issues: 0" looked like a working check. Always assert the mutation landed (count the field before/after) before reading the validator's silence as a result.

- ~~🚩 OPEN (v44.49) — the Conferences-card "Max Aid" stat is derived by string-splitting prose, and renders non-numerically for 10 of 25 conferences.~~ (historical detail on the bug, retained for the reasoning) `js/app.js:2576` does `c.scholarships.split('Up to')[1]?.trim().split(' ')[0] || c.scholarships.split(' ')[0]`, so the stat tile shows **"NCAA"** (NEC, Summit, CACC — their strings begin `"NCAA D1 — up to …"`, and the split is **case-sensitive** so lowercase `up to` never matches), **"Army"** (Patriot), **"NAIA"** (AMC), **"equivalent"** (SAC, Sun — `"Up to equivalent of full ride"`), **"Athletic"** (JUCO), and "ZERO" (Ivy, SCIAC — arguably readable). Pre-existing: the exact expression was run against `HEAD`'s strings before any v44.49 edit and re-checked after, yielding **the same 10 values**. **Deliberately not fixed there — the defect is the renderer, not the copy.** Do NOT resolve it by reshaping the 10 prose strings to satisfy the parser: that is backwards, the next copy edit re-breaks it, and `conferences.json.scholarships` exists to be *read*, not parsed. Fix `renderConferences()` to pull the figure from a real field (or regex a number out of the string with an explicit fallback), then the strings are free again. **This is also why the `conferences.json` half of v44.49 is invisible in the UI** — no renderer consumes that field's full text, only the extracted token. No validator can see it (both validators read data, not rendered output), and it is cosmetic — no scoring impact.

- **NEW (v44.39) — `pennstate` deferred to Wave 2: 2026 page published but not populated (third instance).** Its 2026 roster renders **8 players and ZERO midfielders** alongside a complete coaching *and* support staff; the season dropdown confirms 2026 is the newest published season, and switching that dropdown to 2025 returns a full squad with real midfielders through the identical read. Not a scraping failure — do not retry before late August. It retains its 2025-26 `minutesOutlook` and `roster_season` label. Joins `tulsa` and `pittsburgh` below.

- **✅ RESOLVED v44.41 — `stonybrook` minutesOutlook populated**, closing the half of that §6 item still open after v42.34 fixed the coach name. Its live 2026-27 roster renders normally (9 MFs of 29; 3 clear before 2027), so `available` flipped `false → true`. **Its Fit fell 43 → 34** — expected and correct: the record had been scoring the neutral 0.5 (minutes 50) while unavailable, and the real outlook is 26. **`recruit_pathway` was deliberately left UNSET**: Stony Brook publishes no previous-school column and there was no prior classification to retain, so there is nothing to derive from and nothing to carry forward. Populate it only if the school starts publishing the column.

- **NEW (v44.42) — `recruit_risk` was RETAINED, not re-derived, for all 27 Session-2 schools that had one.** It is unscored (only `trajectory[].pct` feeds `scores.js`), and re-deriving 27 judgment values against a newly-invented rule was out of this session's scope. Flagged so a later pass knows the field is untouched — most worth revisiting where the midfield group size moved sharply this refresh: `hofstra` 6→12, `delaware` 4→9, `monmouth` 5→9, `william_mary` 7→11.

- **NEW (v44.38) — `tulsa` and `pittsburgh` deferred to Wave 2: 2026 page published but not populated.** Neither is a scraping failure and neither should be retried before late August. `tulsa`'s 2026 roster page renders its coaching and support staff and **zero players** (control test: `/roster/2025` returns 29 players / 8 MFs through the identical extractor). `pittsburgh`'s 2026 page lists **13 players against 26 on its 2025 page** — roughly half a squad, which would yield a fabricated opportunity score. Both retain their 2025-26 `minutesOutlook` and their `roster_season: "2025-26"` label, which is accurate about where the stored count came from. **This is now the third distinct "published ≠ populated" shape on record** (Indian Hills: correct headers, no rows; Clemson: full squad but stale season; these two: partial or player-free). Always compare the player count against the previous season's page before trusting a fresh roster.

- **NEW (v44.38) — 11 schools publish NO previous-school column, so their `recruit_pathway` cannot be re-derived from the roster.** `fau`, `syracuse`, `georgetown`, `stjohns`, `uconn`, `depaul`, `marquette`, `setonhall` omit the column entirely; `cal` and `villanova` render the field but leave it empty for every midfielder. Their existing classification was **retained, not re-derived**, and each `recruit_pathway_note` now says so explicitly and marks itself lower-confidence. Do not "fix" these by inferring a pathway from the hometown/high-school text — that column holds secondary schools and clubs, not colleges, and reading it as a transfer signal is how a wrong classification would enter. Resolve only if a school starts publishing the column, or from per-player bio pages if the field is ever worth the ~10 page loads per school.

- **NEW (v44.38) — `butler` coach Ian Sarachan has a Tier-1 email on the live staff page (`isarachan@butler.edu`) but `coaches.json` stores null for both email and phone.** Observed in passing during the Big East roster pass and **deliberately not applied** — it is a Change Type 2 edit and the session was scoped to Type 3, so mixing them would have put an unrelated change in a roster commit. The value is recorded here so a follow-up needs no re-research; verify it is still live before writing, then apply as contact-only (no re-rank — `overallScore` doesn't move). `memphis` and `temple`, which carry the same validator warning, genuinely publish **no** email on their staff pages, so those two warnings are honest gaps rather than fixable ones.

- **NEW (v44.38) — `stjohns` head coach is listed as "Dr. David Masur" on the live staff page; `coaches.json` stores "Dr Dave Masur".** Same person, cosmetic naming difference only. Left alone deliberately: any coach **name** change fires the §3a Type 2 "re-rank ALL coaches" trigger, which is a disproportionate amount of churn for a punctuation-and-nickname difference. Fold it into the next genuine coaches.json pass rather than doing it standalone.

- **NEW (v44.30) — Army & Navy `lensScores.value`: does the Value lens credit a $0 sticker price? OWNER DESIGN QUESTION, not drift.** Found by the new VALUE check's sweep. Both service academies store a value well below what the formula yields: **navy** stored 47 vs formula 66, **army** stored 45 vs formula 65. The cause is structural — §4 requires `costNum=0` and all `fin{}` numerics zeroed for service academies, which saturates `affordability` at 1.0 and hands them the full +40 affordability credit. The stored values (both ≈ `fit+3`) deliberately decline that credit, and the reasoning is sound: the "free" tuition is paid for with a 5-year military service commitment, a real cost no dollar figure expresses, and §4 is explicit that these schools are incompatible with Olivier's DPT/MLS pathway. Applying the formula verbatim would promote them to roughly 6th and 8th of 111 on the Value lens. **The `VALUE` check therefore exempts `costNum === 0` rather than reporting two intentional values as errors** — it declines to rule on the design question. Resolve one of three ways when the owner is available: (a) keep the exemption and accept that service-academy value is hand-set (status quo), (b) define an explicit service-academy affordability constant so the value is derived rather than hand-set, or (c) decide the +40 is correct and re-store both. Whichever is chosen, remove or narrow the exemption so the two schools stop being un-checked.
- **NEW (v44.30) — `README.md` version table has no rows for v41–v44.** The header version and school count were stale by four versions (said "Version 40.11" / "110 universities"); both corrected in v44.30, and a single combined **v41–v44** row was added pointing at CHANGELOG.md. The per-version era rows for v41, v42 and v43 are still missing — deliberately not back-filled, since writing era summaries for four versions of work a later session didn't do is how inaccurate history gets manufactured. Low priority, cosmetic; back-fill from CHANGELOG.md when convenient.
- **NEW (v44.27) — 10 schools with an unresolved coach contact conflict, need Tier-1 re-verification.** During the v44.27 architecture consolidation (coach{} removed from school objects, coaches.json made the sole source), a dry-run diff found 21 schools where the school object's `coach.email`/`.phone` disagreed with `coaches.json`'s `contact.email`/`.phone`. 11 were one-sided (one side blank) and resolved cleanly by keeping whichever side had data (2 were live-verified corrections from the owner: Drexel `dc3369@drexel.edu`, Elon `mreeves3@elon.edu` / `336-278-6746` / "Head Men's Soccer Coach"). The other 10 had two DIFFERENT non-blank values on each side with no way to tell which is current without research — per the "never guess coach contact info" rule, `coaches.json`'s existing value was kept as-is (not verified, just not overwritten) and the conflict logged here rather than resolved by guessing: **wakeforest** (email+phone), **ucla** (email), **indiana** (phone), **ucsb** (phone), **charleston** (email+phone), **mercyhurst** (phone), **princeton** (phone), **ocu** (email+phone), **georgian_court** (phone), **columbia_college** (email+phone). Verify each via the school's own official athletics staff page next time it's touched.
...
- **✅ RESOLVED v42.31 — Vermont head coach.** Confirmed via coaches.json: Adrian Dubois is a 2026 1st-season appointee; Rob Dow coached Vermont through 2025 (won the 2025 AEC title) before leaving for Penn State in Dec 2025. Bio (`dubois_vermont`) correctly states both facts.
- **✅ RESOLVED v42.32 — Navy / John Hackworth under-scored.** `overallScore` now 74 (was 42), verified live in coaches.json. Anchored below Vidovich, above Plotkin per the service-academy ceiling reasoning in CHANGELOG.md v42.32.
- **✅ FIXED v42.13 — three dead athletics hosts (owner supplied the correct URLs).** Blast radius was 4 spots across 3 files, not just `juco.json` (found via repo-wide grep of the dead hosts):
  ...
  **Lesson banked (see [[feedback-scraping-process]]):** DNS alone is insufficient — Monroe *resolves* to a parked page; and `athletics@mdc.edu` (a valid email) will false-match a naive grep for `athletics.mdc.edu`. A separate Monroe **Bronx** campus exists at `monroeexpress.com` — the guide's Monroe is **New Rochelle**. The other 26 JUCO hosts are live. **The full sweep this item asked for was DONE in v44.31 — see the next item. No validator check for dead `url`s exists, and probably shouldn't: ~15 hosts bot-block and would be permanent false positives.**

- **✅ DONE v44.31 — full URL sweep of all 333 stored links (111 school `url` + 111 `SITE_URLS` + 111 `DOMAINS`).** Three dead, all fixed: `notredame` `url` (`/sports/mens-soccer` 404 → `/sports/msoc`, in **both** acc.json and coaches.json), `tyler_jc` `url` (`www.apacheathletics.com` NXDOMAIN → bare `apacheathletics.com`), `setonhall` `DOMAINS` (`shupiratesl.com` NXDOMAIN, stray trailing `l` → `shupirates.com`). **Method note worth reusing:** run the sweep with a browser User-Agent, and treat **only NXDOMAIN as proof of death**. A naive check condemns ~15 live hosts — Cloudflare answers **202 with an empty body** (`jcccathletics.com`, `efsctitans.com`, `goreivers.com`) and **403** (`iwcc.edu`, `indianhills.edu`, `umd.edu`, `unc.edu`, `umich.edu`, `uakron.edu`, …), and Python `requests` throws `SSLError`/`ConnectTimeout` on `rutgers.edu`, `uncc.edu` and `chapman.edu`, all of which serve fine in a browser. **The `DOMAINS` failure mode is invisible by design:** Google's favicon service returns a *generic globe*, not an error, for an unresolvable domain — Seton Hall's logo had been silently broken with nothing in the UI or either validator able to flag it (old host 404/726 B placeholder vs new 200/1360 B real icon).
...
- **✅ RESOLVED — COACH overallScore RUBRIC, v43.** `node validate_consistency.js` confirms **110/110 coaches re-scored against §5d, 0 legacy pending**. The Coach Re-score Campaign (see memory `coach_rescore_campaign.md`) completed Step 2 and the global re-rank in v43.12 (main `e1757ec`, 2026-07-18). Design (single holistic score, no division ceiling, no `fitOlivier` cascade) as originally specified in §5d.

- **✅ PARTIALLY RESOLVED — COACH BIO AUDIT men's/women's conflation risk (v43, owner-directed v42.10).** The originating instance — Austin Solomon's bio wrongly crediting Rich Wall's (Mercyhurst women's) departure — is fixed; the current bio (`solomon_mercyhurst`) no longer references Rich Wall, and a repo-wide search for "Rich Wall" returns no hits (confirmed 2026-07-20). **No systematic search for other instances of this failure mode has been done** — still open as a general audit, just not urgent given the specific known case is closed.
...
- ~~Stony Brook coach name AND minutesOutlook~~ — **✅ BOTH RESOLVED.** Coach fixed v42.34 (Ryan Anatol); `minutesOutlook` populated v44.41 off the live 2026-27 roster (`available` now `true`) — see the Stony Brook entry above.
...
- **✏️ RE-SCOPED (audited 2026-07-20) — Big East coach `licence` fields.** Previously logged as "6 fields null"; live check shows **all 11 Big East coaches** currently have `licence: null` (and 73/110 guide-wide) — the field is effectively unpopulated across the board, not a small residual. Verify when contacting programs, as originally noted; just don't scope it as "6."
- **✅ RESOLVED (2026-07-21) — `recruit_pathway` / `recruit_pathway_note` full pass COMPLETE.** All 103 of 110 schools with `minutesOutlook.available: true` are now populated (verified via full-repo scan), researched live via Chrome MCP batch-by-batch across all 10 conference/division files (v44.16–v44.24). Remaining 7 schools (Princeton, Yale, Stony Brook, Suffolk CC, Westchester CC, +2 more) have `minutesOutlook.available: false` and are structurally blocked — `validate_consistency.js`'s `MO-KEYS` check only permits the field on an `available:true` object. Still informational-only throughout, zero scoring cascade (confirmed Issues:0 at every batch). See CHANGELOG.md v44.16–v44.24 for full per-school findings; notable cross-batch pattern: D2/NAIA transfer-rebuild programs (Hofstra, CSU Fullerton, PBA, Barry, Nova SE, Oklahoma City, Columbia College) skew heavily Portal/JUCO-heavy or Transfer-preferred, while most D1 mid-majors and all D3/Ivy-adjacent programs researched came back Freshman-friendly.
...
- **🚩 OWNER-REQUESTED SESSION 4 SUB-TASK (2026-08-06) — populate `minutesOutlook` for Suffolk CC and Westchester CC; the "genuinely absent" claim below is UNPROVEN and should be re-tested.** Neither school has ever had a Minutes Outlook analysis (`available:false` since the v39 batch). Owner supplied the roster links, both of which already match the stored `url`/`DOMAINS`:
  ...
  **The two records in this repo disagree about how hard this was tested, and that is the lead.** `suffolk_cc`'s stored `minutesOutlook.note` says the position column was *"not populated **at time of automated fetch**… revisit with **direct browser access**"* — i.e. a real browser was never tried — while the §6 line below claimed both were *"re-checked via Claude for Chrome MCP."* Those cannot both be true. **`?jsRendering=true` in the owner's own link is strong evidence the column is rendered client-side**, which is exactly the v39 failure mode (two JUCO rosters written off after WebFetch failed; Chrome MCP returned both on the first attempt). Re-read in a real browser before concluding anything.

  Order: (1) check for a **2026-27** roster before using Suffolk's 2025-26 page; (2) Chrome MCP, with `navigate` → wait → read as *separate* calls; (3) if the column is still blank, try player bio pages — but note the `stedwards` precedent, whose bio pages carried no position field either, so this can legitimately dead-end; (4) if recoverable, full Change Type 3 cascade plus `recruit_pathway` (unset on both).
