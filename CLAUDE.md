...
### School → File Reference Table (156 schools)
...
**Current version: v44.80 (2026-08-12).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
- **156 schools**, all full-profile, across 10 conference JSON files. **156 coaches** in `coaches.json`, ranked 1–156.
