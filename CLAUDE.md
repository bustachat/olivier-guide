...
**Current version: v45.22 (2026-09-03).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
- **🚩 OPEN CAMPAIGN, started v45.15 — degreeTitle/acuAlign fabrication audit. Handover for a fresh session below; do not re-derive the scope, it's already computed.**
  ...
  **Scope, computed by diffing every current school id against the data files as they stood at the RULE 0 commit (`97218e9`, 2026-07-10) — 110 of 170 schools existed before RULE 0 and have never had their `degreeTitle` claim checked against a live source:**
  ...
  | Status | Count | Ids |
  |---|---|---|
  | ✅ Fixed (fabricated/false claim) | 6 | `indian_hills`, `iowa_western`, `lynn`, `duke`, `georgetown` (all five fully fabricated degrees — the school has no such program at all), `chapman` (partial — real degree, renamed Kinesiology→Applied Human Physiology, but fabricated course codes attached to it) |
  | ✏️ Minor correction (real program, degree-type/course-code accuracy only) | 8 | `stedwards`, `butler`, `cal`, `charleston`, `charlotte`, `delaware`, `gcu` (see prior entries — all real programs, wrong name/prefix), `louisville` — real degree is "Exercise Physiology, B.S." (Exercise Science is only a minor); real courses use `HSS` prefix, not `EXSC`. None of these eight are fabrications — real, substantive programs with a wrong name and/or fabricated course-code prefix. |
  | ✅ Spot-checked/verified, confirmed real | 48 | `smc`, `tyler_jc`, `arizona_western`, `cowley_cc`, `angelina_college` (5 pre-campaign, checked during the Indian Hills investigation), `barton_cc`, `blinn_college`, `coastal_bend_cc`, `daytona_state`, `dodge_city_cc`, `efsc`, `iowa_lakes_cc`, `johnson_county_cc`, `lsu_eunice`, `miami_dade`, `mohave_cc`, `monroe_college`, `nassau_cc`, `neosho_county_cc`, `northeast_cc`, `phoenix_college`, `pima_cc`, `southeastern_cc_ia`, `suffolk_cc`, `ulster_cc`, `westchester_cc` (21, the rest of Tier 1), `barry`, `columbia_college` (its stored "⚠ No Exercise Science" disclosure is accurate — real offerings are Healthcare Management / Health Sciences), `csula`, `georgian_court`, `keiser`, `nova`, `ocu`, `pba`, `uc_charleston` (9, the rest of Tier 2), `akron`, `army`, `calpoly`, `clemson`, `creighton`, `csuf`, `denver`, `depaul`, `drexel`, `elon`, `fau`, `fiu`, `hofstra`, `indiana` (14, Tier 3 so far) |
  ...
  **Tier 3 IN PROGRESS (v45.22) — 24/69 checked: 14 confirmed real + 7 minor corrections + 2 real fabrications (`duke`, `georgetown` — both elite, extremely well-documented schools with NO such major at all).** Don't assume prestige or thoroughness of documentation protects against fabrication — the two worst finds this campaign came from two of the most selective schools in the guide.

  | 🚩 **Tier 3 remaining — lower risk, check last** (major D1 conferences — large, well-documented programs, but still technically unverified) | 45 | `marquette`, `maryland`, `memphis`, `mercyhurst`, `michigan`, `michiganstate`, `monmouth`, `navy`, `ncstate`, `northeastern`, `northwestern`, `notredame`, `ohiostate`, `pennstate`, `pittsburgh`, `princeton`, `providence`, `rutgers`, `setonhall`, `smu`, `stanford`, `stjohns`, `stonybrook`, `syracuse`, `temple`, `tulsa`, `uab`, `uca`, `ucdavis`, `ucirvine`, `ucla`, `uconn`, `ucriverside`, `ucsb`, `ucsd`, `unc`, `usf`, `vermont`, `villanova`, `virginia`, `wakeforest`, `washington`, `william_mary`, `wisconsin`, `xavier`, `yale` |
