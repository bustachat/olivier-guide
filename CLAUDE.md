...
**Current version: v44.94 (2026-08-17).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
| **5** | The NJCAA gap-fill campaign JUCOs (56 schools added v44.71–v44.85, region by region — the "39" figure in the `project-roster-refresh-campaign` memory is stale/wrong, 56 is the real count against `data/juco.json`). They carried accurate data from add time; the purpose here is re-verification, not a fix. Working sub-batches in add order, oldest first. | **🔄 IN PROGRESS.** Sub-batch A (Region 1, v44.93) — `yavapai_college`, `eastern_arizona`, `csn_college` re-verified with real churn; `community_christian_college` re-confirmed still unpublished. Sub-batch B (Region 2, v44.94) — `eastern_oklahoma_state`, `rose_state`, `national_park`, `rich_mountain` genuinely unchanged (facts re-verified, no data drift); `connors_state`'s head-coach vacancy filled (Steve Moore hired, scored 52 per §5d, coaches re-ranked); `neo_am`'s "Interim" coach tag dropped (Tyler Douthitt now permanent, no score change). ~46 schools remain across Regions 4–6, 9, 10, 14, 16–20, 24. |
