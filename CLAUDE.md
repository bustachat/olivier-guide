...
- **✅ RESOLVED v45.57 — added JUCO division sub-filtering to Explore's Division filter row.** Owner-reported: the Division chips (`D1/D2/D3/NAIA/JUCO/Ivy`) let you isolate NCAA D1 from D2/D3, but the single `JUCO` chip mixed NJCAA Division I, II and III together with no way to narrow — and the Conference row's `NJCAA` chip is a different filter dimension (ANDs with Division, not a substitute), which is why selecting `D1` + `NJCAA` silently returned "no schools match" rather than doing what the owner expected. Added three chips — `JUCO DI` / `JUCO DII` / `JUCO DIII` — using the existing `njcaaDivision` field via a new `njcaadiv` card dataset attribute. They stay in the same `div` filter type as `D1`/`JUCO` (not a new filter type) specifically so multiple Division-row chips keep OR-ing together the way they already did — `applyFilters()` special-cases `div` values starting `JUCO-` to match `dataset.njcaadiv` instead of `dataset.div`. Santa Monica (CCCAA, no `njcaaDivision`) still matches the plain `JUCO` chip but none of the three sub-chips — correct, since it isn't NJCAA-divisioned. Verified live: `JUCO-I` alone → 75 schools, `+JUCO-II` → 84, `D1+JUCO-I` → 142 (67+75, confirming the union still works).
