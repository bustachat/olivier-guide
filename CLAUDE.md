...
- **🚩 Found + fixed v45.15 (owner-reported): Indian Hills' `degreeTitle` was fully fabricated — "A.A. in Exercise Science & Kinesiology" does not exist at IHCC.** Written in v24.1 (2026-06-23), ~2.5 months before RULE 0 (§15, v42.9) made real-browser Tier-1 verification mandatory. No validator can catch a fabricated degree name — it's not an internal-consistency check, only a live source check can. Corrected to the real picture (a generic Associate of Science, plus a genuine but selective/non-transfer Physical Therapist Assistant A.A.S.) — see CHANGELOG v45.15. **A same-session spot-check of 7 other JUCOs spanning old (v27–v39, pre-RULE-0) and recent (v44.79–v44.84, post-RULE-0) additions found all 7 genuinely real** (`smc`, `tyler_jc`, `arizona_western`, `cowley_cc`, `angelina_college`, `trinidad_state`, `illinois_central`) — this looks like an isolated fabrication, not a systemic pattern. `glendale_cc_az` could not be checked (its site and `maricopa.edu` both returned a persistent Cloudflare challenge in-session — a network result, not a data finding; its claim is unconfirmed, not disproven). **A full audit of every pre-v42.9 school's degree claim against a live catalog has NOT been done** — the spot check is reassuring but not exhaustive; if a future session has the budget for it, checking the remaining pre-RULE-0 vintage (roughly everything added before v42) against each school's own program-finder/catalog page would close this out properly.
