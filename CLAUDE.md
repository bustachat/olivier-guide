...
**Current version: v45.40 (2026-09-15).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
| **7E — Region 4 (Chicago)** | `truman_college`, `daley_college`, `kennedy_king_college`, `wilbur_wright_college` | The "school's own site" here is `region4sports.com` (Lineup tab, §15 layout table). **Never open `citycollegesofchicagoathletics.com` — it is compromised** (§6D). |
