...
- **🚩 `malcolm_x_college` head coach position is VACANT as of v45.30 — re-verify next time this school is touched.** Jesse Rosen (head coach 2014-2025) left in May 2026 for Chicago State University; no successor was found despite an extended search. Stored as `name: "Vacant"`, `overallScore: 30` (neutral placeholder, floor of rk-solid). Do not guess a name — check `region4sports.com` or `ccc.edu` (not `citycollegesofchicagoathletics.com`, which is compromised — see §6D) for a new hire, and re-score properly per §5d once one is confirmed, then re-rank all coaches.
