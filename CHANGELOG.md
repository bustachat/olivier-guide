...
**Refreshed (3 of 7 checked):**
- **`kennedy_king_college`** — NJCAA's own roster page, previously thin (1 goalkeeper, no class years), now publishes a complete 2026-27 squad (27 players, real positions and class years). `mf_total` 6→12, `cleared_before_2027` 6→2, `recruit_risk` Low→High, trajectory Yr1 59%→47%/Yr2 72%→60%, `fitOlivier` 46→42, `lensScores.minutes` 64→52, `.value` 50→48. Coach (Jonathan Pachar) independently confirmed unchanged via NCSA, ProductiveRecruit, and active October 2025 box scores.
- **`harcum_college`** — the school's own roster page now publishes a complete 2026-2027 squad (17 players, full positions and class years), an improvement on the prior "1 GK, no full picture" state. `mf_total` 5→6, `cleared_before_2027` 5→1, `recruit_risk` Low→Medium, trajectory Yr1 58%→50%/Yr2 71%→63%, `fitOlivier` 55→53, `.minutes` 63→55, `.value` 35→34. Coach (David Hughes) independently confirmed unchanged via a September 17, 2026 Instagram post plus SportsRecruits.
- **`southwestern_illinois`** — the school's own roster page, previously stuck on a stale 2025 title, now reads "2026 Season" with full positions and class years (20 players), matching the school's own active fall 2026 game schedule. `mf_total` 10→7, `cleared_before_2027` 10→6, trajectory Yr1 65%→59%/Yr2 78%→72%, `fitOlivier` 42→40, `.minutes` 70→64, `.value` 60→58. Coach (Lindsay Eversmeyer) independently confirmed unchanged via a recent ("3 weeks ago") local Facebook mention plus SportsRecruits.

All 3 score cascades computed directly from `js/scores.js`'s real functions (loaded via Node's `vm` module, not a hand-written reimplementation) to rule out formula drift.

**Still blocked, same reasons re-verified rather than assumed (4 of 7 checked):**
- **`truman_college`** and **`trinidad_state`** — both schools' own sites (region4sports.com, tsctrojans.com) are now behind a genuine CAPTCHA-style "Let's confirm you are human" wall, not the milder Cloudflare 202/302 challenge this project has documented elsewhere. This was not bypassed (bot-detection bypass is against this project's own standing rules). NJCAA's roster pages were re-checked instead: Truman's now has real 2026-27 positions but still zero class years for any player; Trinidad State's still has only 3-4 of 28 players with both fields filled in. Both notes refreshed to record what was actually checked; no data changed.
- **`pacific_northwest_christian_college`** — the school's own site now shows a real 2026 season (15 players, up from the prior check), but still only 5 of 15 players carry any position label at all, and none of the 5 is a confirmed midfielder. Note refreshed; no data changed.
- **`wilbur_wright_college`** — NJCAA's roster page still says "No roster available," unchanged. But a real, separate finding surfaced while researching this school's coach (see below): the program currently has **no head coach**.

**Coach vacancy found — `wilbur_wright_college`, resolves a standing "genuinely ambiguous" open item from v45.30.** The owner passed a link to `citycollegeofchicagoathletics.com` (singular "college") claiming to show the current coach. That domain is one letter off from `citycollegesofchicagoathletics.com` (plural), which this project's own history already documents as compromised — it now redirects to gambling/spam content. An automated safety check independently blocked navigation to the owner's link before it could be read, so its content (including an email address that appeared only in a search-result snippet) was never used. Resolved instead via a Tier-1 source reachable without that domain: a St. Louis Intercollegiate Athletic Conference (SLIAC) press release dated 2026-06-29 confirms Gabe Billings — Wilbur Wright's previous soccer point of contact and Director of Athletics — has left the college entirely to become Head Women's Soccer Coach at Eureka College. The article describes his Wilbur Wright role purely as prior CV, removing the ambiguity the v45.30 finding had flagged. A part-time "Head Coach - Men's Soccer" posting (May 28, 2026) has since closed, but no successor announcement was found despite a real search across recruiting platforms and news results. `coaches.json` updated to `name: "Vacant"`, `overallScore: 30` (neutral placeholder, matching this project's established vacancy convention), with the departure and the failed search for a successor both documented in `record`/`bio`/`note`.

**Process note — a jargon leak was introduced and caught before commit.** The first draft of several `trajectoryNote` fields and the Malcolm X coach bio (the latter already live from v45.72) used the phrase "this session" to describe a blocked source, which `check_no_jargon.py` correctly flagged as internal process language rendering to real visitors. All 5 instances were reworded to describe what was found rather than when it was checked. This included one small follow-up fix to already-shipped v45.72 data — a reminder that this checker should run on every batch, not just ones that feel novel.

**Validation:** `validate_schools.py` PASS (174/174, warnings unchanged in substance — one new expected "contacts unverified" warning for the newly-named Wilbur Wright vacancy entry), `validate_consistency.js` **Issues: 0**, `check_roster_arithmetic.py` PASS, `check_juco_trajectory.py` PASS (all 3 refreshed schools' stored trajectories match the JUCO formula exactly), `check_no_jargon.py` PASS after the fix above. Confirmed live in a local preview: all 3 refreshed schools' `fitOlivier`/`lensScores`/`minutesOutlook` match exactly what was stored, confirming no formula drift.

**Files:** `data/juco.json` (7 schools' `minutesOutlook`, 3 schools' score cascade), `data/coaches.json` (`wilbur_wright_college` vacancy, `malcolm_x_college` jargon fix), `CLAUDE.md` (§6B/§6C/§6D items updated), `athletes/olivier.json` (guideVersion).
...
**Spot-checked 7 schools (RULE 0, live catalog per school):** `indian_hills` (already independently verified earlier the same session, not re-checked), `rose_state`, `northeast_texas_cc` and `lynn` came back clean — every cited course code and title matched the real, live catalog exactly. **3 of the remaining 6 (50%) had real, live-verifiable errors:**

- **`pba`** (Palm Beach Atlantic, `data/d2.json`) — had the highest `acuAlign` in the entire guide (14) but the stored note gave almost no course-level evidence. Live-checked PBA's real "Applied Physiology and Kinesiology: Concentration in Pre-Physical Therapy, B.S." catalog page: 10 of the 14 credited units have solid required-course support (confirmed against real course codes: BIO 2281/2283, HHP 3263, HHP 4203, HHP 3013, HHP 3233, HHP 4003, HHP 3423, HHP 3343, HHP 4433/4813). The other 4 (`EXSC222`, `EXSC216`, Psychology of Sport, `EXSC249`) had no required-course support — 2 (`EXSC216`, Psychology of Sport) rely on courses that exist only as **electives** (students pick 2 of 9), undisclosed in the note; 2 (`EXSC222`, `EXSC249`) had no course match at all, even elective. `acuAlign` 14→10, `lensScores.academic` 89→68.
- **`harcum_college`** (`data/juco.json`) — the note credited one BIO 103/104 Anatomy & Physiology sequence to 3 separate buckets (Anatomical Foundations, Human Biology, Physiological Bases), against this guide's own established no-double-counting convention. Live-checked Harcum's real PTA course sequence page: found PTA 109 "Functional Anatomy" (exact name match, already correctly credited) and PTA 114 "Applied Kinesiology" — a required course literally described as "the study of human motion" — which was NOT credited to Mechanical Bases of Exercise at all. Dropped the double-counted Human Biology and Physiological Bases credits, added Mechanical Bases via PTA 114. `acuAlign` 7→6, `lensScores.academic` 52→47.
- **`monroe_college`** (`data/juco.json`) — `Research & Ethics` was stored `covered: true` with zero supporting evidence anywhere, not even in the note's own text — a plain data error, not a judgment call. Live-checked Monroe University's real "Exercise Science, AS" catalog page (Monroe rebranded to Monroe University; catalog.monroeu.edu): found `LA-227 Sports Psychology`, a required course that directly matches the Psychology-of-Sport unit, and `EX-290 Exercise Science Practicum`, a required practicum matching Professional Experience — neither was credited. Corrected: `Research & Ethics` → false, Psychology of Sport → true, `EXSC388/EXSC389` → true (net +1). `acuAlign` 5→6, `lensScores.academic` 42→47. (Two other courses the note already cited, `EX-250 Exercise Programming` for `EXSC126` and the shared A&P sequence for Human Biology, are the same kind of generous/debatable interpretation questions found at `harcum_college` — noted but deliberately left untouched here since they weren't part of what was reported and approved before this fix; worth a look if `monroe_college` is revisited.)

`fitOlivier`/`lensScores.overall` unaffected for all three (acuAlign has been informational-only since v37.1). Validated clean: `validate_schools.py` PASS, `validate_consistency.js` Issues: 0, `check_no_jargon.py` PASS (20,646 rendered strings checked). CRLF intact. Only `acuAlignNote`, `acuUnits[].covered`, `acuAlign`, and `lensScores.academic` changed per school.

**The result confirms the concern that prompted this check: a 50% error rate among schools that had already passed internal-consistency, none of which were caught by any validator.** The other 33 of the original 40 remain genuinely unverified — see CLAUDE.md §6F for the open item.

**Files:** `data/d2.json` (`pba`), `data/juco.json` (`harcum_college`, `monroe_college`), `athletes/olivier.json` (`guideVersion` v45.68 → v45.69).
...
Owner's first ruling: fix only the citations visibly wrong to a reader (the 43 notes + the reference facts), leave the 16-unit table alone. After being shown the live ACU Alignment tab — where the table's own left-hand column still read "ANAT100 — Anatomical Foundations", "BIOL125 — Human Biology 1", etc. for every visitor, guide-wide, not just the 43 schools — owner asked for the table fixed too. See CLAUDE.md §6F "ACU rubric review, 2026-09-23" for the full finding.

**New real-code mapping (9 of 16 slots), verified against ACU's official 2027 Handbook:**
`ANAT100`→`EXSC142`, `EXSC225`→`EXSC126`, `EXSC224`→`EXSC120`, `EXSC204`→`EXSC233`, `EXSC296`→`EXSC249` (ACU folded the old Sport Psychology / Health & Exercise Psychology split into one real unit — the code goes on this slot, the closer label match), `EXSC398`→`EXSC388/EXSC389` (ACU's practicum is now two stages, both real codes kept visible). `BIOL125`, `EXSC199`, `EXSC122` have no standalone unit in ACU's real current curriculum — left blank (`unit: ""`) rather than inventing a replacement. 7 codes were already real and untouched: `EXSC222`, `EXSC322`, `EXSC394`, `EXSC321`, `EXSC216`, `EXSC187`, `EXSC230`.

**What changed:**
- **`ACU_UNIT_META`** (`js/app.js`) — the 16-row master definition table's `unit` field updated per the mapping above; `label`/`usEquiv`/`coverage` untouched (those describe content, not the ACU code, so nothing about what each bucket *means* changed).
- **`renderACUTable()`** (`js/app.js`) — two fixes: (1) the row template no longer shows a stray " — " when `meta.unit` is blank (`Human Biology 1` instead of ` — Human Biology 1`); (2) **a real bug caught before shipping** — the "programs covering this unit" matching was `u.acuUnits.find(x => x.unit === meta.unit)`, which breaks once 3 rows share the same blank `unit:""` value: `.find()` always returns the *first* blank slot, so all three rows (Human Biology 1, Psychology of Sport, Research & Ethics) would have silently shown the same school list. Fixed to match by array position instead (guaranteed by the validator to be in identical order to `ACU_UNIT_META`), not by value.
- **All 174 schools' `acuUnits[].unit`** (`data/*.json`, all 10 conference files) — the same 9-code rename applied mechanically at every one of the 16 fixed positions. Pure schema-key rename: `covered` values, `acuAlign`, `lensScores`, and `fitOlivier` were verified byte-identical to `HEAD` for all 174 schools.
- **`validate_schools.py`** (`REQUIRED_ACU_UNITS`) and **`validate_consistency.js`** (`CANON`) — both hardcoded the old 16-code list and would have flagged all 174 schools as wrong the moment the data changed; updated to the new list in the same commit.
- **CLAUDE.md** — the 16-code list and the two "false pattern by acuAlign" tables (§3a Type 8, §5 schema) translated to the new codes, preserving the same exclusion-priority order, just with corrected codes; §7 Phase 1C's research checklist now points at §5 instead of duplicating the list a fourth time.
- **A second and third live occurrence of the "four units most likely to transfer" claim**, missed by the first sweep (different phrasing — "and EXSC322" vs "EXSC322" defeated the original exact-string pattern): `index.html`'s "WES Credit Recognition tip" box directly under the ACU Alignment table, and `js/app.js`'s per-school "WES Credit Recognition" block in the Details modal's Degree tab (rendered for every one of 174 schools, not just once). Both corrected the same way as the Glossary entry and CLAUDE.md §13 fixed earlier in this version.
- **`athletes/olivier.json`** — `wesTransferableUnits[]` carried the identical wrong 4-code list as structured data (unused by any renderer, confirmed by grep, but still wrong); corrected to the 3 real codes. `auUnitsPlanned[]` also cites unrelated-looking codes (`EXSC301`...`EXSC390`, a suspiciously round made-up sequence) representing Olivier's own personal course plan — **left untouched**, since correcting it would mean inventing his actual planned unit sequence (which major, which order) rather than fixing a verifiable fact; flagged as an open item.

**Scope discipline:** nothing about scoring changed anywhere in this version. `acuAlign`, `acuUnits[].covered`, `lensScores`, and `fitOlivier` confirmed byte-identical to `HEAD` for all 174 schools — the acuAlign score was always computed by matching a US school's real content against each bucket's *description*, never by checking for a literal ACU transcript code, so relabeling the code changes nothing about any prior judgment.

**Verified:** `json.tool` passes on all 10 conference files + `athletes/olivier.json`; CRLF line endings intact on all 10 data files (olivier.json is natively LF-only, unaffected); `node --check js/app.js`; `validate_schools.py` PASS (22 warnings, unchanged); `validate_consistency.js` Issues: 0; `check_no_jargon.py` PASS; diff confirmed to touch only `acuAlignNote`/`acuUnits[].unit` in data files, no `covered`/score fields; browser-verified live — the ACU Alignment tab's left column now shows the real codes (or a blank-code label with no stray dash), the Glossary and both WES tip blocks render the corrected 3-unit claim, no console errors beyond the pre-existing favicon-fetch noise.

**Files:** all 10 `data/*.json` conference files, `js/app.js`, `validate_schools.py`, `validate_consistency.js`, `index.html`, `CLAUDE.md`, `athletes/olivier.json` (`guideVersion` v45.66 → v45.67).
...
**Schools added (170 → 174), all researched in Chrome on 2026-09-17** (the athletics sites for NWACC, Oxford and Southwestern Christian showed a human-verification check, which the owner cleared):
- **NorthWest Arkansas Community College** (`nwacc`, Bentonville AR, NJCAA DI Region 2). First season 2026, 6-1 so far. HC Dean Johnson (USSF B; former Loyola Marymount associate head coach; Davis & Elkins 41-6-4; Northeast Texas CC 46-16-3) with a six-person staff including a performance coach. Home games at David Gates Stadium, Rogers Heritage High School; no campus housing. CAPTE-accredited PTA A.A.S. (ACU 9). Roster: 10 midfielders, all freshmen. Cost $23,692 (international $240/credit + fees, 24 credits; NWACC 2025-26 food and housing estimate). Fit 49.
- **Oxford College of Emory University** (`oxford_college_emory`, Oxford GA, Region 17). NJCAA lists its men's soccer in Division I for 2026-27 (Division II in 2025-26), but Emory's admissions FAQ says Oxford competes in NJCAA Division III and cannot offer athletic scholarships. **Owner ruling: treat and label it Division III** (no athletic aid, −8 funding penalty, athletic slider locked). HC Greg Moss-Brown (USSF A; two-time NCAA D1 All-American at Hartwick). 2-2-2 so far; 3-5 in 2025-26. Associate of Arts only, no exercise-science courses (ACU 1). 14 midfielders: 4 clear, 10 return. Cost $93,854 (Oxford catalog 2026-27). GPA: Emory Common Data Set 2025-26 average entering GPA 3.80, so "below". Fit 47.
- **Southern Crescent Technical College** (`southern_crescent_tech`, Griffin GA, Region 17). Club season fall 2025, NJCAA play from August 2026. HC Nathan Suddeth (first year, both teams). Home games at Locust Grove High School; no housing. AS General Studies with A&P electives (ACU 2). No roster with positions anywhere (the NJCAA roster lists 11 names with no positions or class years), so Minutes Outlook is unavailable. Cost $42,892 (foreign-student rate 4× in-state, three-semester year, college living estimate). Fit 45.
- **Southwestern Christian College** (`southwestern_christian`, Terrell TX, Region 5). HC Josue Romero (second season). Residence halls. General associate degrees only (ACU 0). 2026-27 roster lists 15 players with 10 defenders and no forwards, so positions aren't reliable and Minutes Outlook is unavailable. Cost $21,438 (college cost table). Fit 51.

Coaches added and all 174 re-ranked (Johnson 80, Moss-Brown 122, Romero 169, Suddeth 171). `js/app.js` DOMAINS / SITE_URLS / SOCIAL and JUCO intro, `conferences.json` (labels, 93 schools, desc, olivierNote), `conf-prestige.json`, CLAUDE.md reference table updated. Full rosters for NWACC and Oxford archived to `data/rosters/`.

**Bugs found and fixed during the add:**
- **21 JUCOs never appeared in the Pathways tab's "Recruiting Pathway by School" section.** They stored `recruit_pathway`/`recruit_pathway_note` on the school object, but the app reads them from `minutesOutlook`. Moved (paris_jc, jacksonville_college, texas_southmost, crowder_college, jefferson_college_mo, harcum_college, hagerstown_cc, harford_cc, slcc, snow_college, north_idaho_college, college_of_southern_idaho, truckee_meadows_cc, pacific_northwest_christian_college, illinois_central, southwestern_illinois, lincoln_trail, central_georgia_tech, ranger_college, noc_enid); empty placeholders removed from colorado_northwestern_cc and usu_eastern. snow_college and college_of_southern_idaho used "Mixed entry", which isn't a valid value, now "Mixed". No other field changed. Pathways tab now covers 152 schools (was 132).
- **`apply_roster_refresh.py` (used by `refresh_school.py`) still used a single JUCO division strength**, missing v45.53's Division II/III split, so a refresh of a Division II or III JUCO stored a Fit one point too high. It set Oxford to 48; corrected to 47. The Python copy now matches `js/scores.js` for all 174 schools.

**New guardrails:** `validate_consistency.js` `PATHWAY-LOC` fails if any school stores the pathway fields on the school object; the `FIT` check is now exact (the 1-point tolerance is what let the stale copy through). Two negtest cases added.

**Verified in a fresh local session:** all 11 tabs load with no console errors or "undefined"/"NaN"; all 9 Details tabs for the four new schools; Financial tab totals ($23,692 / $93,854 / $42,892 / $21,438) with Oxford's athletic slider locked; Compare shows all four; coaches ranked and badged; map dots on land; filters D1+JUCO 76, D3+JUCO 5, JUCO 93; Pathways 152; Minutes Outlook and Conferences list all four. `validate_consistency.js` Issues: 0; `validate_schools.py` PASS; wording, JUCO trajectory, roster arithmetic, snapshot and coach ranking checks PASS.
...
**Follow-up, same day (docs only, no version bump): NJCAA stats roster adopted as a secondary step for JUCO refreshes.** Owner-directed. Tyler's data was cross-checked against `njcaa.org/teams/407485412185999056/msoc/2026-27/div1/roster`. **All 34 players match exactly** on name, position and class, including the 14 midfielders and the 7 sophomores who clear. GP 5 confirms the 2026-27 roster is in live use. **Caveat:** Tyler's athletics site now runs on the same platform with the same team ID, so for Tyler this is a consistency check, not an independent source. The process is written into CLAUDE.md §15 ("NJCAA stats roster") and roster-refresh `SKILL.md` step 1, covering both validation and fallback. **Blinn College** was tested to prove the URL pattern works for other schools: the guide holds 2025-26 (12 MFs), while NJCAA shows a populated 2026-27 roster (36 players, 11 M, GP 7). Blinn's own site was not checked, so this proves only the URL pattern. Blinn's next refresh starts on Blinn's own roster page. Owner ruling, same day: a refresh never starts from NJCAA.

**Correction, same day: both sites are geo-blocked, and the "no VPN" access claims were wrong.** The owner later got the CloudFront 403 on `njcaa.org` from home broadband *and* from 5G mobile, while their VPN worked. Their VPN had been on during every successful load this session, and Claude-in-Chrome inherits the owner's network. So the note saying a cache-busting string got through "with no VPN" was false: the VPN did it. `njcaa.org` and `apacheathletics.com` (same platform) refuse Australian traffic. CLAUDE.md §6C/§15 and the roster-refresh skill now say to turn the VPN on first.
...
**1. Climate-Neutral lens (6th "View by" pill).** Answers "how would this school rank if warm weather didn't matter?" — `calculateFitScore()` with the climate term dropped and the surviving weights renormalised to 100 (soccer 47.1 / minutes 41.2 / city 11.8), housing and funding penalties still applied.

Climate is a bigger lever than its 15% label suggests: `climateScore()` returns 1.0 warm / 0.2 cold, so the term is a **12-point swing** — larger than the city term's maximum (7), the housing penalty (6), or the funding penalty (8). **101 of 170 schools are cold**, and **19 of the current top 20 are warm**. Under the lens the top-20 warm share falls to 7, only 6% of schools stay within 5 places of their default rank, and cold schools holding a top-3 division badge go 3 → 6. It bites hardest in D1 (Georgetown enters the top 3) and JUCO (Iowa Western 68 and Monroe 64 displace Rose State and Eastern Florida State); D2/NAIA/D3/Ivy barely move, because those divisions are already climate-homogeneous.

**Computed live in `climateNeutralFit()` (js/scores.js), never stored — this was the main design decision.** A 7th `lensScores` key would have meant editing 170 school objects, the §5 schema, and both validators (`validate_schools.py:108` *errors* on a key mismatch, `validate_consistency.js:110` flags one), and it would have created a stored derived value that nothing recomputes at runtime — the `lensScores.value` drift class from v44.30. Every input already exists on the school object, so there was nothing to store. Same precedent as `dynamicGpaStatus()` (v36.5) and `renderRecruitPathwaySummary()`. The weights are renormalised from `athleteConfig.scoreWeights` rather than hardcoded, so the lens follows automatically if `scoreWeights` is ever retuned.

`lensValue(u, lensKey)` (js/app.js) is the new single accessor — it returns the computed value for `climateNeutral` and the stored key otherwise. It replaced four separate inline `lensScores[key]` reads (`lensRank`, `lensRankByDivision`, `lensRankByConference`, `applySort`'s `lensAwareFit`), so the stored/computed split lives in exactly one place. Re-sorting comes free: **Best Fit sort was already lens-aware.** The lens changes ordering and badges only — **it does not change any school's displayed Fit Score.**

**2. Fit Score band filter (3 chips: 58+ / 48–57 / under 48).** Cuts set from the real distribution across all 170 schools (min 29, median 48, p75 55, max 71), giving **35 / 55 / 80**. The originally-proposed 71–100 / 51–70 / 0–50 split was measured first and rejected: its top band held **3 schools** (Cal, UCLA, Tyler JC) and its bottom band held 58% of the guide. New `fitband` branch in `applyFilters()`, reading the live score off `unis` rather than `card.dataset.fitscore` (the dataset copy is only written by `recalculateAllScores()`). Multi-select ORs and composes with every existing filter.

**3. Fit/Dev colour split — a live bug this surfaced.** `sc()` coloured scores green ≥ 90 / amber ≥ 80 / red below. Since v37.1 removed GPA, cost and ACU alignment from the formula, the maximum achievable Fit Score has been **71** — so **every one of the 170 schools rendered red**, on the card, in the Compare row and in the modal. The thresholds were calibrated for the pre-v37.1 blended score and were never revisited.

**`sc()` was deliberately NOT recalibrated, because it is shared with the Dev Score**, and dev scores genuinely run to 93 (median 57). Retuning it to 58/48 would have turned 83 dev scores green. Instead a separate `fitColor()` (js/scores.js) now serves the 5 Fit-Score call sites (card, Compare "Overall Fit", modal, and two writebacks in `recalculateAllScores()`); `sc()` keeps its 90/80 scale for the 6 Dev-Score sites. `fitColor()` and the filter chips both read `fitBand()`, so the colour and the filter can never disagree. **Do not "unify" these two functions — they describe different scales on purpose.**

Two stale prose claims fixed in the same pass, both promising unreachable numbers: the card's Fit Score tooltip ("90%+ = excellent") and the Glossary's Fit Score entry ("90%+ = excellent opportunity. 80–89% = strong. Below 70% = meaningful trade-offs"). A Glossary entry for the new lens was added.

**Files:** `js/scores.js`, `js/app.js`, `index.html`, `athletes/olivier.json` (version only).

**Verified locally** (`npx serve`, port 8787, all 170 schools loaded): all 6 lenses sort with **0 out-of-order pairs** across 135 adjacent-card comparisons (the 5 pre-existing lenses regression-tested alongside the new one); band chips return exactly 35 / 55 / 80 with every visible card in-band; multi-select ORs to 90; composes with Division (D1 ∩ lower = 33); Clear resets to 170 with 0 active chips; card/modal Fit colours correct at each band (SMU 67 green, NC State 49 amber, UVA 42 red); `lensRankByDivision('climateNeutral')` returns the predicted D1 (Cal/UCLA/Georgetown) and JUCO (Iowa Western/Tyler JC/Monroe) top 3. Zero JS console errors — the only console output is the pre-existing favicon/logo network failures documented in §6D. **Lens badges could not be confirmed in the DOM**: `applyLens()` paints them inside `requestAnimationFrame`, which does not fire while the browser pane is hidden — confirmed environmental, since all 5 pre-existing lenses also rendered 0 badges under the same conditions, and the underlying ranking data was verified correct directly.
...
**Root cause: same pattern as Indian Hills — a plausible-sounding degree name with no real program behind it.** IWCC's own site search returns zero hits for "kinesiology" and zero relevant hits for "health sciences" as a program; its full Programs of Study catalog (`catalog.iwcc.edu`) lists no Kinesiology or Health Sciences track at all. IWCC's real, only relevant offering is the **Exercise Science Transfer Major, A.S.** — a 61-credit, 4-semester sequence (First Aid, Health, Weight Training, intro Sports Medicine, Care & Prevention of Athletic Injuries, Personal Trainer) confirmed live at `iwcc.edu/academic_programs/health/exercise-science-transfer-major-a-s/`. The fabrication wasn't confined to `degreeTitle` — the school's `courses[]` array separately invented three non-existent courses ("KIN 110 — Introduction to Kinesiology," "BIO 169 — Human Physiology," "HHP 220 — Exercise Science Foundations"; IWCC has no KIN or HHP course prefix at all), and `facilities[]`/`facilityDetails.sportsScience`/`.academicLabs` all referenced a "Kinesiology programme"/"Kinesiology Lab" that doesn't exist.

**Fix — Change Type 9 (§3a).** `degreeTitle` corrected to "A.S. Exercise Science Transfer Major." `kinRank`, `acuAlignNote`, `courses[]`, `facilities[]`, and the two `facilityDetails` mentions all rewritten to describe the real program, sourced from IWCC's own published 4-semester course sequence. `acuAlign` corrected 8 → 4 — verified unit-by-unit against `ACU_UNIT_META`'s real definitions (not the plausible-sounding code names): only `ANAT100` (Human Anatomy & Physiology I/II, BIO 168/173), `BIOL125` (Human Biology 1, BIO 157 — near-exact title match), `EXSC216` (Resistance Training ← Principles of Weight Training, PEH 170), and `EXSC199` (Psychology of Sport ← Sport and Exercise Psychology, PSY 210) are genuinely covered; the program has no dedicated Exercise Physiology, Biomechanics, or Research Methods course, so `EXSC225`, `EXSC224`, `EXSC122`, and `EXSC398` all flipped from the previously-stored `true` to `false`. Cascaded per the documented formula: `lensScores.academic` 40 → 36. **`fitOlivier` and `lensScores.overall`/`.value` are untouched** — confirmed `js/scores.js` contains zero references to `acuAlign`/`acuUnits` (ACU alignment left the Fit Score formula in v37.1), so this correction cannot move Iowa Western's rank.

**Process note — nearly re-introduced the exact bug this file's own §8 DON'T rule was written to prevent.** The first draft of the new `acuAlignNote` opened with "Academic-credential audit correction (v45.16): the previously stored ... does not exist..." — internal audit-trail language, in a field confirmed live in the browser to render verbatim in the Details modal's "ACU Degree Alignment" box. Caught before commit by re-reading the rendered modal screenshot, not by any validator (no check reads `acuAlignNote` for this). Rewritten to plain, parent-readable language before verifying and committing. Worth remembering for the remaining ~99 schools in this audit: every fix will touch a rendered note field, and the audit's own paper trail is exactly the kind of language that rule exists to keep out.

**Verified:** `python -m json.tool data/juco.json` passes. `validate_schools.py`: 0 errors, 19 warnings (unchanged, all pre-existing). `validate_consistency.js`: **Issues: 0**. Confirmed live in a local browser — Iowa Western's Details modal "Degree & ACU Align" tab renders the corrected title and "4/16" ACU units with the plain-language note; `fitOlivier` confirmed unchanged at 61; zero new console errors.

**Files:** `data/juco.json` (`iowa_western` — `degreeTitle`, `kinRank`, `acuAlign`, `acuAlignNote`, `acuUnits[EXSC225/EXSC224/EXSC122/EXSC398]`, `courses[]`, `facilities[]`, `facilityDetails.sportsScience`, `facilityDetails.academicLabs`, `lensScores.academic`), `CLAUDE.md` (§1, §6 current-version lines; §6F status table), `athletes/olivier.json` (`guideVersion` v45.15 → v45.16).

---

### v45.15 (2026-09-03) — Fix: Indian Hills' "A.A. in Exercise Science & Kinesiology" was fabricated — no such degree exists

Owner asked directly why they couldn't find any sports-related study program on Indian Hills' own website, having noticed the guide's claimed degree didn't match what they saw searching the school's site themselves.

**Root cause: a fabricated credential from an old, pre-research-discipline session.** `degreeTitle`/`acuAlignNote`/`acuAlign`/`acuUnits` for `indian_hills` were written in commit `e7b471e` (v24.1, 2026-06-23) — roughly 2.5 months and ~20 major versions before RULE 0 (§15, added v42.9) made real-browser Tier-1 verification mandatory for every research claim. No source URL or research trail survives for that commit. Checked live against Indian Hills' official Program Finder (96 programs) and course catalog: **zero matches for "exercise," "kinesiology," "sports," "athletic training," "physical education," or "fitness" anywhere in the program list.** No validator can catch this class of error — `validate_schools.py`/`validate_consistency.js` check internal consistency (does `acuAlign` match the count of `covered:true` units), not whether the underlying claim is real; only a live source check (which is what the owner did) can.

**What's actually real, confirmed against Indian Hills' official catalog:** a genuine **Physical Therapist Assistant, A.A.S.** (CAPTE-accredited, 85 credits/7 terms) exists and does include real courses the old note referenced out of context — `BIO 175/176` Human Anatomy, `BIO 178/179` Human Physiology, `PTA 121` "PTA Kinesiology," `PTA 203` "PTA Therapeutic Exercise" — but it's a selective, career-technical program (admissions testing, 20 clinical observation hours, background checks) built to produce a licensed PTA directly, not a stepping-stone transfer degree, and not realistic alongside two years of full-time JUCO soccer. The school's actual general transfer degree is a generic **Associate of Science (A.S.)** with no sport-science specialization — though its science-elective bucket does explicitly list Human Anatomy (with lab) and General Biology as options any A.S. student can choose.

**Fix — Change Type 9 (§3a):** `degreeTitle` corrected to "Associate of Science, A.S. (general transfer degree — no dedicated Exercise Science, Kinesiology, or Physical Education program)," matching the established convention already used for other no-dedicated-program JUCOs (`lincoln_trail`, `ranger_college`, `truman_college`, etc.). `acuAlignNote` and `kinRank` rewritten to state the real picture plainly. `acuAlign` corrected 4 → 2 (only `ANAT100` and `BIOL125` are genuinely covered by the confirmed Human Anatomy + Biology electives; `EXSC225`/`EXSC224` — Exercise Physiology / Kinesiology & Human Movement — have no real equivalent outside the restricted PTA track, so both flipped to `covered:false`). Cascaded per the documented formula (`(acuAlign/16 × 0.85 + 0.15) × 100`, verified against `lincoln_trail`/`truman_college`'s own stored values): `lensScores.academic` 32 → 26. **`fitOlivier` and `lensScores.overall`/`.value` are untouched** — ACU alignment was removed from the Fit Score formula entirely in v37.1, so this correction cannot and does not move Indian Hills' rank.

**Spot-check of 7 other JUCOs, spanning old (v27–v39) and recent (v44.79–v44.84, post-RULE-0) additions, to check whether this was systemic:** `smc` (Kinesiology AA-T — real), `tyler_jc` (Kinesiology, AS — real), `arizona_western` (Exercise, Wellness and Nutrition — real), `cowley_cc` (Medical/Health Professions Transfer, explicitly listing a Physical Therapy pre-professional track — real, name paraphrased but accurate), `angelina_college` (A.A. Health and Physical Education — real), `trinidad_state` (Exercise Science, PE & Recreation, General Exercise Science Concentration, with the CSU Pueblo BS articulation this guide already cites — real), `illinois_central` (Physical Education, A.S. — real). **`glendale_cc_az` could not be checked — its site (and the district-wide `maricopa.edu`) returned a persistent Cloudflare challenge page in this session's browser; that's a network result, not a data finding, and its stored claim is unconfirmed either way, not disproven.** All 7 successfully checked schools were genuinely real — **this looks like an isolated fabrication from one early session, not a systemic pattern**, though a full audit of every pre-v42.9 (pre-RULE-0) school's degree claim has not been done and remains an open item (§6F).

**Verified:** `python -m json.tool data/juco.json` passes. `validate_schools.py`: 0 errors, 19 warnings (unchanged). `validate_consistency.js`: **Issues: 0**. Confirmed live in a local browser — Indian Hills' Details modal "Degree & ACU Align" tab renders the corrected title, note, and "2/16" ACU units; `fitOlivier` confirmed unchanged at 43; zero new console errors.

**Files:** `data/juco.json` (`indian_hills` — `degreeTitle`, `kinRank`, `acuAlign`, `acuAlignNote`, `acuUnits[EXSC225]`, `acuUnits[EXSC224]`, `lensScores.academic`), `CLAUDE.md` (§1, §6 current-version lines; §6F new open item), `athletes/olivier.json` (`guideVersion` v45.14 → v45.15).
...
**Root cause, and it's a genuinely different bug class from any prior icon fix this project has made.** `buildModalHeader()` only ever tries `DOMAINS[u.id]` (the athletics-specific domain, e.g. `uclabruins.com`) for the modal logo — never the school object's own `domain` field (`ucla.edu`), which the *card* renderer (`logoUrl()`) already uses successfully. For UCLA, `uclabruins.com/favicon.ico` genuinely 404s. The old 2-stage chain then fell back to Google's `s2/favicons` proxy for that same broken domain — but Google's `faviconV2` endpoint returns **HTTP 404 status with a real, valid, byte-identical ~726-byte generic-globe PNG in the response body anyway**. A browser `<img>` tag only checks whether valid image bytes came back, not the HTTP status code, so `onerror` never fires a second time — the chain silently "succeeds" showing a meaningless placeholder, and never reaches the text-initials fallback that was supposed to be the true last resort. Confirmed by hashing the response bytes: all affected schools return the exact same SHA-256, proving it's one shared generic asset, not 26 different "small" icons.

**A naive "which DOMAINS[] entries return ~726 bytes from Google's proxy" sweep produced a false-positive-heavy list of 26.** Re-verifying against what the app *actually* tries first (the direct `/favicon.ico` fetch, not the proxy) cut that to the real number: only 14 schools were genuinely broken end-to-end, because most of the 26 have a perfectly good *direct* athletics-domain favicon that the proxy check never exercised. This is the same "verify against the real thing, don't trust the intermediate signal" lesson this session's mascot-name re-audit had already surfaced once — it recurred immediately in a completely different part of the codebase.

**Fix — `js/app.js`, `buildModalHeader()` / `handleModalLogoError()`:** the modal now tries, in order: (1) `DOMAINS[u.id]` direct favicon, (2) the school's own `domain` direct favicon (skipped if identical to stage 1), (3) Google's proxy for the **school domain**, not the athletics one, (4) text initials. This resolved 6 of the 14 confirmed-broken schools for free, with zero research needed, because their university `.edu` domain already had a working icon the modal simply never tried: **`ucla`, `smc`, `northwestern`, `william_mary`** (all fixed via stage 2, direct `.edu` favicon) and **`ucsb`, `calpoly`** (fixed via stage 3 — their `.edu` domain's direct favicon also 404s, but Google's proxy for the `.edu` domain returns a real, hash-confirmed-non-generic icon). `virginia` needed a 7th `ICON_OVERRIDES` entry pointing straight at Google's proxy URL for `virginia.edu`, because the bare domain's TLS certificate is registered to a different hostname and hangs rather than failing promptly in a real browser (confirmed: `www.virginia.edu` also 404s directly, `uva.edu` doesn't resolve at all) — the override skips the slow/hanging direct attempt and goes straight to the confirmed-working (hash-verified, real Rotunda icon) proxy URL.

**Genuinely unresolved, disclosed rather than papered over:** 15 schools have no working favicon anywhere in the data — no distinct university domain to fall back to (`DOMAINS[]` and `domain` are identical, mostly small JUCOs/CCs with one combined athletics+institution site) — so they still show the generic globe in the modal: `glendale_cc_az`, `neosho_county_cc`, `iowa_lakes_cc`, `blinn_college`, `coastal_bend_cc`, `angelina_college`, `eastern_arizona`, `central_wyoming`, `laramie_county_cc`, `western_nebraska_cc`, `trinidad_state`, `jefferson_college_mo`, `harcum_college`, `central_georgia_tech`, `noc_enid`. A quick attempt to backfill these via Wikipedia infobox images was abandoned mid-session — most of Wikipedia's thumbnails for these schools are campus photos or signage, not usable icons at a 30px modal size, and shipping those would trade one wrong-looking icon for a different wrong-looking one. These need the same real Tier-1 logo-sourcing rigor as v45.11–v45.12 (Google Images search, cross-checked against ≥2 independent sources, saved as a local `assets/logos/` asset) — tracked as an open item in CLAUDE.md §6 rather than rushed.

**Verified locally** via `openDetail()` calls against all 26 originally-flagged schools plus a 10-school regression sample (major D1/D2/Ivy schools untouched by this change) — confirmed real icons load (`naturalWidth`/`complete`, not just "a request fired") for every fixed school, confirmed the 15 still-broken schools fail the same way as before (no regression, no silent new failure), and confirmed zero JavaScript runtime errors in console across the whole test run (network-level 404/DNS/cert failures from the fallback chain's earlier stages are expected and unavoidable — those aren't bugs).

**Files:** `js/app.js` (`buildModalHeader()`, `handleModalLogoError()`, `ICON_OVERRIDES` — `virginia` entry added). `validate_schools.py`: 0 errors, 19 warnings (unchanged). `validate_consistency.js`: **Issues: 0**. `negtest.py`: 7/8 proven (the 8th is the expected uncommitted-file guard, not a failure — re-confirms clean after commit).
...
**What got built, in dependency order, each tested against real repository data rather than synthetic examples:**

1. **`qa-suite`** — bundles Phase 4's validation sequence (`validate_schools.py`, `validate_consistency.js`, `json.tool`/`node --check` on changed files, a conditional `negtest.py` run) into one command. Building it surfaced a stale fixture in `negtests/checks.json`: the `MAXAID / missing field` case's `find` string used a bare `\n` where `data/conferences.json` actually uses CRLF line endings, so `negtest.py`'s own `newline=""` file-open mode meant the string never matched — the case was silently proving nothing (`MUTATION-NOOP`). Fixed; the suite now goes 8/8 proven.

2. **`new-school`** — sequences Change Type 1's research (Phase 1, subsections 1A-1J) and file-edit (Phase 3, 3A-3F) steps, plus two scripts: a proper JSON-parsing duplicate check (replacing the grep one-liner CLAUDE.md's own Phase 1A text already warned "misses compound IDs like tyler_jc"), and a cross-file coverage checker for the touchpoints neither `validate_schools.py` nor `validate_consistency.js` covers — `conferences.json`'s `guideSchools[]`/`otherSchools[]`, `conf-prestige.json`, `pipeline.json`, and this file's own School → File Reference Table. The coverage checker's first version false-matched "UC Riverside" against "California Baptist" (an unrelated school in a different conference's `otherSchools[]`) via the word "California," pulled from the `full` field's "University of California, Riverside" — fixed by preferring tokens from the school's own `name` field first, falling back to `full` only when `name` alone yields nothing.

3. **`add-coach`** — sequences Change Type 2, plus a rank-order checker (verifies `coaches.json`'s `rank` field is both a contiguous 1..N sequence AND actually sorted by `overallScore` — `validate_schools.py` only catches duplicate rank *values*, never a genuine inversion) and a bio-hygiene sweep for the two specific bugs CLAUDE.md already documents by name: a stale email embedded in `bio` prose that disagrees with `contact.email` (v44.35's exact St. Edward's precedent), and a hardcoded athlete name (v44.28's rule). **Running the bio sweep for real found two live instances, not hypothetical ones:**
   - `muuss` (Wake Forest) — `contact.email`/`contact.phone` were stale (`bmuuss@wfu.edu` / `336-758-5640`). Verified live via godeacs.com's own staff page: current email is `muussrp@wfu.edu` (which the coach's own `bio` text had already correctly cited as "HC email" — the discrepancy was entirely in the unrendered `contact` object), current phone `336-758-5783`. Fixed.
   - `coach_rose_state` — `bio` ended "...directly relevant to **Olivier's** own field," a hardcoded athlete name. Reworded to "a player pursuing a pre-PT/Kinesiology pathway."

4. **`roster-refresh`** — sequences Change Type 3. Discovered mid-build that `apply_roster_refresh.py` (repo root) is not the reusable tool its name suggests: it's a ~1,300-line hardcoded `PATCHES` dict, one literal Python entry per school, built up across many past sessions, with no way to refresh one school without editing that file directly. Built a real single-school calculator (`refresh_school.py`) that imports the formula functions from `apply_roster_refresh.py` directly (per this repo's own "never a second copy of a scoring formula" rule) rather than re-deriving them, computes the correct cascade for both the 4-year and JUCO trajectory formulas, and — new — snapshots a school's `rising_senior_2027_names`/`rising_junior_2027_names` *before* overwriting them, diffing to catch an unexpected departure and queue it to `roster_moves_queue.json`. Three companion checkers, each proving a real, previously-unfilled gap:
   - `check_roster_arithmetic.py` — the real invariant, verified empirically against all 158 `available:true` schools before writing the check, is `cleared + rising_sr + rising_jr <= mf_total` (an inequality — 107 of 158 have a legitimate untracked freshman group `mf_total` accounts for but no bucket names; an equality check would have been 107 false positives on day one). Also found `monroe_college`'s `rising_junior_2027_count` (2) disagreeing with its own `rising_junior_2027_names` array length (1) — traced to a deliberate collective placeholder string ("Current Fr MFs → sophomores by 2027") standing in for two untracked individuals, not a data-entry error, so the checker treats this class as an advisory warning, never a hard failure.
   - `check_juco_trajectory.py` — verifies a JUCO's stored `trajectory[].pct` actually matches what the documented v44.92 formula computes from its own stored `cleared_before_2027`/`mf_total`. Nothing else checks this — the FIT check verifies scores are consistent with whatever trajectory happens to be stored, never that the trajectory itself is right. Clean pass on all 81 available JUCOs.
   - `check_no_jargon.py` — sweeps `trajectoryNote`/`recruit_pathway_note` (both render **verbatim** on the live Minutes Outlook tab) for leaked internal process language — §-references, `CLAUDE.md` citations, backtick code identifiers, ALL_CAPS constant names, the "MIXED VINTAGE" marker. This is the same bug class v44.89 already fixed once (15 entries) — and the check found it had recurred: **`neosho_county_cc` and `lsu_eunice`'s `recruit_pathway_note` were still disclosing "trajectory deliberately NOT recomputed — §14's Opportunity Score table cannot reproduce any stored JUCO trajectory... pending the JUCO calibration item in CLAUDE.md §6."** That caveat became false the moment v44.92 shipped the real JUCO formula and recalculated both schools' trajectory numbers (confirmed live — both show real, non-placeholder percentages) — the note was simply never updated to match. Fixed both: removed the obsolete trailing sentence.

5. **`transfer-tracking`** — companion skill, run after a refresh wave (not per school). A cross-school duplicate-name scan across all 170 schools' current midfielder lists (no roster history needed — a name at two schools in the same snapshot is a candidate on its own) plus a processor for the departure queue `roster-refresh` produces. Run for real: 1,120 tracked names, 12 cross-school hits — read carefully, most sit in `cleared_names` at *both* schools, which is the weakest possible transfer signal (graduating in two places at once isn't a move, it's almost certainly two different people sharing a name). Deliberately does not write to `proPlayers.nextLevel` on a confirmed match — Section 5b's own "one confirmed hit vs. multiple hits across tracker years" distinction needs the same judgment as the original research, not automation.

6. **`mls-pipeline`** — kept separate from `transfer-tracking` on purpose (different trigger: the SuperDraft's own annual calendar, not a refresh wave; different data source: `pipeline.json`'s `mlsDraft[]`, not `nextLevel`). Cross-references a year's draft picks against the current snapshot and the full departure-queue history (including resolved entries — the queue must stay append-only for this to work at all). Verified against the exact Edouard Nys / Northeast CC → UIC example Section 5b already uses as its canonical illustration of why this gap exists.

**A real, still-open gap found across three of these skills, not fixed as part of this session (deliberately — it's a schema decision, not a script bug):** `coaches.json` has no structured "verified" field anywhere. The word "verified" appears exactly once repo-wide, as free prose inside one `overallScoreNote`. The v44.88 coach spot-check rule is real, required, and CLAUDE.md documents exactly why it was added (a prior batch claimed the step "done" for 10 schools when only 1 was genuinely checked) — but nothing in the schema can prove today, from the data alone, that the check ever happened. Flagged for the next time this area is touched; see §9.

**Files:** `.claude/skills/qa-suite/`, `.claude/skills/new-school/`, `.claude/skills/add-coach/`, `.claude/skills/roster-refresh/`, `.claude/skills/transfer-tracking/`, `.claude/skills/mls-pipeline/` (all new), `negtests/checks.json`, `data/coaches.json`, `data/juco.json`, `athletes/olivier.json` (guideVersion), `CLAUDE.md` (§9, version headers).

`validate_schools.py`: 0 errors, 24 warnings (unchanged). `validate_consistency.js`: **Issues: 0**. All six skills' scripts run clean via `qa-suite` and their own smoke tests.
...
Batch 3 covers the 13 non-JUCO schools never previously attempted by any prior wave: `ncstate`, `clemson`, `ucla`, `charlotte`, `ucsb`, `ucsd`, `csuf`, `lynn`, `nova`, `csula`, `chapman`, `uc_charleston`, `georgian_court`. All 13 researched live via Claude-in-Chrome per RULE 0. **4 had not yet published a 2026-27 roster** (`clemson`, `nova`, `csula`, `chapman`) — confirmed via each site's own season dropdown (no 2026/2026-27 option present), and in every case the current-season page's MF count matched what's already stored exactly, so these were left untouched rather than guessed at.

The other **9 were refreshed with the full trajectory/score cascade** (non-JUCO, so the JUCO facts-only carve-out in §6E does not apply): `ncstate` (12 MFs, fit 56→49), `ucla` (10 MFs, fit 65→71 — real churn, 8 of 10 clear/rise-to-senior), `charlotte` (8 MFs, fit 65→55), `ucsb` (11 MFs, fit 63→59), `ucsd` (9 MFs, fit 63→51), `csuf` (12 MFs, fit 53→47), `lynn` (11 MFs, fit 53→61), `uc_charleston` (14 MFs, fit 49→41), `georgian_court` (15 MFs, fit 27→42 — the largest swing in the batch, driven by 10 of 15 MFs clearing in one graduating class). `apply_roster_refresh.py`'s `PATCHES` dict was extended with all 9 entries (opportunity-score formula and trajectory interpolation unchanged, reused as-is) and run per conference file; every already-applied school in those same files reproduced byte-identical before/after values, confirming no drift.

**A real extraction bug was caught and fixed mid-session, not after.** The first attempts at `ucsb` and `csuf` used a naive per-field `querySelectorAll` zip (separate arrays for name/position/class-year, joined by index) — this silently misaligns whenever one field has fewer populated entries than another (confirmed on `csuf`'s sparse previous-school column: 28 populated cells across 34 players). Re-extracting via each player's own card container (`.sidearm-roster-player`, then `.querySelector()` scoped to that element) produced completely different class-year distributions for both schools. **Every school's data was re-verified with the card-scoped method before being trusted**; `ucsb`'s and `csuf`'s notes both flag this explicitly as a caution for the next session hitting a similar Sidearm card layout.

Coach spot-check (per the Change Type 3 rule added v44.88) run for all 9 refreshed schools via each school's own roster/coaches page — all 9 stored names matched exactly, no `coaches.json` changes needed. `recruit_pathway` reclassified on evidence for 3 schools (`charlotte` Freshman-friendly→Mixed, `ucsb` Freshman-friendly→Mixed, `csuf` Portal/JUCO-heavy→Mixed, all citing the specific transfer counts found) and reconfirmed-not-just-retained for 2 (`ncstate` Mixed, `georgian_court` Freshman-friendly); retained without re-derivation where the site publishes no previous-school column (`ucsd`, `lynn`, `uc_charleston`), each flagged lower-confidence per the established convention.

`validate_schools.py`: 0 errors, 25 warnings (unchanged baseline). `validate_consistency.js`: **Issues: 0**. Local browser-verified (`olivier-guide`, port 8787): NC State's card and Minutes Outlook tab confirmed against the computed values exactly (Fit 49%, Minutes score 38, correct cleared/rising-junior names and trajectory labels); Georgian Court's card confirmed (Fit 42%, coach Dino Raso). No console errors beyond expected offline favicon 404s.

Files: `data/aac.json`, `data/acc.json`, `data/big-ten.json`, `data/big-west.json`, `data/d2.json`, `apply_roster_refresh.py`, `athletes/olivier.json` (guideVersion), `CLAUDE.md` (§6C batch plan). See the `project-roster-refresh-campaign` memory for the corrected campaign status.
...
Every stored **name** was already correct — the Batch 1 red flags (an assistant showing on `northeast_cc`'s roster page, a team manager on `cowley_cc`'s roster table, an empty Coaching Staff section on `angelina_college`) were all artifacts of reading the *roster* page instead of the school's dedicated *coaches* page, which in every case cleanly listed the stored head coach. **4 of 9 had a stale `title` field**, corrected along with the matching bio text (per the existing Type 2 bio-sweep rule): `daytona_state` "Head Coach" → "Head Men's Soccer Coach"; `johnson_county_cc` "Head Men's Soccer Coach" → "Head Coach"; `coastal_bend_cc` "Head Men's Soccer Coach" → "Head Coach"; `glendale_cc_az` "Head Men's Soccer Coach" → "Head Coach Men's and Women's Soccer" (Jeff Perry coaches both programs — the entry's own `overallScoreNote` already said so, the `title` field just hadn't been updated to match). No `overallScore` changed on any of the 9, so no coach re-rank was required — rank order is untouched by a title/bio-only edit. `efsc` remains untouched/deferred, unrelated to this pass.

**Also closed a standing process gap this surfaced:** nothing in §3a Change Type 3 (the minutesOutlook/roster-refresh cascade) required a coach spot-check as part of a roster refresh — the Batch 1 miss was a one-off backlog item, not a rule, so the same gap could recur on the next roster-refresh batch. Added a permanent "Coach spot-check — required alongside every roster refresh" paragraph to Change Type 3's cascade in `CLAUDE.md`, plus a line in its "Tabs to verify" list, so a future session is required to do this in-line rather than defer it.

`data/coaches.json`, `athletes/olivier.json` (guideVersion bump), `CLAUDE.md` (§3a Change Type 3 — new coach spot-check rule; §6C — resolved the Batch 1 coach-verification backlog item, updated the campaign's "Next action" pointer). `validate_schools.py`: PASS, 170 schools, 25 warnings (unchanged). `validate_consistency.js`: **Issues: 0**. Local browser-verified via a temporary local server: fetched `data/coaches.json` live in-page and confirmed all 4 corrected titles render with unchanged, sequential ranks; Coaches & Staff → Profiles tab spot-checked for `coastal_bend_cc`, confirming the corrected bio text ("Head Coach") renders in place of the old "Head Men's Soccer Coach" phrasing; zero console errors from `coaches.json` parsing (only the usual favicon 404s for domains unreachable from this environment).

Roster campaign: no `minutesOutlook`/`roster_season` fields touched this session — Batch 2 proper (the 7 "published but empty" JUCOs deferred to late August, plus the `tyler_jc` re-check) is still pending.

---

### v44.87 (2026-08-16) — Roster refresh campaign Batch 1: 10 JUCOs facts-refreshed to 2026-27, 1 deferred (Change Type 3, JUCO)

`data/juco.json` — facts-only refresh (§3a Type 3 JUCO rule) for the 10 schools the 2026-08-06 survey had confirmed as "populated on 2026-27, extraction pending": `miami_dade`, `northeast_cc`, `daytona_state`, `iowa_lakes_cc`, `angelina_college`, `johnson_county_cc`, `glendale_cc_az`, `arizona_western`, `cowley_cc` — plus `coastal_bend_cc`, newly confirmed this session as genuinely populated on 2026-27 (its earlier scripted probe had 403'd — a CloudFront datacenter-IP block, not a data gap; the Claude-in-Chrome connector on a real residential IP reached it cleanly, same pattern as Iowa Western/Suffolk in v44.86). All 11 rosters were read directly via Claude-in-Chrome (Rule 0) — several needed raw `<table>` DOM extraction rather than `get_page_text`, since Johnson County CC / Iowa Lakes / Angelina all render their roster inside a table that the page-text extractor's article-detection heuristic misses entirely (returns "no text content found" despite a fully populated table existing in the DOM).

For each of the 10: `mf_total`, `roster_season` 2025-26→2026-27, `cleared_before_2027` (+ names), and `recruit_risk` were re-derived from the live roster applying the JUCO class-year rule (a 2026-27 sophomore clears before Olivier's Aug 2027 arrival; a freshman returns as a 2027-28 sophomore and competes with him). Several schools' new sophomore-class names exactly matched the prior season's stored freshman names (Miami Dade, Arizona Western, Cowley, Iowa Lakes), independently confirming the extraction is accurate season-over-season. `trajectory[].pct` and the score cascade (`fitOlivier`, `lensScores`) were deliberately **left untouched** on all 10 — same JUCO calibration-gap reasoning as v44.86 (§6E); each `trajectoryNote` carries the ⚠ MIXED VINTAGE disclosure.

**`efsc` (Eastern Florida State) was deferred, not refreshed.** Its live 2026-27 roster is populated (33 players, checked in both List and Headshot view) but genuinely publishes no position data for any outfield player — only the 3 goalkeepers show a Position field. Confirmed via direct DOM cell inspection (the position `<td>` is an empty text node, not a rendering artifact) rather than assumed from the visible table. This is a field-level gap in the same family as the OCU/St Edward's precedent (§15) — the squad is real, the position column just isn't published this season. Kept on stored 2025-26 data with a disclosure note; re-check if EFSC starts publishing positions.

Arizona Western's roster header reads "HOMETOWN/PREVIOUS SCHOOL" but the column actually contains phonetic pronunciation guides, not transfer data — noted so a future session doesn't misread it as a `recruit_pathway` signal. Cowley CC's season-scoped roster URL serves a page whose own `<title>` says "Standings" (a site-side mislabel); the table content was verified as genuine roster data before use.

`data/juco.json`, `athletes/olivier.json` (guideVersion bump), `CLAUDE.md` (§6C — removed the 10 resolved schools from the pending list, logged `coastal_bend_cc`'s resolution, corrected the stale "not yet committed" note on the v44.86 Iowa Western entry which was in fact committed with v44.86, updated the roster_season ledger count). `validate_schools.py`: PASS, 170 schools, 25 warnings (no new ones). `validate_consistency.js`: **Issues: 0**. Local browser-verified via a temporary local server: `unis[]` shows all 10 refreshed schools with correct `mf_total`/`roster_season`/`cleared_before_2027`, `efsc` correctly still on 2025-26, all `fitOlivier`/`lensScores` values unchanged from before the refresh (confirms the facts-only cascade moved zero scores); Minutes Outlook tab spot-checked for 3 schools; zero console errors (only expected favicon 404s for schools whose athletics domains don't resolve from this environment).

Roster campaign progress: 74 schools now on 2026-27 (was 64), 40 on 2025-26 (was 51 minus 1 deferral wrinkle — see CLAUDE.md §6C for the exact ledger), 7 `available:false` unchanged. Next batch: the 7 "published but empty" JUCOs deferred to late August (`pima_cc`, `barton_cc`, `phoenix_college`, `smc`, `indian_hills`, `mohave_cc`, `southeastern_cc_ia`), plus re-checking `tyler_jc`.
