...
### School → File Reference Table (174 schools)
...
| Lamar CC | `data/juco.json` | `lamar_cc` | JUCO | NJCAA DI / Region 9 |
...
| Northeastern Junior College | `data/juco.json` | `northeastern_junior_college` | JUCO | NJCAA DI / Region 9 |
| Northeastern Oklahoma A&M | `data/juco.json` | `neo_am` | JUCO | NJCAA DI / Region 2 |
| NorthWest Arkansas CC | `data/juco.json` | `nwacc` | JUCO | NJCAA DI / Region 2 |
...
| Oxford College (Emory) | `data/juco.json` | `oxford_college_emory` | JUCO | NJCAA DIII / Region 17 |
...
| Southern Crescent Tech | `data/juco.json` | `southern_crescent_tech` | JUCO | NJCAA DI / Region 17 |
| Southwestern Christian | `data/juco.json` | `southwestern_christian` | JUCO | NJCAA DI / Region 5 |
...
| Trinidad State | `data/juco.json` | `trinidad_state` | JUCO | NJCAA DI / Region 9 |
...
| Western Nebraska CC | `data/juco.json` | `western_nebraska_cc` | JUCO | NJCAA DI / Region 9 |
...
| Yavapai College | `data/juco.json` | `yavapai_college` | JUCO | NJCAA DI / ACCAC |
...
| `data/conferences.json` — `mlsPicksByYear` on the conference's card | The Conferences tab's MLS Picks column and card line (v45.64). Counts picks in the last 5 SuperDrafts by the conference's CURRENT members; recount every year with the mls-pipeline skill (step 0b). |
...
For JUCO schools, also add: `"juco2yr": true`, `"jucoTier": "Elite" | "Standard"`, and `"jucoTierNote"` (informational badge only, NO Fit Score cascade). **Elite rule (owner-approved 2026-09-17, time-bound):** in the last 3 completed seasons (rolling; 2023-2025 as of v45.53) the school won or finished runner-up at its NJCAA national championship, OR had 2+ NJCAA All-Americans in one season. Final Four does not count; older titles do not count. A top-5 final national poll was also approved but cannot currently be verified (njcaa.org rankings unavailable), so it is not applied until a source exists. **Re-check every season** once the new All-America teams and finals are published. The note is the badge tooltip, so write it as plain facts naming the season. 15 of 89 JUCOs are Elite as of v45.53. NJCAA schools also carry `"njcaaDivision": "I" | "II" | "III"` (added v45.53), which feeds division strength and must match `fundingPathway` (I full, II capped, III none; `NJCAA-DIV` check). Santa Monica (CCCAA) has neither `njcaaRegion` nor `njcaaDivision`.
...
- `minutesOutlook.recruit_risk` — must be exactly `Low` | `Medium` | `High`. The renderers have no branch for anything else: `Very High`, `Medium-High`, `Moderate`, or sentence-style values all fall through to the green "Open" label — the opposite of the researched meaning. **Value rule (v45.55, `RISK` check):** count the midfielders still on the roster in the recruit's first season, `mf_total − cleared_before_2027`: 7+ `High`, 3–6 `Medium`, 0–2 `Low`. It renders as Crowded / Moderate / Open under "Entry Competition". Never hand-set it.
...
mlsPicksByYear {"2022":n,…,"2026":n}, mlsPicksSource,   ← v45.64, replaces the hand-rated mlsPipeline text; counted from MLS's SuperDraft trackers, each pick's college matched to its current conference in the NCAA directory (audit list: data/mls-draft-picks-2022-2026.json). GROUP check enforces it.
scholarships,
...
`divStrength` map (`DIV_STRENGTH` in scores.js): D1=1.0, IVY=0.9, D2=0.8, NAIA=0.65, D3=0.5, JUCO=0.6. **Since v45.53 NJCAA schools use `NJCAA_DIV_STRENGTH` by `njcaaDivision`: I=0.6, II=0.55, III=0.42**, from the average Massey rating of each division against Division I (final 2025 season, masseyratings.com/csoc2025/njcaa/ratings: DI 6.50, DII 5.91, DIII 4.57). Recompute from Massey only if the owner asks; the whole term is worth at most 4 Fit points. Note this is a *different* map from the retired `soccerLevelMap` (which had JUCO=0.75); don't confuse the two if referencing old data.
...
**Current version: v45.66 (2026-09-21).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
- **174 schools**, all full-profile, across 10 conference JSON files. **174 coaches** in `coaches.json`, ranked 1–174.
...
- **Climate-Neutral lens + Fit Score band filter (v45.32).** A 6th "View by" lens ranks schools with the climate term removed and the surviving weights renormalised to 100. It is **computed live** by `climateNeutralFit()` in `js/scores.js` and read through `lensValue()` in `js/app.js` — there is **no 7th `lensScores` key**, deliberately: both validators hard-enforce exactly 6 keys, and a stored derived value nothing recomputes is the v44.30 `lensScores.value` drift class. **Superseded in part by v45.33 — the lens DOES swap the DISPLAYED Fit Score.** v45.32 shipped it as ordering-and-badges only, which left the visible Fit column non-monotonic (the ACC section read 71, 67, 67, 52, 49, 48, 47, **55, 55**, 42, 39, **52**, 36) because the cards were sorted by a number that was not on screen — owner-reported as *"why don't the fit scores change when I select climate neutral?"*. While the lens is active the card tile now shows the climate-neutral value (label `FIT · NO CLIMATE`) and the canonical score moves to a `.cn-delta-chip` in the metadata row (`Standard fit 52 ▼9`); the modal mirrors it. **`school.fitOlivier` is still never written** — the swap is display-only, so sorting, Compare, both validators and every stored score stay canonical. All three Fit display paths (card tile, modal template, and `openDetail()`'s post-paint `requestAnimationFrame` write) route through one `displayFit(u)` helper — that last one silently reverted the modal before it was included, and could not be caught by a test in a hidden browser pane, where rAF never fires. **Generalised to all six lenses in v45.34** (the deferral recorded here lasted one day): every lens now shows the score it sorts by, with its own tile label — `ACU MATCH` / `MINUTES` / `LIFESTYLE` / `VALUE` / `FIT · NO CLIMATE` — and the canonical score on the chip. Academic-First had been out of order in 47 of 135 pairs; all six now read 0 of 135. **Only `overall` and `climateNeutral` use `fitColor()`** — `LENS_TILE` marks them `fitScale: true` because their score is a Fit Score on the Fit scale. The other four have their own ranges (academic 15–95, minutes 12–78, lifestyle 0–100, value 19–69), so the 58/48 cuts would assert bands nobody defined; they render in neutral ink via `displayFitColor()`. For the same reason the chip's delta arrow is **conditional** — subtracting an ACU score from a Fit Score is valid arithmetic and meaningless, so only the Fit-scale lenses show one. **v45.35 added `redundantWith`**: Academic-First's score is a pure monotonic transform of `acuAlign`, so its swapped tile duplicated the ACU Align tile exactly; that third tile now shows the canonical **Fit Score** under this one lens (and the chip is suppressed, since it would duplicate it back). Declare the collision on `LENS_TILE` rather than adding a `currentLens ===` branch if another lens ever reuses an existing tile's input. Alongside the lens, three Explore `fitband` filter chips cut at **58 / 48** (34 / 55 / 81 schools as of v45.55), sharing `fitBand()` with the display colour so the two cannot disagree.
- **Conferences tab derives its school lists and counts (v45.63–v45.65).** Each `conferences.json` card and `conf-prestige.json` row has a `group` (the `resolveConfGroup()` key; an array for the JUCO card). Guide schools, In Guide and "Programs in Guide" are computed from `unis`; the stored figures are `soccerTeams` (verified Total Programs, with `soccerTeamsSource`) and `mlsPicksByYear` (counted SuperDraft picks 2022-26 by current members, with `mlsPicksSource`). JUCOs show one card per NJCAA region. The table has a "Rank by" switch (hand-set conference ranking by default, owner ruling: no official cross-division conference ranking exists; or MLS picks). The GROUP check enforces all of it.
...
- **`roster_season` doubles as the roster-refresh ledger** — the Minutes Outlook tab shows at a glance which schools are on 2026-27 data and which are still on an older season (now with an explicit ✓/⏳ badge and filter as of v45.03, see below). **Recount as of v45.74 (2026-09-24): 155 on 2026-27 / 5 on 2025-26 / 14 `available:false`** (174 total; verified against the raw data). The 5 remaining older-season schools are `monroe_college` (owner ruling, left alone), `truman_college`, `wilbur_wright_college`, `trinidad_state`, `pacific_northwest_christian_college` — all 4 non-Monroe ones genuinely blocked this session too (see §6C). This supersedes the v45.45 figure (150/8/12, which itself was against 170 schools — the guide has since grown to 174). Do not re-derive this count from memory; re-run the query in §6C's method instead.
...
- **✅ RESOLVED v44.61 — "Soccer Level" and "Pre-PT Path" no longer parse prose.** Both now read short AUTHORED fields, `soccerLevelShort` (≤24 chars) and `prePTShort` (enum-locked to Outstanding · Excellent · Very Strong · Strong · Good · Foundation · Poor · Transfer Pathway). **Both counts logged here were LOW — the real figures were 7 and 4, not 6 and 2** (`yale`/`princeton` "Strong via science pathway" had been missed). The new `SHORTFIELDS` check has four halves including a comment-stripped grep of `js/app.js` AND `js/dashboard.js`. **📌 The long `soccerLevel` string now has NO renderer consumer** — stored reference data, like `conferences.json.scholarships` after v44.50. That also makes the long-deferred UX-D1 *formatting* item moot for display: the inconsistent JUCO shapes no longer reach the UI. **`miami_dade`'s deferral (left as `NJCAA` because its division was "unpublished on its own site") was superseded and RESOLVED v45.57** — see the entry directly below.
- **✅ RESOLVED v45.57 — `soccerLevelShort` standardized to "NJCAA Division I/II/III" for all 88 NJCAA schools; owner-reported inconsistency (Miami Dade showed bare `"NJCAA"`, Daytona State showed `"NJCAA Division I"` for the identical fact).** 40 schools corrected: 1 (`miami_dade`) from the bare `"NJCAA"` fallback, 39 from the `"NJCAA DI — Region N"` format used by the NJCAA gap-fill campaign batches. All values were derived from each school's already-stored `njcaaDivision` field (I/II/III), not re-researched — except `miami_dade`, whose division the v44.61 note said was unverified. Re-checked live (RULE 0): Miami Dade's own official athletics Instagram (`@mdcsharksmsoc`, linked from `mdcathletics.com`) states "NJCAA Division 1" directly in its bio, so the claim is genuinely published and the label now reads `"NJCAA Division I"`. `njcaa.org` itself could not be reached to cross-check (geo-blocked without the owner's VPN, per §6C) — the Instagram bio is the Tier-1 evidence on file for this one school; re-verify against njcaa.org directly next time this school is touched with the VPN on. No score cascade — `soccerLevelShort` is a display-only field with no scoring consumer (§3a note: the long `soccerLevel` string already has none; this short field doesn't either).
- **✅ RESOLVED v45.58 — Division row: D1/D2/D3 + JUCO now narrows JUCOs by NJCAA division.** Owner rule: `D1`+`JUCO` shows **only NJCAA Division I JUCOs** (no four-year D1 schools); `D2`+`JUCO` Division II; `D3`+`JUCO` Division III; `JUCO` alone all 89; `D1` alone, `D1`+`D2` etc. unchanged. Implemented in `divFilterMatch()` (`js/app.js`, above `applyFilters()`), reading `njcaaDivision` via the card's `data-njcaadiv`. NAIA/Ivy chips still add to the result. Santa Monica (CCCAA, no division) appears under `JUCO` alone but not once a level is selected. **History, so it is not repeated:** v45.57 first added separate `JUCO DI/DII/DIII` chips, then reverted them after Claude wrongly argued the OR behaviour already met the ask. The owner's original report meant the level chips should narrow JUCO; ask for the exact expected result of a chip combination before arguing about the existing mechanics.
...
- **✅ RESOLVED v45.51 — `index.html` Financial Model search box said "Search 110 schools…".** Now "Search schools…", so it cannot go stale.

#### B. Coach data

- **✅ RESOLVED v45.71 — `malcolm_x_college`'s vacant head coach position filled: Aziz Tahir.** Jesse Rosen (head coach 2014-2025) left in May 2026 for Chicago State University. His successor, Aziz Tahir, was confirmed via three independent recruiting-platform listings (NCSA, SportsRecruits, ProductiveRecruit — all agreeing on the same name) plus a `directory.ccc.edu` employee-directory record confirming he is a genuine current CCC employee. `coaches.json` updated: `name`, `title`, `record`, `bio`, `overallScoreNote`, `note`. **`overallScore` deliberately left at 30 (neutral placeholder)** — no CV, playing background, or licence has surfaced for him anywhere yet, and §5d forbids scoring without evidence; re-score properly once a real bio is found. Contact details (`email`/`phone`) stay `null` — unconfirmed. No re-rank needed (score unchanged at the same rank-174 floor, so no gap was created). **🚩 New finding this session, worth its own line below:** a second, singular-form domain (`citycollegeofchicagoathletics.com`, no "s") was blocked by an automated safety classifier as a possible risk before it could be read — do not assume it is safe just because it differs from the already-documented compromised plural-form domain.
- **✅ RESOLVED v45.74 — `wilbur_wright_college`'s Gabriel Billings has left the college entirely; no longer ambiguous.** The v45.30 finding treated a June 2026 article as ambiguous "past-tense-leaning CV language." Reading the full source (SLIAC's own official announcement, 2026-06-29, not just a snippet) removes the ambiguity: Eureka College "selected Gabe Billings to lead its women's soccer program," and his Wright role is described purely as prior CV ("Most recently, he has served as...") with no indication of a concurrent dual role. `coaches.json` updated to `name: "Vacant"`; no successor found despite a real search (a part-time "Head Coach - Men's Soccer" posting from May 28, 2026 has since closed with no announced hire). See §6C's 2026-09-24 re-check table for detail.
...
| **4** | `tyler_jc` — access confirmed working 2026-08-20 (direct, then VPN + cache-bust if 403; see the dated note below the table). **Use the `msoc` URL slug, not `mens-soccer`, and wait a few seconds before reading — both a wrong slug and an early read produce a false "0 players" result on this site.** **Update 2026-09-14: the site has migrated to a new platform.** The live roster is now at `apacheathletics.com/teams/407485412185999056/msoc/2026-27/roster` (reached in Claude-in-Chrome with the owner's VPN on — a VPN is required, see the geo-block note below the table; still wait ~4s before reading). The old `/sports/msoc/index` URL still resolves but lands on an empty stats stub with no roster link, and the old coaches page (`/sports/msoc/coaches/index`) now 404s, with no staff directory anywhere on the new site. | **✅ DONE v45.36** — refreshed to 2026-27 (34 players, 14 MFs: 7 So. clear, 7 Fr. return; 5 GK). Fit 71→67. **Coach spot-check NOT completed**: no live staff page exists on the new platform, so Steve Clements is unconfirmed for 2026-27 and `coaches.json` was left unchanged. The stored `url` also still points at the stats stub. Re-check both once the new site publishes a coaches page. **Cross-checked 2026-09-14 against the NJCAA stats roster: all 34 players match exactly on name, position and class (GP 5). Same platform, same team ID, so this is a consistency check, not an independent source (§15).** |
...
#### Batch 7 — the remaining 38 schools on 2025-26 rosters (planned 2026-09-14) — ✅ DONE v45.37–v45.45 (2026-09-15)

**Outcome: 30 of 38 refreshed to 2026-27; 8 kept on 2025-26.** Every sub-batch was committed, pushed, and verified live. Per-school detail, including every NJCAA cross-check result, is in CHANGELOG v45.37–v45.45.

**Re-checked 2026-09-24 (v45.74) — 3 of the 8 refreshed, 4 still blocked, 1 left per owner ruling:**

| School | 2026-09-15 state | 2026-09-24 outcome |
|---|---|---|
| `monroe_college` | 2026 roster is a 59-player **four-year** roster (NCAA D2 transition, §6F). **Owner ruling: leave for now.** | Not touched — owner ruling still stands, excluded from this re-check |
| `truman_college` | Region 4 site has no 2026-27 season; NJCAA roster has positions but every class year blank | **Still blocked** — region4sports.com now behind a CAPTCHA-style human-verification wall (not bypassed, per policy); NJCAA's roster is real for 2026-27 (28 players, real positions) but still has **no class years at all**. Note refreshed, data unchanged. |
| `kennedy_king_college` | Region 4 site has no 2026-27 season; NJCAA roster has only 1 GK | **Refreshed.** NJCAA's roster now publishes a complete 2026-27 squad (27 players) with real positions AND class years — the specific blocker cleared. `mf_total` 6→12, `cleared_before_2027` 6→2, `recruit_risk` Low→High, trajectory Yr1 59→47/Yr2 72→60, `fitOlivier` 46→42. Coach (Jonathan Pachar) independently confirmed unchanged via 2 recruiting platforms + active box scores. |
| `wilbur_wright_college` | Region 4 site has no 2026-27 season; NJCAA says "No roster available" | **Still blocked**, same reason — but a real, separate finding: **the program's head coach, Gabe Billings, has left the college entirely**, confirmed via a Tier-1 SLIAC press release (2026-06-29) announcing his hire as Eureka College's Head Women's Soccer Coach. This resolves the "genuinely ambiguous" open item from earlier — he did not stay in any capacity. `coaches.json` updated to `name: "Vacant"`; no successor found despite a real search (a part-time head-coach posting closed May 2026 with no announced hire). Roster data unchanged. |
| `trinidad_state` | Own page and NJCAA both list 29 names with ~22 positions/classes blank | **Still blocked** — own site (tsctrojans.com) now behind the same CAPTCHA wall as Truman; NJCAA's 2026-27 roster still has only 3-4 of 28 players with both position and class filled in. Note refreshed, data unchanged. |
| `pacific_northwest_christian_college` | Own page and NJCAA both list 15 players, 10–11 positions blank, 1 GK | **Still blocked** — own site now shows a real 2026 season, but still only 5 of 15 players carry any position label, and none of those 5 is a confirmed midfielder. Note refreshed, data unchanged. |
| `harcum_college` | Own page 17 players / 1 GK; NJCAA "No roster available" | **Refreshed.** Own official roster page now publishes a complete 2026-2027 squad (17 players, full positions and class years). `mf_total` 5→6, `cleared_before_2027` 5→1, `recruit_risk` Low→Medium, trajectory Yr1 58→50/Yr2 71→63, `fitOlivier` 55→53. Coach (David Hughes) independently confirmed unchanged via a September 17, 2026 Instagram post plus 2 recruiting platforms. |
| `southwestern_illinois` | Own roster page still titled 2025 season; NJCAA class years blank | **Refreshed.** Own roster page now titled "2026 Season" with full positions and class years (20 players), matching the school's own active fall 2026 schedule. `mf_total` 10→7, `cleared_before_2027` 10→6, trajectory Yr1 65→59/Yr2 78→72, `fitOlivier` 42→40. Coach (Lindsay Eversmeyer) independently confirmed unchanged via a "3 weeks ago" Facebook post plus SportsRecruits/LinkedIn.

**Lesson for next time:** a Presto-platform site returning a "Let's confirm you are human" page is a genuine CAPTCHA, not the milder Cloudflare 202/302 challenge documented elsewhere in this file — do not attempt to click through it (§8 forbids bypassing bot-detection). NJCAA's own team roster page remains the correct fallback and, unlike in mid-September, now has real class years for some — though not all — of these schools; always re-check rather than assuming the September finding still holds.
...
**✅ RESOLVED v45.47 — rendered-field jargon leak, recurrence #3.** All 40 rendered "this session" hits were rewritten in plain language (the 41st is `coaches.json` `overallScoreNote`, which is internal-only). Widening `check_no_jargon.py` to scan **every** rendered string found 38 more leaks the old checker never read: backtick-quoted field names (`tuition`/`fees`/`roomBoard`) in 26 `fin.internationalNote` strings (they render in the Financial Model tab), `ACU_UNIT_META` in 7 `acuAlignNote` strings, "browser MCP" in 2 `trajectoryNote` strings, and "earlier research pass" in 3 `recruit_pathway_note` strings. All were fixed, and only string values changed (verified leaf-by-leaf against HEAD). The checker failed with 81 findings on the pre-fix data and passes after. Stale-but-false claims that dropping "this session" would have left behind were deleted rather than kept: the `southeastern_cc_ia` and `coastal_bend_cc` `rec` sentences saying no roster data was available (both now have 2026-27 rosters). Suffolk and Westchester now say plainly that the official roster publishes no positions.

**✅ RESOLVED v45.48 — research-batch/campaign comparisons.** 255 rendered sentences said "this batch"/"this campaign" (the ~180 estimate was low), plus 97 more in variant wording ("this entire campaign", "the batch's", "Batch 2", "audit-discovered", "already-guide") and 45 cost notes describing the guide's own past estimate errors ("the previous stored $X was an unresearched ballpark"). All rewritten or removed; 325 strings changed, plus the JUCO intro in `js/app.js`; no number or key changed. **Rule used, keep it:** a batch-scoped superlative was re-scoped only when the batch equals a group a reader can see and that group is unchanged in the guide (the five Chicago schools, the four USC campuses, the five Wyoming schools, the eight Region 18 schools). Otherwise it became a non-superlative ("one of the lowest costs in the guide"), and every such cost claim was checked against the whole guide first: several "cheapest in this campaign" claims were false guide-wide (Northeast Texas CC is 5th cheapest; Paris JC's tuition is 18th among JUCOs; Pacific NW Christian, not Jacksonville, is the smallest school; Northwestern is 5th most expensive, not 1st). Malcolm X text still said Jesse Rosen "has led the program since 2014"; corrected to his May 2026 departure and the vacancy. The same wording in the `js/app.js` JUCO section intro was fixed too; `check_no_jargon.py` reads data only, so UI copy still needs a manual sweep. The batch/campaign patterns are now FAIL, not WARN.

**✅ RESOLVED v45.49 — "undefined" in the Vermont and Paris JC modals.**
- `vermont`: `facilityDetails.extras` and `note` written from UVM's official facility pages (indoor turf 2011, Athletic Performance Center 2005, Student-Athlete Success Center, ESPN SportsCenter August 2025). The stored `stadium` text also wrongly said **natural grass**; UVM's own Virtue Field page says FieldTurf synthetic turf, so that was corrected.
- `paris_jc`: re-verified in Chrome. Jace Starling is confirmed on Houston Christian's roster (prior school Paris JC 2023-24). Jonathan Chairez was announced by Paris JC (June 2026 release) but is on neither Missouri State's 2026 roster nor its 14-player signing class. Under §5b, one confirmed transfer is not a rate, so owner ruling (2026-09-16): `perYear` 1.0 → `null` (neutral), `d1Count: 1`, `yearsCovered: "2025-2026"`, `years: 2` stored for audit. **Score moved:** `fitOlivier` 53 → 56, `lensScores.overall` 53 → 56, `soccer` 40 → 45, `value` 62 → 64. The v44.81 state-snapshot line calling Paris JC's rate "a real measured 2-year D1 transfer rate (1.0/yr)" is superseded by this. **Re-check Chairez on Missouri State's roster** at the January 2027 portal recheck; if he appears, Paris JC has 2 confirmed transfers and qualifies for a measured rate again (Phoenix precedent, excluded from the divisor).
- `js/app.js` now hides the Extras block and the Facilities verdict when those fields are missing, and shows "D1 transfer rate" instead of "undefined … over undefined" when the `nextLevel` counts are missing. **This hid, but did not fill, the same gap at three more schools:** `akron`, `gcu` and `denver` have no `facilityDetails.extras` (they do have a verdict). My v45.48 all-modal scan missed them because it read each modal after only 60 ms; the v45.49 scan waits until the modal shows the right school's name.

**✅ RESOLVED v45.50 — Akron/GCU/Denver Extras, internal stamps, and prose that contradicted stored rosters, coaches and counts.** Full list in CHANGELOG v45.50. Durable lessons:
- **Prose drifts from data after every refresh, and no validator reads prose against data.** 35 `trajectoryNote`s and ~20 `rec`/`olivierMatch`/`facilities[]` strings still quoted an older roster (e.g. Harford "entire 10-player midfield clears" vs stored 2 of 5); 3 schools' texts named a departed or interim-stale coach (Connors State "vacant", NEO A&M "interim", NOC-Enid Bassoff, Illinois Central Carreno); conference texts quoted stale counts ("All 14 Big Ten schools", "13 guide schools in the AAC", "53 guide schools"). `check_no_jargon.py` checks wording only. **After any roster refresh or coach change, sweep every text for that school's name, midfielder counts and coach name, including `conferences.json`, `conf-prestige.json` and the `js/app.js` section intros.**
- **A stored JUCO roster can be internally impossible:** `usc_salkehatchie` stored 4 of 4 midfielders clearing while 3 were listed as freshmen on a 2026-27 roster. Re-read live: the real 2026-27 roster has 13 midfielders (2 So., 11 Fr.). Refreshed via `refresh_school.py`: fit 50 → 47, minutes 61 → 51, value 48 → 46.

**🚩 Found v45.50, not fixed — conflicting coach tenure facts (not guessed):**
- `usc_salkehatchie` / William Glass: `coaches.json` says 16th season at Salkehatchie; his official page (uscsalkathletics.com/william-glass) says he is "entering his 19th season". School texts now avoid a season count. Resolve `yearsHC`/`record` against a dated release before re-scoring.
- `noc_enid` / Aron Bassoff: school texts called 2025-26 his debut season, but `coaches.json` records 19-27-5 over 3 seasons. School texts now avoid the tenure claim. Confirm Bassoff's start date on NOC's own releases.
- `monroe_college`: `recruit_pathway_note` says 13 MFs on the 2025 roster but `mf_total` is 11. Left alone per the owner's "leave Monroe" ruling.

**🚩 Found v45.47, not fixed — related leaks the checker cannot fail on yet:**
- **"Verified v38 via…"** version stamps in `confRecord[].note` and `housing.note` (e.g. `daytona_state`, `efsc`, `pba`, `georgian_court`) are internal version labels. No pattern covers them yet.
- **Stale facts next to the edited text:** `southeastern_cc_ia` `rec` still calls Henrique Vieira an interim head coach (confirmed permanent v44.91). The `trajectoryNote` for `southeastern_cc_ia` and `lsu_eunice` still describe 2025-26 roster counts (17 and 5 MFs) although both schools are on 2026-27 rosters (18 and 8). Fix these the next time either school is refreshed.

**📌 Harcum → other JUCO moves — CONFIRMED 2026-09-16:** Shoma Komata → Barton CC and Zian Hamou-Maamar → Garden City CC, both JUCO-to-JUCO, no scoring relevance. **Evidence:**
- Harford CC's official box score vs Harcum, 9/20/2025 (`harfordathletics.com/sports/mens-soccer/stats/2025/harcum-college/boxscore/7314`), lists Harcum #11 Zian Hamou-Maamar (C, starter) and #5 Shoma Komata (C, sub), alongside Harcum's Akihiro Fujikawa and Titouan Blot, who are still on Harcum in 2026-27.
- Both players are on the 2026-27 rosters at Barton (#32, MF, So., Tokyo) and Garden City (So., Versailles), and absent from those schools' 2025-26 rosters.
...
**Why now:** every school below was last checked between Aug 6 and Aug 17, before the fall season began, and most had published no 2026-27 roster at that point. By mid-September, teams have played several games. §15 names Sept–Nov as the best scraping window, so rosters should now be populated. **Target: finish by end of October**, well before the fall transfer-portal window (~Nov 24 – Dec 24) makes rosters move again.

**Scope:** 38 schools (34 JUCO, 1 D1, 2 D2, 1 D3). Re-derive the list before starting; do not trust this count:
`python -c` over `data/*.json` for `minutesOutlook.available == true and roster_season != "2026-27"`. The 12 `available:false` schools are **not** in scope (see "Not in scope" below).

**Order of sub-batches** (one commit and one CHANGELOG entry per sub-batch, the same rhythm as Batch 5):

| Sub-batch | Schools | Notes |
|---|---|---|
| **7A — 4-year schools** | `clemson` (D1), `nova`, `csula` (D2), `chapman` (D3) | Done first: **full 4-year cascade**, and Clemson carries the highest fit (67). No NJCAA fallback exists for these. If a school still has no 2026-27 roster, it stays deferred. **✅ v45.37:** all 4 refreshed to 2026-27 (fit: Clemson 67→60, Nova 57→64, Cal State LA 59→55, Chapman 45→48); 3 coach titles, 1 email, 1 yearsHC corrected. |
| **7B — Region 2 (OK/AR)** | `rose_state`, `neo_am`, `national_park`, `connors_state` | Rose State has the highest JUCO fit in this batch (68). **✅ v45.38:** all 4 refreshed from their own sites and cross-checked on NJCAA (fit: Rose State 68→63, NEO A&M 59→57, National Park 59→58, Connors State 53→53); all 4 coaches confirmed, no edits. |
| **7C — Region 5/14 (TX)** | `blinn_college`, `hill_college`, `jacksonville_college`, `texas_southmost`, `ranger_college` | NJCAA already showed Blinn's 2026-27 roster; **still start on Blinn's own site** (§15). **✅ v45.39:** all 5 refreshed from their own sites and cross-checked on NJCAA (fit: Blinn 63→57, Hill 61→60, Jacksonville 61→52, Texas Southmost 60→56, Ranger 59→55); all 5 coaches confirmed, no edits. |
| **7D — Region 1/8 + Santa Monica** | `efsc`, `phoenix_college`, `smc` | `efsc` published 2026-27 with **no positions** (Aug), so if its own page still lacks them, this is a legitimate NJCAA fallback case. `phoenix_college` is NJCAA **DII** (use `div2` in the NJCAA URL). `smc` is CCCAA, so it has **no NJCAA fallback**. **✅ v45.40:** all 3 refreshed from their own sites (fit: EFSC 65→60, Phoenix 62→57, Santa Monica 55→51); EFSC and Phoenix, both flagged in August, are now fully published; Phoenix coach title corrected. |
| **7E — Region 4 (Chicago)** | `truman_college`, `daley_college`, `kennedy_king_college`, `wilbur_wright_college` | The "school's own site" here is `region4sports.com` (Lineup tab, §15 layout table). **Never open `citycollegesofchicagoathletics.com` — it is compromised** (§6D). **✅ v45.41:** Daley refreshed from the NJCAA roster (fit 49→45); Truman (no class years), Kennedy-King (1 GK) and Wright (no roster) kept on 2025-26; coach check not possible (no current staff page). |
| **7F — Region 6 (KS)** | `seward_county_cc`, `garden_city_cc`, `dodge_city_cc` | Garden City and Seward had empty 2026-27 pages on Aug 17. Seward has the largest MF group in the batch (20). **✅ v45.42:** all 3 refreshed from their own sites and cross-checked on NJCAA (fit: Seward 52→45, Garden City 50→47, Dodge City 48→44); Garden City's no-forwards labelling disclosed; Dodge City coach title corrected. |
| **7G — Region 9/18 (West)** | `otero_college`, `northwest_college`, `trinidad_state`, `slcc`, `north_idaho_college`, `pacific_northwest_christian_college` | Watch the redshirt class-year trap at `slcc` (§15). **✅ v45.43:** 4 refreshed (fit: Otero 52→48, Northwest 50→44, Salt Lake CC 48→47, North Idaho 47→47); Trinidad State and Pacific NW Christian kept on 2025-26 (both sources incomplete); 2 coach titles corrected, PNWCC contact filled. |
| **7H — East (NY/PA/MD/SC)** | `monroe_college`, `nassau_cc`, `ulster_cc`, `harcum_college`, `montgomery_college`, `hagerstown_cc`, `usc_union` | `monroe_college` is mid-transition to NCAA D2 (§6F) and plays a CACC schedule, so its NJCAA page may not exist. `harcum_college`'s stored `url` is a `harcum.edu` CMS page, so find the real athletics roster. `nassau_cc`/`ulster_cc` are NJCAA **DIII** (`div3`). **✅ v45.44:** 5 refreshed (fit: Nassau 42→39, Montgomery 47→43, Hagerstown 40→36, USC Union 55→51, Ulster 34→32; Hagerstown merged sources, Ulster from NJCAA); Monroe (owner: leave) and Harcum (half-published, no NJCAA roster) kept on 2025-26; Nassau coach title corrected. |
| **7I — Region 24 (IL)** | `southwestern_illinois`, `lincoln_trail` | Small closing batch. **✅ v45.45:** Lincoln Trail refreshed from its own site (fit 47→43); Southwestern Illinois kept on 2025-26 (roster page still on 2025, NJCAA has no class years). Batch 7 complete: 30 of 38 refreshed. |

**Per school, in this order, no shortcuts:**
1. **Open the school's own roster page first**, in Claude-in-Chrome. Pick the 2026-27 season from the page's own selector and wait ~5s before reading. Never start from NJCAA (§15).
2. **Sanity-check the roster before trusting it:** squad size vs the stored prior season; goalkeeper count (0–1 means half-published); midfielder share (far above ~30% is suspect); class-year regex traps (§15).
3. **One of three outcomes, recorded explicitly:**
   - **(a) Populated 2026-27** → refresh with `refresh_school.py`, including `full_roster`. JUCO rule: `So.` clears, `Fr.` returns.
   - **(b) No 2026-27 roster, or published but empty** → leave the stored data alone. Record the date and exactly what the page showed.
   - **(c) School page down, blocked, or empty** → **JUCOs only**: the NJCAA stats roster may be used as the source, provided its GP > 0. Say so in plain language in `trajectoryNote`.
4. **NJCAA cross-check** (JUCOs, outcome a): compare squad count, names, positions and class years. Record match or differences in the CHANGELOG. Remember a match is not independent for schools on the NJCAA platform (§15).
5. **Coach spot-check** on the school's official coaches page (§3a Type 3). If none exists, say so; never mark it done.
6. **Rewrite `trajectoryNote` and `recruit_pathway_note` by hand** in plain language. The script does not do it.

**Per sub-batch:** `check_roster_arithmetic.py` → `check_juco_trajectory.py` → `check_no_jargon.py` → `check_roster_snapshot.py` → qa-suite (`Issues: 0`) → local browser check (Minutes Outlook card shows ✓ 2026-27, and Explore sort position looks right) → CHANGELOG entry + `guideVersion` bump → commit, push → verify live data. **Then update this table's row with the result.**

**Network (settled 2026-09-14):** Claude-in-Chrome uses the owner's own network. `njcaa.org` and any school on the NJCAA site platform (e.g. `apacheathletics.com`) geo-block Australia, so they need the owner's **VPN on**. Some other hosts (Suffolk, `goreivers.com`) instead block datacenter IPs and work only on the owner's normal connection. **When a CloudFront 403 appears, stop and ask the owner to switch VPN state, then retry once with a cache-busting query string.** Never record "roster unavailable" from a 403 (§15).

**Afterwards (still applies):** the 4-year schools refreshed here get rechecked in the January 2027 portal recheck (the cloud reminder already exists). Schools left on outcome (b) at the end of October get one final look in November, then are recorded as "no 2026-27 roster published" for the season.
...
- **✅ ACCESS METHOD CONFIRMED 2026-08-20, CORRECTED SAME DAY — `tyler_jc`'s athletics host (`apacheathletics.com`) 403s direct/residential connections with a CloudFront block page, but a VPN reliably reaches it.** Distinct from the datacenter-IP CloudFront block seen elsewhere in this project (Suffolk, `goreivers.com`) — those block *hosting-provider* IPs specifically and a normal residential connection sails through; this one 403s the owner's own normal residential connection too, and only clears with a VPN, which points to a geo/traffic-pattern WAF rule rather than a datacenter-IP rule. **A second gotcha on top of the VPN requirement: CloudFront caches the 403 error page itself** — the very next request after connecting the VPN returned the byte-identical cached error (same `Request ID`), and only cleared once a cache-busting query string (`?_=1`) forced a fresh request. **CONFIRMED GEO-BLOCK 2026-09-14, and it covers `njcaa.org` too.** The owner tested from two unrelated Australian networks, home broadband and a 5G mobile connection. Both returned the CloudFront 403 on every page, including the `njcaa.org` homepage and `robots.txt`, while the VPN worked. Two separate IPs failing identically rules out an IP-reputation or rate-limit block on one address: **the platform refuses Australian traffic (possibly all non-US traffic), and waiting will not lift it.** `njcaa.org` and `apacheathletics.com` run on the same platform (identical team IDs in their URLs), so they block together. **Claude-in-Chrome drives the owner's own Chrome, so it inherits whatever network the owner is on.** A page loading for Claude only means the owner's VPN happened to be on; it proves nothing about direct access. This is how a v45.36-era note came to wrongly claim a cache-busting string got through "with no VPN". **Standing procedure (replaces the 2026-08-20 one): do not try a direct connection. Ask the owner to turn the VPN on before opening either site. If the first request after connecting still shows the error page, that is CloudFront's cached 403; append one throwaway query string (`?_=1` or `&_=1`) and retry once. A cache-bust only clears a cached error. It never gets past the geo-block itself.**
...
**🚩 FLAGGED, owner said "leave for now" (2026-09-15) — mid-year (spring-semester) arrivals are not covered by this schedule.** The Jan 5, 2027 re-check runs before spring-semester rosters are usually posted. Fall-window transfers and early-enrolling recruits typically join in **mid-January**, and spring training/exhibition play runs roughly **Feb–April**. Junior college sophomores can also leave in January (NJCAA has no portal windows). Nothing is scheduled between Jan 5 and May 20, so a January arrival or departure would go unseen for about four months, and it is extra competition that already has a spring of training by August 2027. **Proposed, not done:** (1) move the Jan 5 cloud reminder (`trig_01E3WYpZjbTGgLEWcpCsoDzg`) to about **Feb 20, 2027**; (2) note the mid-year reason in the table above; (3) add a coach question: "Do you bring in mid-year players, and would you consider a January start?" **Also unverified:** a search result says the NCAA approved a fall-and-spring competitive season for DI men's soccer. If that applies from 2027-28, spring games would count toward minutes and the Minutes Outlook projection model would need revisiting. Confirm on ncaa.org (Claude-in-Chrome) before acting. Spring-season practice described here is general knowledge, not yet verified against an NCAA source.
...
- **🚨 `citycollegesofchicagoathletics.com` is now COMPROMISED, not merely dead — found v45.30, do not navigate to it in any future session.** Prior sessions (§C, the v44.74 batch) documented this as "a parked GoDaddy lander on every path." As of v45.30 it instead redirects to an unrelated gambling/spam site ("337SPORTS: Bandar Akaitaly Situs Slot Gacor..."). This is the primary athletics domain referenced (though not the live `DOMAINS[]`/`domain` fields, which already point to `region4sports.com`/`ccc.edu` per the entry above) for all 5 City Colleges of Chicago JUCOs — `daley_college`, `kennedy_king_college`, `malcolm_x_college`, `truman_college`, `wilbur_wright_college`. **Use `region4sports.com` (team pages) or `ccc.edu`/`directory.ccc.edu` (staff directory) instead** — both worked cleanly throughout the v45.30 coach audit. This also means any future coach-verification pass on these 5 schools cannot rely on Bing/Google search snippets that surface this domain — check the live page, don't trust a cached snippet, since the domain itself is no longer safe to open.
- **🚩 A second, SINGULAR-form domain — `citycollegeofchicagoathletics.com`, no "s" after "college" — surfaced v45.71 and is NOT the same host as the compromised plural-form one above.** The owner passed a link on this domain claiming to show Malcolm X's coaching staff; an automated safety classifier blocked navigation to it before it could be read, flagging it as a possible risk, so its content was never confirmed either way — not proven malicious, not proven safe. **Do not assume it is fine just because it differs from the already-documented compromised domain by one letter** — that's exactly the shape a typosquat would take. The fact it was actually named — Aziz Tahir — was independently confirmed instead via three unrelated recruiting platforms (NCSA, SportsRecruits, ProductiveRecruit) plus `directory.ccc.edu`, none of which required opening this domain. If a future session needs this domain specifically, flag it to the owner rather than trying to work around a safety block.
- **📌 `.nojekyll` was investigated and REJECTED (v45.35) — do not re-propose it.** Raised after two misbehaving Pages builds, then measured and found to solve nothing here. (1) **Jekyll did not cause either failure** — the *same commit* errored with `duration: 0ms` then built cleanly in 35s on retry, and a content failure is deterministic. (2) **Jekyll is not processing this repo's content anyway** — `CHANGELOG.md` is served as raw `text/markdown` with its first line intact, i.e. copied verbatim, because files without YAML front matter are static in Jekyll; so there is no Liquid-parse failure class to remove. The only remaining claim was "builds might be faster than 37–41s", never measured, against newly serving dotfiles and `_`-prefixed files from the Pages domain. **Exposure is a non-issue either way: the repo is PUBLIC** — `.claude/` already returns 200 at `raw.githubusercontent.com`, and `CLAUDE.md`/`CHANGELOG.md` were serving 200 from the Pages site until the exclude below.
- **📌 `_config.yml` keeps CLAUDE.md and CHANGELOG.md off the PUBLISHED SITE only (owner decision, 2026-09-12).** `bustachat.github.io/olivier-guide/CLAUDE.md` now returns **404**; the site itself is unaffected (verified live: v45.35, 170 schools, 170 cards, coaches resolving, all 6 lens pills). **This does NOT make them private** — the repo is public, so both remain readable at `raw.githubusercontent.com` (still 200). Only a private repo changes that, and Pages on a private repo needs a paid plan; the owner reviewed that and chose the exclude alone. Don't re-open it as though it were an oversight. To undo: delete `_config.yml` and push.
...
- **✅ RESOLVED v45.13 — the modal logo fallback chain only ever tried `DOMAINS[u.id]` (the athletics domain), never the school's own `domain` field, and Google's favicon proxy silently masked the failure.** Owner-reported: UCLA's modal showed a generic placeholder globe. Root cause: `uclabruins.com/favicon.ico` 404s, and the old chain's only fallback — Google's `s2/favicons` proxy for that same broken domain — returns HTTP 404 **with a real, byte-identical ~726B generic-globe PNG in the body anyway**. A browser `<img>` only checks whether valid bytes came back, not the status code, so `onerror` never fires a second time and the chain "succeeds" showing a meaningless placeholder. `buildModalHeader()`/`handleModalLogoError()` now also try the school's own `domain` (direct, then via Google's proxy for THAT domain) before giving up to text initials — this fixed `ucla`, `smc`, `northwestern`, `william_mary`, `ucsb`, `calpoly` for free (their `.edu` domain already had a working icon the modal never tried). `virginia` needed a 7th `ICON_OVERRIDES` entry (Google's proxy URL for `virginia.edu` directly) because the bare domain's TLS cert is registered to a different hostname and hangs rather than failing promptly. **15 schools remain genuinely broken** — no distinct domain to fall back to anywhere in the data (`DOMAINS[]` === `domain`) — see the open item below; do not assume this fix covers them.
- **✅ RESOLVED v45.71 — all 15 remaining broken-icon schools given real local logos via `ICON_OVERRIDES`.** `glendale_cc_az` (Gauchos), `neosho_county_cc` (Panthers), `iowa_lakes_cc` (Lakers), `blinn_college` (Buccaneers), `coastal_bend_cc` (Cougars), `angelina_college` (Roadrunners), `eastern_arizona` (Gila Monsters), `central_wyoming` (Rustlers), `laramie_county_cc` (Golden Eagles), `western_nebraska_cc` (Cougars), `trinidad_state` (Trojans), `jefferson_college_mo` (Vikings), `harcum_college` (Bears), `central_georgia_tech` (Titans), `noc_enid` (Jets). **Root cause confirmed for most: not a missing icon, but the whole domain sitting behind Cloudflare bot-protection** — a cookie-less `<img>` request (exactly what a real visitor's browser sends) gets a 202/302 challenge page instead of the icon, even though the icon itself is real and distinct per school (verified by hash — these are NOT the shared/generic Sidearm template). Live-fetched instead via 3 methods depending on what each host allowed: (1) most schools' own `/assets/favicons/favicon-*.png` or `apple-touch-icon.png` resolved through their real CDN once fetched with a real browser session or `curl -sL` (the underlying asset lives on an open CloudFront distribution one redirect away — `curl -L` alone was enough for 6 of them); (2) several schools' logos were found pre-labelled with alt text on a same-conference rival's site (`cdn.prestosports.com/action/cdn/logos/id/*.png` conference-standings widgets — `western_nebraska_cc`, `laramie_county_cc`, `trinidad_state`, `eastern_arizona` all found this way, confirmed by the widget's own alt-text naming the school); (3) `central_georgia_tech`, `harcum_college`, `noc_enid`, `coastal_bend_cc` had no usable school-site asset at all, sourced instead from each program's own official social account (X/Twitter profile photo or a clearly-attributed Facebook/X post), Tier-1 cross-checked against ≥2 independent sources appearing in the same search. All 15 assets stored under `assets/logos/` and confirmed loading (`complete:true`, correct `naturalWidth`) on both the Explore card and the Details modal via the shared `ICON_OVERRIDES` path — no `validate_schools.py`/`validate_consistency.js` regressions (`Issues: 0` unchanged). **Tooling note for next time:** a JS-triggered `fetch()+blob()+anchor.click()` download only lands reliably ONCE per tab — a second download in the same tab silently no-ops (transient user-activation expires after the first `await`). Open a fresh tab (`tabs_create_mcp`) per download rather than debugging why "it worked before." Also: Google Images results can have 2-3 DOM `<img>` elements sharing identical `alt` text at different scroll positions (grid thumbnail + enlarged preview + a duplicate) — matching by `alt` string alone can silently fetch the wrong one; always verify the downloaded file's actual pixel content (`Read` the PNG) before trusting it, never trust the fetch call's reported byte size alone.
...
- **✅ OWNER RULING 2026-09-17 — Army & Navy `lensScores.value`: don't change.** Option (a) stands: both keep their hand-set values (navy 47, army 45, about fit+3) instead of the formula's 66/65, because the free tuition is paid for with a 5-year service commitment. The `VALUE` check keeps its `costNum === 0` exemption. Do not raise this again unless the owner reopens it.
...
- **✅ RESOLVED v45.53 — NJCAA division strength split using Massey ratings (owner-approved 2026-09-17).** I 0.6 / II 0.55 / III 0.42 via the new `njcaaDivision` field; see §5 and CHANGELOG v45.53. Five scores moved by 1 point (Neosho County and the four Division III schools).
- **✅ RESOLVED v45.53 — Elite JUCO tiering bar.** Owner chose the written rule limited to the last 3 seasons (§5). 39 → 15 Elite. **Open:** (1) the top-5 final-poll route cannot be checked (njcaa.org rankings page says rankings are unavailable; United Soccer Coaches shows only the current season), so Barton, Arizona Western and Southeastern were not credited for October 2025 rankings; (2) njcaa.org's 2023-24 and 2024-25 All-America pages did not load, so a school qualifying only on 2023 or 2024 All-Americans may be missing; (3) re-run the rule after the 2026 season, which drops 2023 (Monroe, Tyler, Iowa Lakes and Phoenix's runner-up finish) from the window.
...
- **📌 `mlsPicks5yr` = picks in the five most recent MLS SuperDrafts, attributed to the college on MLS's own tracker (2022–2026 as of v45.55).** Recount from mlssoccer.com's draft tracker / "every team's selections" page after each December draft, check each year's total against the official pick count, then re-store `fitOlivier`/`overall`/`soccer`/`value` and the Pro Pipeline table (`MLS-TABLE` check). The v45.55 recount found 48 of 170 stale, some by 13 picks, and 47 Fit Scores moved. Draft years use MLS's naming: the "2026 SuperDraft" was held in December 2025.
...
- **🚩 ACU rubric review, 2026-09-23 — owner asked to recheck the ACU course against the live acu.edu.au page and ACU's official 2027 Handbook, since ACU rebuilt the Bachelor of Exercise and Sports Science for 2027 entry.** Confirmed live (acu.edu.au + `acu.edu.au/handbook/Handbook-2027/...`, RULE 0): the course is a genuine ground-up redesign — 240cp, 20 shared core units, plus a new structural choice of major (Sports Performance or Exercise Rehabilitation) that didn't exist before, with roughly 29 real exercise-science unit codes across the core/professional-experience/major schedules (full list and codes in the session transcript, not reproduced here). Checked this guide's 16-unit `acuUnits[]` rubric (§5, `ACU_UNIT_META` in `js/app.js`) against it: **7 of 16 codes match a real current ACU unit exactly or near-exactly** (EXSC222, EXSC321, EXSC216, EXSC187, EXSC230, EXSC322, EXSC394 — though EXSC394's real content shifted to a rehab-major elective); **9 of 16 do not correspond to any real current ACU code** (`ANAT100`, `BIOL125`, `EXSC225`, `EXSC224`, `EXSC204`, `EXSC199`, `EXSC296`, `EXSC122`, `EXSC398`). None of this touches `fitOlivier` — ACU alignment has been informational-only since v37.1 — and no school's `acuAlign`/`lensScores.academic`/`acuUnits[].covered` value needs to change, since those were always scored by matching real US-school content against each bucket's *description*, not by literally checking for a matching ACU transcript code. **Owner ruling (2026-09-23): do not rebuild the 16-unit rubric or re-audit the 174 schools now** — fix only the citations that were visibly wrong. Fixed same day: (1) this Athlete Context line (§13) corrected in place; (2) the Glossary's WES entry in `index.html`, which made the identical wrong claim to real visitors; (3) 43 schools' `acuAlignNote` text (`northeast_cc`, `monroe_college`, `indian_hills`, `tyler_jc`, `daytona_state`, `barton_cc`, `cowley_cc`, `arizona_western`, `efsc`, `murray_state_ok`, `eastern_oklahoma_state`, `connors_state`, `neo_am`, `rose_state`, `national_park`, `rich_mountain`, `hill_college`, `western_texas`, `truman_college`, `daley_college`, `malcolm_x_college`, `kennedy_king_college`, `wilbur_wright_college`, `coffeyville_cc`, `garden_city_cc`, `seward_county_cc`, `paris_jc`, `jacksonville_college`, `northeast_texas_cc`, `texas_southmost`, `jefferson_college_mo`, `harcum_college`, `hagerstown_cc`, `harford_cc`, `slcc`, `snow_college`, `north_idaho_college`, `college_of_southern_idaho`, `usu_eastern`, `truckee_meadows_cc`, `pba`, `lynn`, `gcu`), which cited one of the 9 fake codes directly to the reader (e.g. "Exercise Physiology coursework → EXSC225") — the 5 codes with a clean real replacement were swapped in (`ANAT100`→`EXSC142`, `EXSC225`→`EXSC126`, `EXSC224`→`EXSC120`, `EXSC204`→`EXSC233`, `EXSC398`→`EXSC388`/`EXSC389`); the 4 with no clean current equivalent (`BIOL125`, `EXSC122`, `EXSC199`, `EXSC296` — ACU folded the latter two into one real unit, `EXSC249`) had the fake code dropped and the existing plain-English content description kept, rather than inventing a new citation. Verified: only `acuAlignNote` changed in the diff (43 line-pairs across `data/juco.json`/`d2.json`/`d1-other.json`); every school's `acuAlign`, `acuUnits[]`, `lensScores`, and `fitOlivier` confirmed byte-identical to before against `HEAD`; `validate_schools.py`, `validate_consistency.js` (Issues: 0) and `check_no_jargon.py` all pass; CRLF line endings intact.
  ...
  **Third pass, same day — owner asked to re-verify the other 43 schools' `acuAlign` scores are accurate.** Read all 43 notes' course-to-unit mappings against each school's own stored `acuUnits[]` data: 41 internally consistent, 1 clear contradiction (`gcu`), and 2 more with a pattern worth checking (`tyler_jc`, `usu_eastern` — both cite a course literally titled "Biomechanics" toward the wrong bucket). Owner: resolve just these 3 against each school's real, current course catalog (RULE 0) — not the other 40. Findings:
  - **`gcu`** — the note was right, the data was wrong. Two units the note describes as covered (`EXSC187` Growth/Motor Development, `EXSC230` Motor Control & Learning) were stored `covered:false`, and one the note gives no support for (`EXSC388/EXSC389`, Professional Experience) was stored `true`. Data corrected to match the note. `acuAlign` 14→15, `lensScores.academic` 89→95.
  - **`usu_eastern`** — a clean course-label swap. USU's real catalog: KIN 3250 "Anatomical Kinesiology" ("study of the anatomical bases of human movement") is the lower-division course and a listed prerequisite for KIN 4200; KIN 4200 "Biomechanics" ("biomechanical principles fundamental to efficient human movement") is upper-division and a direct name match for this guide's `EXSC321` unit ("Biomechanics"). The note had it backwards — KIN 4200 was credited to `EXSC120` (the lower-division Mechanical Bases bucket) while `EXSC321` sat uncredited. Swapped: `EXSC222` (previously credited via KIN 3250) → false, `EXSC321` (now credited via KIN 4200) → true. Same total count (7), so `acuAlign`/`lensScores.academic`/`fitOlivier` unchanged — pure accuracy fix.
  - **`tyler_jc`** — bigger than the other two. The note is scoped to the Strength & Conditioning track, but its real 2026-27 catalog doesn't support 2 of the 5 currently-credited units — `EXSC322` "Exercise Physiology: Adaptation" (no upper-division exercise-physiology course exists in this 2-year AS track) and `EXSC120` "Mechanical Bases of Exercise" (credited via KINE 1301, which isn't even in the S&C track — it's General-track-only — and whose real content is a historical/career-overview survey, not mechanics) — while 2 real S&C-track courses went uncredited: PHED 1350 "Fundamentals of Strength and Conditioning" (a direct match for `EXSC216`, Resistance Training) and PHED 2358 "Psychological Aspects of Human Performance" (a direct match for the Psychology-of-Sport bucket). The old note also mislabelled `EXSC322` as "Biomechanics" in one clause — it isn't; `EXSC321` is, and TJC's AS track has no course reaching that level either. Owner approved the corrected mapping. `acuAlign` 5→4, `lensScores.academic` 42→36; `fitOlivier` unaffected (doesn't read `acuAlign`).

  All three validated (`validate_schools.py`, `validate_consistency.js` — Issues: 0); only `acuAlignNote`/`acuUnits[].covered`/`acuAlign`/`lensScores.academic` changed per school, confirmed via diff against the version before this pass. CRLF intact. **The other 40 schools were deliberately NOT re-researched** — this was a bounded, owner-scoped spot-check, not a full re-audit of all 43.

  **Fourth pass, same day — owner asked why the other 40 weren't re-verified, and whether they're actually fine.** Answer: "internally consistent" (the third pass above) only confirmed the note and the stored `covered` flags agree with each other — it does not confirm either is factually correct against the school's real catalog. That gap is not hypothetical here: the earlier fabrication-audit campaign found 11 schools where a fabricated note and fabricated data agreed with each other perfectly, which a self-consistency check would have missed entirely. Owner asked for a spot-check of 5–8 of the 40 to get a rough sense of the error rate. **Result: 7 checked (RULE 0, live catalog per school), 3 of 6 independently-checked (50%) had real errors** — see CHANGELOG.md v45.69 for full detail. Fixed same day: `pba` (`acuAlign` 14→10 — the guide's highest count, overstated by 4 units that had no required-course support), `harcum_college` (`acuAlign` 7→6 — one A&P sequence had been credited to 3 separate buckets), `monroe_college` (`acuAlign` 5→6 — a unit marked `true` with zero supporting evidence anywhere, plus 2 real required courses that weren't credited). `rose_state`, `northeast_texas_cc`, and `lynn` all came back clean — every cited course matched the real, live catalog exactly.

  **✅ RESOLVED, same day — all 34 remaining schools checked.** Live-verified 19 (`northeast_cc`, `daytona_state`, `barton_cc`, `cowley_cc`, `arizona_western`, `efsc`, `murray_state_ok`, `coffeyville_cc`, `garden_city_cc`, `jacksonville_college`, `jefferson_college_mo`, `connors_state`, `neo_am`, `eastern_oklahoma_state`, `snow_college`, `north_idaho_college`, `harford_cc`, `slcc`, `hagerstown_cc`); mechanically re-checked the remaining 9 (`national_park`, `rich_mountain`, `hill_college`, `western_texas`, `seward_county_cc`, `paris_jc`, `texas_southmost`, `college_of_southern_idaho`, `truckee_meadows_cc`) against the same two failure patterns using only their existing, already-detailed notes — all 9 already applied the standard correctly (real required courses, no double-counting, electives correctly excluded), so no live re-check was needed. The 5 City Colleges of Chicago schools (`truman_college`, `daley_college`, `malcolm_x_college`, `kennedy_king_college`, `wilbur_wright_college`) were left as-is, resting on the v44.74 batch's own documented `catalog.ccc.edu` verification. `indian_hills` was already resolved earlier the same session via an owner-supplied catalog citation.

  **Result: 18 of 19 live-checked schools needed a correction** (only `murray_state_ok` came back clean). Two failure patterns dominated: (1) **generic requirement slots credited as if guaranteed** — `barton_cc` (`acuAlign` 9→0) and `efsc` (5→0) turned out to be flexible Liberal-Studies-style degrees with almost no named required courses at all; `daytona_state` (4→0) had "Health & Wellness Track" mistaken for a real major when it's just one of several generic advising labels on an ordinary A.A.; `northeast_cc`/`cowley_cc`/`arizona_western` (5/5/8 → 2/2/2) each had 2-3 units resting on a broad "Introduction to X" survey course or an unconfirmed elective, now excluded on the same standard. (2) **a course genuinely required, but credited to the wrong bucket or not credited at all** — `harford_cc` (8→6) and `slcc` (7→4) both had elective-pool credits removed but kept their real required-course credits; `hagerstown_cc` (4→5) gained a unit its own note had overlooked (a required science sequence that guarantees real A&P content either way); `coffeyville_cc` (3→2) lost a clinical-practicum credit that turned out to sit in a secondary "Additional Courses" list, not the primary recommended one. `connors_state`, `garden_city_cc`, `jacksonville_college`, `jefferson_college_mo`, `snow_college` all dropped to 0 after their own notes turned out to already disclose the exact elective-only or generic-survey problem, just not yet applied to the stored `covered` flags. `neo_am` (3→1) and `eastern_oklahoma_state` (2→1) each lost one unit to the same pattern while keeping one genuinely required credit.

  All corrected via `acuAlignNote` + `acuUnits[].covered` + `acuAlign` + `lensScores.academic`; `fitOlivier`/`lensScores.overall` confirmed unaffected throughout (acuAlign has been informational-only since v37.1). Validated clean: `validate_schools.py`, `validate_consistency.js` (Issues: 0), `check_no_jargon.py` (20,646 strings). CRLF intact. See CHANGELOG.md for the full per-school detail.
...
- **🚩 PLAN (owner request, 2026-09-18) — review the Division I gaps between the guide and each conference's Total Programs.** Owner ruling (v45.62): missing D1 schools stay visible as gaps until reviewed; do not add any without the owner's pick. **Source of the list:** NCAA member directory, men's soccer, academic year 2026-27 (`web3.ncaa.org/directory/api/directory/memberList?type=12&division=I&sportCode=MSO`, `conferenceName` = sport conference). Re-pull it at the start of the review; memberships changed for 2026 and may again. Picks = MLS SuperDraft picks 2022-26 from `data/mls-draft-picks-2022-2026.json`.
  ...
  **The 67 missing programs in the 15 conferences the guide covers** (guide/total):
  | Conference | Guide / total | Missing (MLS picks 2022-26 where >0) |
  |---|---|---|
  | ACC | 13/15 | Boston College (3), Virginia Tech (1) |
  | Big Ten | 11/11 | none |
  | Big East | 12/12 | none |
  | AAC | 8/9 | Missouri State (1) |
  | Big West | 3/7 | CSUN (2), Utah Valley (2), Sacramento State (1), Cal State Bakersfield |
  | Pac-12 | 3/7 | Oregon State (8), San Diego State (3), California Baptist (1), Gonzaga (1) |
  | Mountain West | 2/6 | San José State (2), Air Force (2), Utah Tech (1), UNLV |
  | Ivy | 2/8 | Cornell (5), Penn (4), Brown, Columbia, Dartmouth, Harvard |
  | CAA | 8/10 | Campbell (2), UNC Wilmington (1) |
  | ASUN | 1/9 | Lipscomb (2), Florida Gulf Coast (1), North Florida (1), Bellarmine, Jacksonville, Queens, Stetson, West Florida |
  | WCC | 1/9 | Portland (7), San Diego (4), Loyola Marymount (3), Seattle (3), San Francisco (2), Saint Mary's (1), Santa Clara (1), Pacific (1) |
  | America East | 1/8 | New Hampshire (5), Bryant (3), UMBC (1), Albany, Binghamton, NJIT, UMass Lowell |
  | NEC | 1/9 | Central Connecticut, Chicago State, Fairleigh Dickinson, Howard, Le Moyne, LIU, Stonehill, New Haven |
  | Summit | 1/6 | UMass (1), Oral Roberts, Kansas City, Omaha, St. Thomas (MN) |
  | Patriot | 2/10 | Boston University (1), Loyola Maryland (1), American, Bucknell, Colgate, Holy Cross, Lafayette, Lehigh |

  **Second, larger gap (separate decision, later):** the directory lists 213 D1 men's programs; the guide covers 69 and the table above holds 67, so **77 play in conferences with no card at all** (Atlantic 10, Big South, Horizon, Metro, Missouri Valley, Ohio Valley, Southern, Sun Belt). A school added from one of those also needs a new conference card, prestige row, `group` key and chip entries (Change Type 1 "new conference" rows).

  **Steps:**
  1. **Triage table, no data edits.** For each of the 67: state, warm/city per this guide's own rules (§7 1H), MLS picks, whether an athletics roster and coach page exist, and any rule that already decides it. **Already decided by earlier owner rulings, list but do not research:** the 6 Ivy schools (no athletic scholarships; Ivy is out of research campaigns, §6C) and Air Force (service academy; §4 rule, narrative warning only). Suggested priority for the owner, not a decision: warm-climate programs first (12 in California, 5 in Florida, Utah Tech, UNLV, Campbell/UNC Wilmington), then the strongest pipelines elsewhere (Oregon State 8, Portland 7, New Hampshire 5, Boston College 3, Seattle 3).
  2. **Owner picks** Add / Watch / Skip for each. Record the picks here.
  3. **Add in small batches** with the new-school skill (Change Type 1, RULE 0 research, ≤4 schools per commit, one conference per batch where possible). No Conferences-tab edits are needed per school: cards, counts and Programs in Guide are derived (v45.63). Update the CLAUDE.md reference table and `otherSchools[]` per the skill.
  4. After each batch: qa-suite, local browser check of Explore, the school modal and the Conferences card, then CHANGELOG + version bump.
  5. Revisit the 77-program gap as its own decision once step 3 is done.
- **🚩 Found 2026-09-18, not fixed (small):** (1) Cal Poly's 2025 `confRecord` says 3rd in the Big West, but its coach record/bio say back-to-back regular-season titles 2024-25; check bigwest.org's 2025 standings. (2) Santa Monica's `conf` says "CCCAA / SCFA", but cccaasports.org's 2026 standings list it in **Western State-South**; confirm and correct the label. (3) The Mountain West card has no founding year (not found on themw.com); fill it only from a Tier-1 page. (4) The 2022 SuperDraft parsed as 78 selections against the 79 the mls-pipeline skill quotes; no guide conference is affected.


- **✅ RESOLVED v45.72 — all 11 stored Division II JUCOs and all 5 stored Division III JUCOs checked against NJCAA's live 2026-27 team lists. No moves needed; this recheck came back completely clean.** njcaa.org loaded without a VPN this session (reachable at the time, unlike prior sessions — access can vary, don't assume either state). Checked each school individually via the division team-stats page's own search box (`njcaa.org/sports/msoc/2026-27/div{1,2,3}/teams`), not the aggregated list, per the Region 15 lesson. **Division II, all 11 confirmed present on the live DII list, unchanged:** `northeast_cc`, `phoenix_college`, `pima_cc`, `glendale_cc_az`, `neosho_county_cc`, `southeastern_cc_ia`, `iowa_lakes_cc`, `johnson_county_cc`, `national_park`, `hagerstown_cc`, `harford_cc`. **Division III, 4 of 5 confirmed present on the live DIII list, unchanged:** `nassau_cc`, `ulster_cc`, `suffolk_cc`, `westchester_cc`. **`oxford_college_emory` — the known discrepancy persists unchanged:** NJCAA's live 2026-27 list still places Oxford's team under Division I, same as it did when this was first found and resolved (v45.60) — the guide correctly stores it as Division III per the owner's ruling (Emory's own admissions FAQ says Oxford competes in NJCAA Division III and cannot offer athletic scholarships), so this is not a new finding, just confirmation the old one still holds. Since 2026-27 memberships were already current when Division I was checked (v45.59) and now DII/DIII are too, this campaign is closed until the 2027-28 season list is published — re-run it then, not before.
...
- **✅ CAMPAIGN COMPLETE (v45.15 → v45.29) — degreeTitle/acuAlign fabrication audit, both the 110-school pre-RULE-0 sweep and the 60-school post-RULE-0 follow-up. Full results and methodology preserved below for reference.**
  ...
  **Origin, and a reversal (v45.66):** the owner could not find a sports-related program on Indian Hills Community College's website, and v45.15 concluded that the stored `degreeTitle` — "A.A. in Exercise Science & Kinesiology" — was fully fabricated after a Program Finder search (96 programs) returned no match. **That conclusion was wrong.** On 2026-09-21 the owner pasted the college's own catalog entry, "Exercise Science and Kinesiology, A.A" (Ottumwa campus, 64 credits, a transfer major for Iowa, Iowa State and Northern Iowa: `https://ihcc.catalog.acalog.com/preview_program.php?catoid=71&poid=10172`), so the degree name written in v24.1 (2026-06-23) was real. What v24.1 did get partly wrong was the ACU mapping: it credited Exercise Physiology (`EXSC225`), but the 64 credits contain no such course. v45.66 restored `degreeTitle` and re-mapped against the catalog's course list: `ANAT100` ← BIO 175/176, `BIOL125` ← BIO 120/121/122, `EXSC224` ← PET 231, so `acuAlign` is 3 and `lensScores.academic` 31 (`fitOlivier` untouched, ACU alignment left the Fit Score in v37.1). **A Program Finder or majors-list search that returns zero matches is NOT proof a program is absent.** Why the finder missed this one was never established (it builds its list dynamically, and a probe in v45.66 was inconclusive). Confirm any absence claim against the catalog's own program page or search (for Acalog catalogs, `preview_program.php`) and record the URL checked. **No validator can catch this class of error** — `validate_schools.py`/`validate_consistency.js` check internal consistency (`acuAlign` matches the `covered:true` count), never whether the underlying claim is real. The Physical Therapist Assistant A.A.S. found in v45.15 is real and separate (a selective career program, not a transfer degree). **Second fabrication found v45.16, first of the Tier 1 pass: `iowa_western`'s "A.A. Kinesiology / A.S. Health Sciences" — IWCC has no Kinesiology or Health Sciences program at all; real degree is "Exercise Science Transfer Major, A.S."** Same fix pattern applied (`acuAlign` 8→4, `lensScores.academic` 40→36); see CHANGELOG.md v45.16 for the full worked example, including a near-miss where the first draft of the corrected `acuAlignNote` itself violated the §8 "no internal audit language in rendered fields" rule — caught by re-reading the live modal render before commit, not by any validator. **Third find v45.17, first of Tier 2, a genuinely different shape: `chapman`'s degree itself is real (Kinesiology was formally renamed to Applied Human Physiology), but its `courses[]` array cited five "KIN ###" course codes that don't exist under either name — the fabrication was in the supporting course list, not the degree claim itself.** `acuAlign` corrected 12→5 (evidence-based, not a guess in the other direction — see CHANGELOG.md v45.17). This confirms the audit needs to check `courses[]` and `acuAlignNote`'s cited course codes, not just `degreeTitle` in isolation — a real degree name can still carry fabricated supporting detail. **Fourth find v45.18: `lynn`'s "B.S. in Exercise Science" was fully fabricated — Lynn has no Exercise Science, Kinesiology, or Athletic Training program anywhere, confirmed against its complete undergraduate degree list.** Real closest options: Sports Management (business) or Biology (pre-med, includes a genuine anatomy/physiology course). `acuAlign` corrected 11→2.

  **Scope, computed by diffing every current school id against the data files as they stood at the RULE 0 commit (`97218e9`, 2026-07-10) — 110 of 170 schools existed before RULE 0 and have never had their `degreeTitle` claim checked against a live source:**

  | Status | Count | Ids |
  |---|---|---|
  | ✅ Fixed (fabricated/false claim) | 11 | `iowa_western`, `lynn`, `duke`, `georgetown`, `northwestern`, `stjohns`, `ucsb`, `ucsd`, `villanova`, `washington` (all ten fully fabricated degrees — the school has no such program at all; `northwestern` and `ucsd` both distinctively reused a *real* course prefix from an unrelated source — Michigan's `MOVESCI` and UCSD's own Education Studies `EDS`, respectively — rather than inventing one from nothing; `stjohns` is the only one with no real adjacent major found at all, `acuAlign` corrected to 0), `chapman` (partial — real degree, renamed Kinesiology→Applied Human Physiology, but fabricated course codes attached to it) |
  | ✏️ Minor correction (real program, degree-type/course-code accuracy only) | 16 | `indian_hills` (reversed v45.66 — counted as a fabrication until then; the degree is real, only its ACU mapping needed correcting), `stedwards`, `butler`, `cal`, `charleston`, `charlotte`, `delaware`, `gcu`, `louisville`, `setonhall`, `stonybrook`, `uab`, `uca`, `ucdavis`, `vermont` (see prior entries — all real programs, wrong name/prefix), `virginia` — real degree is a B.S.Ed. in Kinesiology (School of Education and Human Development), not a plain B.S.; the content description was already accurate, only the degree-type suffix was missing. None of these fifteen are fabrications — real, substantive programs with a wrong name and/or fabricated course-code prefix. |
  | ✅ Spot-checked/verified, confirmed real | 82 | `smc`, `tyler_jc`, `arizona_western`, `cowley_cc`, `angelina_college` (5 pre-campaign, checked during the Indian Hills investigation), `barton_cc`, `blinn_college`, `coastal_bend_cc`, `daytona_state`, `dodge_city_cc`, `efsc`, `iowa_lakes_cc`, `johnson_county_cc`, `lsu_eunice`, `miami_dade`, `mohave_cc`, `monroe_college`, `nassau_cc`, `neosho_county_cc`, `northeast_cc`, `phoenix_college`, `pima_cc`, `southeastern_cc_ia`, `suffolk_cc`, `ulster_cc`, `westchester_cc` (21, the rest of Tier 1), `barry`, `columbia_college` (its stored "⚠ No Exercise Science" disclosure is accurate — real offerings are Healthcare Management / Health Sciences), `csula`, `georgian_court`, `keiser`, `nova`, `ocu`, `pba`, `uc_charleston` (9, the rest of Tier 2), `akron`, `army`, `calpoly`, `clemson`, `creighton`, `csuf`, `denver`, `depaul`, `drexel`, `elon`, `fau`, `fiu`, `hofstra`, `indiana`, `marquette`, `maryland`, `memphis`, `mercyhurst`, `michigan`, `michiganstate`, `monmouth`, `navy`, `ncstate`, `northeastern`, `notredame`, `ohiostate`, `pennstate`, `pittsburgh`, `princeton` (honest no-major disclosure, accurate), `providence`, `rutgers`, `smu`, `stanford`, `syracuse`, `temple`, `tulsa`, `ucirvine`, `uconn`, `ucriverside`, `ucla`, `unc`, `usf`, `wakeforest`, `william_mary`, `wisconsin` (genuine compound name "Kinesiology: Exercise and Movement Science"), `xavier`, `yale` (honest no-major disclosure, accurate) (47, all of Tier 3 confirmed real) |
  | ⚠️ Inconclusive (network-blocked, not a data finding) | 1 | `glendale_cc_az` — its site + district-wide `maricopa.edu` both hit a persistent Cloudflare challenge; retry with a different network/method, don't assume either way |

  **✅ TIER 1 CLOSED (v45.16) — all 22 JUCOs checked: 21 confirmed real + 1 fixed (`iowa_western`).**
  **✅ TIER 2 CLOSED (v45.19) — all 12 D2/NAIA/D3 schools checked: 9 confirmed real + 2 fixed (`chapman`, `lynn`) + 1 minor correction (`stedwards`).**
  **✅ TIER 3 CLOSED (v45.28) — all 69 major D1/Ivy schools checked: 47 confirmed real + 14 minor corrections + 8 real fabrications (`duke`, `georgetown`, `northwestern`, `stjohns`, `ucsb`, `ucsd`, `villanova`, `washington`).**

  **✅✅✅ CAMPAIGN COMPLETE — all 110 pre-RULE-0 schools checked (109 resolved + 1 network-inconclusive).** Final tally (corrected v45.66): **10 full fabrications** (`iowa_western`, `lynn`, `duke`, `georgetown`, `northwestern`, `stjohns`, `ucsb`, `ucsd`, `villanova`, `washington` — the school had no such program at all), **1 partial fabrication** (`chapman` — real degree, fabricated course codes), **16 minor corrections** (real programs with a wrong name, a wrong course-code prefix or a wrong course-to-unit mapping; includes `indian_hills`, reversed in v45.66 — not fabrications), **82 confirmed real as originally stored**, **1 inconclusive** (`glendale_cc_az`, network-blocked, never resolved either way). Don't assume prestige or thoroughness of documentation protects against fabrication — most of this campaign's worst finds came from selective, extremely well-documented schools (Duke, Georgetown, Northwestern, UCSD, UCSB, Villanova, Washington), not obscure ones. The 60 schools added *after* RULE 0 (v44.71 onward) were never in this campaign's scope — they were supposed to be live-verified at add time (2 spot-checked since, both confirmed real), but a systematic audit of that group has not been done and remains a genuinely open item if ever revisited.

  **✅ POST-RULE-0 FOLLOW-UP COMPLETE (v45.29) — all 60 schools added AFTER RULE 0 (v44.71 onward, the NJCAA gap-fill campaign) checked against live sources.** Scope was the same diff-against-`97218e9` method, minus one false positive: `delaware` appears in that diff because it moved from `caa.json` to `d1-other.json` after RULE 0, but it's a pre-existing school already covered (and fixed) in the 110-school campaign above — the real post-RULE-0 set is 60, not 61. **Result: 0 fabrications, 59 confirmed real as stored, 1 mapping correction** (`rose_state` — real courses HPER 1213 "Introduction to Health & Sports Sciences" and HPER 1202 "Health & Wellness" were credited against EXSC224/Biomechanics and EXSC199/Sport Psychology respectively, neither of which the courses actually teach; `acuAlign` 5→3, `lensScores.academic` 42→31, `fitOlivier` unchanged — this was a pre-existing, already-flagged open item, not a new find). This is a materially different result from the pre-RULE-0 campaign (11 fabrications + 1 partial in 110 schools) and confirms RULE 0 (mandatory real-browser Tier-1 verification, in force since v42.9) was genuinely followed when this batch was researched — a real-world confirmation that the rule works, not just a compliance checkbox. 53 of the 60 were checked directly this session; `trinidad_state`/`illinois_central`/`yavapai_college` carried forward from prior-session spot-checks; `daley_college`/`kennedy_king_college`/`truman_college`/`wilbur_wright_college` (City Colleges of Chicago) rely on the v44.74 batch's own documented `catalog.ccc.edu` verification; `usc_salkehatchie`/`usc_sumter`/`usc_union` inherit `usc_lancaster`'s directly-confirmed finding that all four USC regional campuses share one system-wide associate curriculum (AA/AS/Business/Criminal Justice only, no dedicated Exercise Science associate) — a structural fact, not a per-campus guess. See CHANGELOG.md v45.29 for the school-by-school list.

  **Companion coach-data audit (v45.30) — same 60 schools, checked `coaches.json` for stale coach names/titles.** 57/60 confirmed current; 3 genuine departures found and fixed (`illinois_central`, `noc_enid`, `malcolm_x_college` — the last left `"Vacant"`, no successor found). See CHANGELOG.md v45.30 and §6D below for the `citycollegesofchicagoathletics.com` domain-compromise finding this surfaced.

  **Methodology per school (matches this session's Indian Hills/spot-check method exactly):** real browser (RULE 0, §15) → navigate to the school's official Program Finder / academic catalog (not a marketing page) → search the full page **text content** (not just `innerText`, which respects `display:none` and misses paginated/hidden list items — use `document.body.textContent` via `javascript_tool`) for "exercise," "kinesiology," "sports," "athletic training," "physical education," "fitness" → confirm the *specific* stored `degreeTitle` string actually exists as a named program, not just that adjacent-sounding words appear somewhere on the site. **Since v45.66: a zero-match is not proof of absence** (see the Indian Hills reversal in the Origin note above) — before recording a program as absent, also look it up in the catalog's own program list and record the URL you checked. **🚩 Open:** the 10 remaining full-fabrication findings were established by this same kind of absence check and have not been re-verified against each school's own catalog; re-check them before treating them as settled. **Since v45.17: also spot-check `courses[]` against the school's real course prefix/codes** — `chapman` proved a real, correctly-named degree can still carry an entirely fabricated supporting course list (invented "KIN ###" codes at a school whose real prefix is "AHP"), which a `degreeTitle`-only check would miss entirely.

  **Fix procedure when a fabrication is found (Change Type 9, §3a) — do NOT deviate from this order:** (1) research and correct `degreeTitle`, `acuAlignNote`, `kinRank`, `acuAlign`, `acuUnits[]` for that one school only (never batch-guess several schools' fixes from a pattern — each is its own live check); (2) recompute `lensScores.academic` from the formula above and re-store it; (3) confirm `fitOlivier`/`lensScores.overall`/`.value` do NOT need to change (they never do for this field, per v37.1 — verify by reading `js/scores.js`'s `calculateFitScore()`, don't assume); (4) `python -m json.tool`, `validate_schools.py`, `node validate_consistency.js` — all must stay clean; (5) browser-verify the Details modal's "Degree & ACU Align" tab renders the correction; (6) CHANGELOG.md entry + `guideVersion` bump + commit, one school (or a tightly-related batch) per commit — v45.15's Indian Hills entry is the template to copy the structure of. **Do not fix a school you have not personally verified live this session** — a plausible-sounding degree name is exactly how the original fabrication looked too.

  **Update this table's status column (don't just delete rows) as schools clear, and re-run the qa-suite skill after every batch.** When Tier 1+2+3 are fully cleared, collapse this into a short "✅ CLOSED — audited all 110 pre-RULE-0 schools, N fabrications found and fixed" line for the state snapshot, the same way other completed campaigns in this section are closed out.

- **✅ RESOLVED v45.54 — Monroe championship history (owner request).** Monroe's own schedules and releases show titles in 2019, 2022 and 2023 (not 2021), a 2021 East District semifinal and a 2024 East District semifinal loss at Harcum (not a national runner-up finish). `titles`, `confRecord`, the coach record/bio/strengths and the `pipeline.json` row corrected. Monroe's division transition and roster stay parked per the owner.
- **✅ RESOLVED v45.55 — stored lens scores.** soccer (108), academic (54) and lifestyle (34) re-stored to their §7 Phase 1J formulas; the `LENS` check now enforces all four derived lenses exactly. Lifestyle had hand-set "cultural" adjustments with no written rule; the formula (warm x 50 + city x 50) now applies everywhere.
...
- **✅ RESOLVED v45.51 — Tyler JC's unverified "#1 D1 transfer feeder nationally, all-time record" claim** was replaced everywhere it rendered (`soccerLevel`, `titles`, `notable`, `jucoTierNote`, `rec`, `extras`, coach bio/strengths, `conferences.json`, `conf-prestige.json`, `pipeline.json`, `js/app.js`) with what the guide can support: the highest measured D1 transfer rate of any JUCO in this guide (about 6.6 a year, 2012-2023). Daytona State's unsupported "top-5 D1 transfer feeder" claim was removed the same way.
...
- **🚩 Low-priority code-quality items, carried since the v36 review (re-checked v45.51).** Already fixed in code, so no longer open: the search keyword is escaped with `esc()`, and `selectSchoolFromBar()` no longer uses a `toString` matcher. Fixed v45.51: the Glossary Minutes Score weights now read Yr1 60% / Yr2 40% (matching `minutesOutlookScore()`), and the FX slider sublabels now read 1.20–1.70 (matching the slider range). Still open: `atarToGpa` is defined in **both** `scores.js` and `app.js` (app.js wins by script load order — **do not reorder the script tags**); `DATA_BASE_URL` means `./data/` in app.js but site root in dashboard.js; `olivier.json` is fetched twice per page load; dashboard `filterToConf('other')` scrolls to the Ivy section (5 Explore sections share `data-confkey="other"`, plus 5 duplicate `id="grid-other"` elements).
...
- [ ] Full official name (for `full`) — **if it includes a mascot/nickname in parens, verify that name against ≥2 independent Tier-1-ish sources (the school's own athletics social account, Wikipedia's college-mascot list, MascotDB.com) before trusting it, even for an already-shipped school.** Two stored mascot names were found wrong in v45.12 (Daley College stored as "(Comets)", actually "Bulldogs"; Kennedy-King stored as "(Hawks)", actually "Statesmen" — "Hawks" genuinely belonged to a *different* school in the same batch, and both cards claiming the identical mascot simultaneously was the tell that something was off). Never assume a mascot name is correct just because it's already in the data.
...
- [ ] Athletics domain (for favicon) — use the domain of the ATHLETICS SITE, not the main university domain. JUCOs often have a separate athletics site (e.g. apacheathletics.com for TJC, not tjc.edu). Verify by loading the favicon: `https://[domain]/favicon.ico`. **A 404 there does not mean no icon exists** — check the page's own `<link rel="icon">` tag (`document.querySelectorAll('link[rel*="icon"]')` in a real browser) for the real path; several real school icons live at a WordPress-generated `wp-content/uploads/...` path or similar, never at the default location. **If the school is one campus of a larger shared-website system (a community-college district, a multi-campus university system), check whether it shares its domain with its sibling campuses** (the City Colleges of Chicago case, v45.12: 5 colleges all resolved to the identical `ccc.edu` favicon, since a favicon is fetched at the domain level and none of the 5 has its own subdomain). If so, no amount of fetch-logic improvement will ever differentiate them — the only fix is a per-school local asset via `ICON_OVERRIDES` (§4), so flag this immediately rather than treating it as a bug to chase later. **When sourcing a mascot logo directly (not a favicon), a plain Google Images search for `"[School Name]" [mascot] logo` (Tier-1 cross-check the result, don't just take the first hit) reliably finds a clean, high-resolution, correctly-shaped asset — this outperforms scraping a social-media profile picture, which is small, gets compressed through a screenshot, and often carries UI chrome (story rings, card borders) that has to be manually cropped out.**
...
**If the new version never appears at all, check the BUILD before blaming cache (added v45.35).**
GitHub Pages builds for this repo take **37–41 seconds**. A build stuck at `status: building` with
`duration: 0ms` has not started and will not finish on its own — one sat that way for **10.5 minutes**
this session while hard reloads were (correctly) achieving nothing.

```bash
gh api repos/bustachat/olivier-guide/pages/builds --jq '.[0:3][] | "\(.status) \(.commit[0:7]) \(.duration)ms"'
gh api -X POST repos/bustachat/olivier-guide/pages/builds   # request a fresh build, no commit needed
```
The re-request built the **identical commit** in 35s. **Two diagnostics matter:** `duration: 0ms` means the
build never started (infrastructure, not your content — a content failure is deterministic and would fail
the retry too); and an `errored` entry immediately followed by a `built` one ~30s later is just Pages
coalescing two rapid pushes, not a real failure. **Do not poll in a loop** — compare the duration against
the ~40s norm, and if it is stalled, re-request.
...
- **Unify `sc()` and `fitColor()` (added v45.32).** They look like duplicates and are not. `fitColor()` (js/scores.js) cuts at **58/48** and serves the 5 **Fit Score** sites; `sc()` (js/app.js) keeps **90/80** and serves the 6 **Dev Score** sites. The two scales are genuinely different: Fit sits at or below ~76 (UCLA, v45.55), while dev scores reach 93 (median 57). Collapsing them either turns 83 dev scores green or returns every Fit Score to the bottom colour — which is the exact bug v45.32 fixed, where all 170 schools rendered red because `sc()`'s thresholds were still calibrated for the retired pre-v37.1 score. If you change a band cut, change it in `FIT_BAND_STRONG`/`FIT_BAND_MIDDLE` only — the filter chips and the colour both read `fitBand()`, and hardcoding a cut anywhere else silently desynchronises them.
- **Add an app-fetched path to `_config.yml`'s `exclude` (added 2026-09-12).** That file exists only to hide `CLAUDE.md`/`CHANGELOG.md` from the published site. Everything the browser fetches must stay published — `index.html`, `js/*`, `data/*.json`, `athletes/*.json`. Excluding one would return a clean 404 and **silently kill the app** while every other check still passed. Note especially that **`data/coaches.json` is public by design, not by accident**: it carries 149 coach emails and 121 phone numbers and is fetched client-side, so no repo setting can hide it — a client-side app can only render data the browser can reach. Excluding it would break the guide and protect nothing. Setting `exclude` also REPLACES Jekyll's defaults; safe today (no Gemfile/node_modules/vendor), so re-add them if one ever appears.
- **Change ONE of the Fit Score's display paths (added v45.35).** There are **three**, and they must agree: the card tile (`refreshLensScoreDisplay()`), the modal template (`buildDetailBody`), and **`openDetail()`'s post-paint `requestAnimationFrame` write**, which re-sets `#modal-fit-score` *after* the template has rendered. All three route through `displayFit()` / `displayFitColor()` — keep it that way. This bit twice in one session: v45.33 shipped a modal that silently reverted to the canonical score, and v45.34 a modal coloured with Fit bands while the card beside it showed neutral. **The rAF path cannot be caught by a test in a hidden browser pane** (rAF never fires there), so a green test run is not evidence it is correct — invoke the write explicitly, or read `openDetail()` and confirm by eye.
- **Store a `climateNeutral` value on `lensScores` (added v45.32).** It is computed live on purpose — see the §6 state-snapshot entry. Adding the key breaks `validate_schools.py`'s 6-key check AND re-creates the drift class the live computation exists to avoid.
- Use aggregator sites (Niche, CollegeData, 247Sports, RosterResource) as data sources — see §15
- **Write internal process/methodology language into any field the app actually renders.** Found live v44.89 by the owner, in the Minutes Outlook tab: `trajectoryNote` and `recruit_pathway_note` (`js/app.js:3692-3693`, `2443-2444`) render verbatim to every visitor, and 15 entries across `data/juco.json`/`data/caa.json`/`data/big-west.json`/`data/d2.json` contained things like "⚠ MIXED VINTAGE... §14's Opportunity Score table cannot reproduce any stored JUCO trajectory... pending the JUCO calibration item in CLAUDE.md §6E" and "`NEXT_LEVEL_NEUTRAL`"/"`D1_RATE_DIVISOR`" (internal code constants) — this is a scholarship guide read by a real athlete, parents, and coaches; none of them should see a CLAUDE.md section number, a backtick-quoted field name, or a code constant. **Before saving any `*Note`/`*_note` field, know whether the renderer touches it — grep the exact field name in `js/app.js`/`js/dashboard.js` first, don't assume.** Two fields in this schema are genuinely internal-only and safe for citation-style language (`§5a`/`§5b`/Tier-1 sourcing shorthand): `devScoresNote` and `coaches.json`'s `overallScoreNote` — neither has a render call anywhere (confirmed by grep, not by the schema doc, which doesn't say so explicitly). Every other note-shaped field ships to the page: `trajectoryNote`, `recruit_pathway_note`, `fin.internationalNote`, `proPlayers.nextLevel.note`, `facilityDetails.note`/`housing.note`, `confRecord[].note`, `acuAlignNote`, `jucoTierNote`, `rec`, coach `bio`, `conferences.json`'s `desc`/`olivierNote`. Write those in plain language a parent or coach would read on a recruiting site — real findings and caveats are fine and expected ("this projection is based on last year's roster and hasn't been updated yet"), internal accounting is not ("§14's table cannot reproduce this, so re-deriving would be a fabricated number"). **Confirmed to recur, v45.14 — owner-reported via a screenshot of Indian Hills' Details modal.** This rule already named `proPlayers.nextLevel.note` as a rendered field, but nobody had actually swept it: **all 89 JUCO schools carrying a `nextLevel` object had this exact leak**, not just Indian Hills — phrases like "p90 anchor of the 7 multi-year JUCOs", "EXCLUDED from the comparison benchmark, same treatment as Phoenix College's n=1 precedent", "owner-approved neutral (v42.12)", and even a live browser-automation bug note ("navigate returns before render — read as a separate call") were rendering verbatim in the Pro Pipeline tab. Worst case, Indian Hills: "exactly the signal dev scores were being distorted to carry" — a direct reference to this guide's own §5a dev-score-calibration history, visible to a real family. All 89 rewritten to plain language in the same session (v45.14) with no scoring cascade (`nextLevel.note` carries no scoring weight). **The lesson: naming a field in this rule is not the same as having swept it** — this list is a checklist for new writes, not proof past writes comply. If a future session finds another instance, `sweep.py` (§15) is the right tool to confirm the true count before fixing — do not assume one bad example means the rest are clean, and do not assume the rest are clean just because a rule already named the field.
...
### NJCAA stats roster — secondary step for every NJCAA JUCO refresh (added 2026-09-14, owner-directed)
...
**How it "validates the date": the page has NO last-updated timestamp.** Read the **GP (games played)** counter in the stats strip above the roster table instead. GP > 0 means this 2026-27 roster is the one being entered into real match box scores, which is stronger evidence than any "updated" label. **GP = 0 means it may be a preseason placeholder.** Treat that like "published ≠ populated", and run the goalkeeper/midfielder-share test before trusting it.

**Conflict rule:**
- **Different seasons:** the newer season wins, provided GP > 0.
- **Same season, different names or positions:** the school's own roster page wins, per the table below. Do not silently pick one. Disclose the difference in the note, and re-check it later.

**Independence caveat, and it matters.** Tyler JC's own athletics site migrated onto this same platform: `apacheathletics.com/teams/407485412185999056/...` carries the **identical team ID** as its NJCAA page. For any school on that platform, the two pages are one database. Agreement between them proves consistency, not independent confirmation. For schools still on Sidearm or another CMS, the NJCAA roster is maintained separately, and a match is a genuine second source.

**Access: VPN REQUIRED.** `njcaa.org` geo-blocks Australian traffic, confirmed 2026-09-14 from both home broadband and 5G (full note in §6C, under the Tyler access method). Ask the owner to turn the VPN on first. A throwaway query string (`&_=1`) only clears a *cached* 403 left over from before the VPN connected; it does not bypass the block. As on every site in this family, `navigate` returns before the table renders, so wait ~5s before reading.

**Layout:** a plain `<table>` with columns `# | Name | Position | Year | Height | Weight | Hometown | Status`. There is **no previous-school column**, so it cannot inform `recruit_pathway`. **Position codes vary by school even on this one site:** Tyler uses `MID`/`DEF`/`FWD`/`GK`, while Blinn uses `M`/`D`/`F`/`GK`. Normalise both. Year is `FR`/`SO`, and the JUCO class rule applies (§3a): on a 2026-27 roster, `SO` clears and `FR` returns.
...
**Field notes from the first full NJCAA pass (Batch 7, 2026-09-15) — read before the next one:**
- **Pages are slow.** A roster table can take 15–25 s to appear, and the teams list ~15 s. A read at 6–10 s often returns an empty table, which is a timing false negative. Poll for `table tbody tr` (or the text "No roster available") before concluding anything. `computer: wait` is capped at 10 s per call, so chain a wait with an in-page poll.
- **"No roster available."** is a real, explicit state. Record it as such.
- **NJCAA data has its own errors, so never let it overrule a complete school roster:** women's-team players mixed into a men's roster (North Idaho, Phoenix), class words ("FRESHMAN") in the position column (Northwest), class years blank for a whole squad (Truman, Southwestern Illinois, NEO A&M), duplicate or mis-encoded name rows (common), extra names absent from the school site (EFSC, Garden City, Salt Lake CC, Seward County).
- **Merging is allowed when each source is complete for a different field and the names match exactly** (Hagerstown: classes from the school site, positions from NJCAA). Only do it when every **midfielder** has both fields, and disclose it in the note.
- **Transient extension block:** Claude-in-Chrome once refused every site ("Could not verify this site's safety category") right after the VPN was switched on. It cleared on its own within minutes; open a fresh tab and retry rather than switching browsers.
...
| 21 | 21 JUCOs stored `recruit_pathway` on the school object, so the Pathways tab never showed them (a new school copied the pattern from one of them) | `PATHWAY-LOC` (the fields may only live in `minutesOutlook`) | `validate_consistency.js` |
| 23 | A Python text-mode rewrite turned the CRLF working copies of `juco.json`/`conferences.json` into LF, so CRLF negtest fixtures went VOID (proved nothing) | After scripted data edits and commit, restore working copies with `rm <file>; git checkout -- <file>`, then run negtest; write new fixtures with `\r\n` like the existing ones | `negtests/checks.json` |
| 22 | `apply_roster_refresh.py` kept a pre-v45.53 division strength, storing Division II/III JUCO Fit one point high; the `FIT` check's 1-point tolerance hid it | `FIT` is now exact; the Python copy was aligned with `js/scores.js` and checked against all 174 schools | `validate_consistency.js`, `apply_roster_refresh.py` |
