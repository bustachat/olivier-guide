...
| **4** | `tyler_jc` — access method confirmed 2026-08-20 (direct, then VPN + cache-bust if 403; see the dated note below the table). Content re-checked with working access: genuinely no roster data published for 2026-27, 2025-26, or 2024-25. | Periodic recheck only — the rebuild may eventually populate a season; nothing to extract today. |
...
- **✅ ACCESS METHOD CONFIRMED, CONTENT ABSENCE RE-CONFIRMED 2026-08-20 — `tyler_jc`'s athletics host (`apacheathletics.com`) 403s direct/residential connections with a CloudFront block page, but a VPN reliably reaches it.** Distinct from the datacenter-IP CloudFront block seen elsewhere in this project (Suffolk, `goreivers.com`) — those block *hosting-provider* IPs specifically and a normal residential connection sails through; this one 403s the owner's own normal residential connection too, and only clears with a VPN, which points to a geo/traffic-pattern WAF rule rather than a datacenter-IP rule. **A second gotcha on top of the VPN requirement: CloudFront caches the 403 error page itself** — the very next request after connecting the VPN returned the byte-identical cached error (same `Request ID`), and only cleared once a cache-busting query string (`?_=1`) forced a fresh request. **Standing procedure: try the direct connection first (residential and Claude-in-Chrome both); if that 403s, ask the owner to enable a VPN; if the VPN'd request still shows the error page, append a throwaway query string and retry once before concluding the block is still active.**

  **With access working, the site's own roster pages were checked directly (not inferred) across three seasons — `/sports/mens-soccer/{2026-27,2025-26,2024-25}/roster` — and all three return "TEAM SEASON NOT FOUND — No exact school team exists for this sport and season."** This is not a loading race (each was re-checked after an explicit wait) and not season-specific (three different seasons, same result), so the earlier "may be a mid-rebuild, no content once reached" hypothesis is now a **confirmed finding, not a guess**: the site is reachable but genuinely has no men's soccer roster data published for any of the last three seasons. Re-check periodically (the rebuild may eventually populate this), but there is nothing to extract today. `coaches.json.url` is also one of only 3 schools guide-wide with no stored URL at all (§6D) — same underlying cause, still unresolved.

  **The full timeline, traced through git history 2026-08-20 — the site was genuinely rebuilt, and the rebuild dropped historical data, not just failed to add new data:**
  1. **2026-06-24 (v27.3)** — `tyler_jc`'s currently-stored roster (13 named players, `roster_season: "2025-26"`) was extracted from a live, working roster page. This is real, not placeholder data, and it's still the guide's live data today.
  2. **A later session** found the site had become an "SPA shell" returning 0 characters of readable text — reachable, but nothing extractable by the standard method.
  3. **Session before 2026-08-16** — found the site now needed a VPN just to load at all, and had no usable content even then.
  4. **2026-08-20 (this entry)** — access confirmed working again (VPN + cache-bust), but **2025-26 — the exact season the guide's own stored data represents — now returns "TEAM SEASON NOT FOUND" on the live site, same as 2024-25 and 2026-27.** That a *past, already-completed* season is missing (not just next season not being set up yet) is the tell: this isn't "hasn't been populated yet," it's a platform migration that didn't bring historical data across. Do not re-derive this timeline from scratch next time this school comes up — it's settled.
  5. **Why this school's staleness isn't urgent despite all of the above:** its stored data is the calibration anchor for the whole JUCO trajectory formula (§14) — chosen specifically because "13 of 13 clear, zero returners" was the cleanest, least ambiguous signal available across all 81 JUCOs. There is no fresher live data to replace it with, and the anchor role means its exact stored values matter beyond just this one school's own card.
