...
- **📌 Considered and DECLINED v45.57 — JUCO division sub-filtering on the Division row.** Owner initially reported the single `JUCO` chip mixes NJCAA Division I/II/III with no way to narrow (true), but on discussion the underlying confusion was actually about the Conference row's separate `NJCAA` chip ANDing against Division (`D1` + Conference-row `NJCAA` correctly returns nothing, since no school is both — see the CHIPS/§4 note on Division vs. Conference being independent filter dimensions). The base behavior the owner actually wanted — `D1` alone excludes all other divisions; `D1`+`JUCO` shows the union — already worked and was never broken. A `JUCO DI`/`DII`/`DIII` sub-filter was built, verified working, then explicitly declined by the owner as unneeded scope. Do not re-add without being asked again.
