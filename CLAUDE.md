...
**Current version: v45.19 (2026-09-03).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
- **🚩 OPEN CAMPAIGN, started v45.15 — degreeTitle/acuAlign fabrication audit. Handover for a fresh session below; do not re-derive the scope, it's already computed.**
  ...
  **Scope, computed by diffing every current school id against the data files as they stood at the RULE 0 commit (`97218e9`, 2026-07-10) — 110 of 170 schools existed before RULE 0 and have never had their `degreeTitle` claim checked against a live source:**
  ...
  | ✏️ Minor correction (real program, degree-type/course-code accuracy only) | 1 | `stedwards` — real, extensive Kinesiology program (6 tracks incl. Pre-PT), but it's a B.A. not the stored B.S.; specific course codes unverifiable, replaced with confirmed real course titles. No fabrication, no acuAlign cascade needed. |
  | ✅ Spot-checked/verified, confirmed real | 35 | `smc`, `tyler_jc`, `arizona_western`, `cowley_cc`, `angelina_college` (5 pre-campaign, checked during the Indian Hills investigation), `barton_cc`, `blinn_college`, `coastal_bend_cc`, `daytona_state`, `dodge_city_cc`, `efsc`, `iowa_lakes_cc`, `johnson_county_cc`, `lsu_eunice`, `miami_dade`, `mohave_cc`, `monroe_college`, `nassau_cc`, `neosho_county_cc`, `northeast_cc`, `phoenix_college`, `pima_cc`, `southeastern_cc_ia`, `suffolk_cc`, `ulster_cc`, `westchester_cc` (21, the rest of Tier 1), `barry`, `columbia_college` (its stored "⚠ No Exercise Science" disclosure is accurate — real offerings are Healthcare Management / Health Sciences), `csula`, `georgian_court`, `keiser`, `nova`, `ocu`, `pba`, `uc_charleston` (9, the rest of Tier 2) |
  ...
  **✅ TIER 2 CLOSED (v45.19) — all 12 D2/NAIA/D3 schools checked: 9 confirmed real + 2 fixed (`chapman`, `lynn`) + 1 minor correction (`stedwards`).** Next up: Tier 3 (69 schools — the largest remaining tier, major D1/Ivy programs).

  | 🚩 **Tier 3 — lower risk, check last** (major D1 conferences — large, well-documented programs, but still technically unverified) | 69 | `akron`, `army`, `butler`, `cal`, `calpoly`, `charleston`, `charlotte`, `clemson`, `creighton`, `csuf`, `delaware`, `denver`, `depaul`, `drexel`, `duke`, `elon`, `fau`, `fiu`, `gcu`, `georgetown`, `hofstra`, `indiana`, `louisville`, `marquette`, `maryland`, `memphis`, `mercyhurst`, `michigan`, `michiganstate`, `monmouth`, `navy`, `ncstate`, `northeastern`, `northwestern`, `notredame`, `ohiostate`, `pennstate`, `pittsburgh`, `princeton`, `providence`, `rutgers`, `setonhall`, `smu`, `stanford`, `stjohns`, `stonybrook`, `syracuse`, `temple`, `tulsa`, `uab`, `uca`, `ucdavis`, `ucirvine`, `ucla`, `uconn`, `ucriverside`, `ucsb`, `ucsd`, `unc`, `usf`, `vermont`, `villanova`, `virginia`, `wakeforest`, `washington`, `william_mary`, `wisconsin`, `xavier`, `yale` |
