...
### School → File Reference Table (167 schools)
...
**Current version: v44.84 (2026-08-13).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
- **167 schools**, all full-profile, across 10 conference JSON files. **167 coaches** in `coaches.json`, ranked 1–167.
