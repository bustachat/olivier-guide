...
**Current version: v45.53 (2026-09-17).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
- **🚩 Monroe titles/confRecord conflict with official results (found v45.53, left per the owner's "leave Monroe" ruling).** `monroe_college` `titles` and `confRecord` say 2024 runner-up and "3x champions 2021-2023". njcaa.org's 2024-25 results page shows the 2024 DI final was Iowa Western v Cowley; Monroe's own Nov 18, 2023 release calls the 2023 title its second straight and third in the last five years. Its `jucoTierNote` and `pipeline.json` row were corrected; the rest waits for the owner.
- **📌 `lensScores.soccer` is stale for many older JUCOs (found v45.53)**, e.g. `indian_hills` 80 against a formula value of 43. Nothing renders or validates it (the Soccer-First lens was retired in v37.1). Low priority; re-store in one pass if it is ever displayed again.
...
- **Unify `sc()` and `fitColor()` (added v45.32).** They look like duplicates and are not. `fitColor()` (js/scores.js) cuts at **58/48** and serves the 5 **Fit Score** sites; `sc()` (js/app.js) keeps **90/80** and serves the 6 **Dev Score** sites. The two scales are genuinely different: Fit cannot exceed ~71 since v37.1, while dev scores reach 93 (median 57). Collapsing them either turns 83 dev scores green or returns every Fit Score to the bottom colour — which is the exact bug v45.32 fixed, where all 170 schools rendered red because `sc()`'s thresholds were still calibrated for the retired pre-v37.1 score. If you change a band cut, change it in `FIT_BAND_STRONG`/`FIT_BAND_MIDDLE` only — the filter chips and the colour both read `fitBand()`, and hardcoding a cut anywhere else silently desynchronises them.
