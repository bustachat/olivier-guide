...
**Current version: v45.37 (2026-09-14).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
| **7B — Region 2 (OK/AR)** | `rose_state`, `neo_am`, `national_park`, `connors_state` | Rose State has the highest JUCO fit in this batch (68). |
