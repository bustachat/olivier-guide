...
**Current version: v45.48 (2026-09-16).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
**🚩 Found v45.48, not fixed — two modals render the literal word "undefined" (pre-existing, present at a7578a3):**
- `vermont` (`data/d1-other.json`): `facilityDetails` has no `extras` or `note`, so the Facilities tab shows "Extras & Unique Perks undefined" and "Facilities verdict: undefined". Needs Tier-1 research, not a placeholder.
- `paris_jc` (`data/juco.json`): `proPlayers.nextLevel` has `perYear` but no `d1Count`/`yearsCovered`/`years`, so the Pro Pipeline tab shows "undefined D1 transfers verified over undefined (undefined yr)". Its note cites two releases (Houston Christian 2025, Missouri State 2026), which suggests `d1Count: 2`, `years: 2`; confirm before storing.
