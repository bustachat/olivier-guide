...
- **🚩 The coloured-initials fallback is unreachable on every surface, because Google's proxy answers with a valid placeholder instead of an error (diagnosed v45.13, still unfixed).** `s2/favicons` returns **HTTP 404 with a decodable ~726B generic-globe PNG in the body**; a browser `<img>` only checks the bytes, so `onerror` never fires again and the terminal initials stage in `handleLogoError`, `handleModalLogoError` and the shortlist's inline chain is dead code for any school that reaches the proxy. v45.13 correctly identified this and then routed around it (adding earlier stages) rather than remediating it, so the 15 schools above render a meaningless globe where legible initials would be strictly better and cost nothing. **A deterministic fix exists and needs no research:** those 15 domains were already hash-confirmed to return the placeholder, so keep a known-placeholder set and skip the proxy stage for it, falling straight to initials. Do not attempt to detect the globe by size/`naturalWidth` heuristics without testing in a real browser first. This is independent of, and much cheaper than, sourcing 15 real logos — do it first.
