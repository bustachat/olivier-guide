...
**Current version: v45.49 (2026-09-16).** Always confirm against `git log --oneline -1` and `guideVersion` in `athletes/olivier.json` — do not trust this line alone. It has sat stale for as many as 13 versions at a time, which is the clearest evidence available that a bloated section stops being read.
...
**🚩 Found v45.49, not fixed:** `akron`, `gcu`, `denver` have no `facilityDetails.extras` (the block is hidden, not wrong). Research each school's own facilities page before adding one.
