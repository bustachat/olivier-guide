...
**`ICON_OVERRIDES` (js/app.js, added v45.10) is an exception list, not a second data source — most schools should never appear in it.** Only add an entry when live-fetching is confirmed (Tier-1, a real browser, RULE 0) to produce a genuinely bad result for that specific school — a wrong/blocked domain, a non-square banner, a third party's degraded proxy rendition — never speculatively. An override value can be either a URL (fetched live, e.g. a confirmed real icon path that isn't `/favicon.ico`) or a local path under `assets/logos/` (a static asset committed to the repo — used only when even a *correct* live URL can't produce a good result, e.g. `tyler_jc`: the school's own athletics site has no square icon at all, so the owner supplied one directly). A local asset is the heavier of the two options — it can drift from the school's real current branding and nothing will ever flag that — so prefer a live URL override when one exists that actually works. **One documented exception to that preference: `virginia`'s override value is Google's *proxy* URL for `virginia.edu`, not an icon path on a school-owned host** — added v45.13 because the bare domain's TLS cert is registered to a different hostname and hangs rather than failing promptly. That is the one case where "a third party's degraded proxy rendition" is the override *value* rather than the reason to add one; do not treat it as a general licence to point overrides at proxies.

**The three render surfaces do NOT share one chain — they share two (v45.14). Know which one you are editing.** A single sentence used to claim all three were identical; they were not, and the difference is exactly where the v45.13 UCLA bug lived.

| Surface | Function | Chain |
|---|---|---|
| Explore card | `logoUrl()` / `handleLogoError()` (js/app.js) | `ICON_OVERRIDES[id]` → school's own `domain` `/favicon.ico` → Google proxy for `domain` → coloured initials |
| Dashboard shortlist | inline in `js/dashboard.js` | same as the card (aligned v45.14 — it previously led with `logo.clearbit.com`, a third-party logo API documented nowhere) |
| Details modal | `buildModalHeader()` / `handleModalLogoError()` (js/app.js) | `ICON_OVERRIDES[id]` → **`DOMAINS[id]`** `/favicon.ico` → school's own `domain` `/favicon.ico` → Google proxy for `domain` → coloured initials |

**The modal is deliberately a strict superset: it is the only surface that tries `DOMAINS[id]` (the athletics host) at all.** Before v45.13 it tried *only* that and never the school's own `domain`, which is why UCLA showed a placeholder in the modal while its card was fine (`uclabruins.com/favicon.ico` 404s, `ucla.edu`'s does not). **The untested mirror-image case is still open:** a school whose own `domain` favicon is broken but whose `DOMAINS[id]` athletics favicon works will now render in the modal and stay broken on the card and shortlist. 83 schools have distinct values for the two fields, so the population is real — nobody has swept it.

**⚠ Google's favicon proxy makes the coloured-initials fallback UNREACHABLE, on every surface.** `https://www.google.com/s2/favicons?domain=…` returns **HTTP 404 with a real, valid, byte-identical ~726-byte generic-globe PNG in the body** when it has no icon for a domain. A browser `<img>` only checks whether decodable image bytes arrived, not the status code, so `onerror` never fires again and the chain silently "succeeds" on a meaningless globe. **Consequence: the terminal initials stage in `handleLogoError`, `handleModalLogoError` and the shortlist's inline chain is dead code for any school that reaches the proxy** — those schools show a globe, not legible initials. Diagnosed v45.13, **fixed v45.15**: `PLACEHOLDER_ICON_DOMAINS` (js/app.js) is an evidence-backed denylist of domains the proxy only answers with the globe, and all three chains skip the proxy stage for them so initials actually render. **Do not "fix" a globe by adding another proxy stage**, and do not add a domain to that set speculatively — hash-confirm it first (the command is in the comment above the set; the placeholder is byte-identical every time). The `ICONFALLBACK` check enforces both halves: a listed domain no school uses is flagged as a typo that silently matches nothing, and a chain that stops calling `isPlaceholderIconDomain()` is flagged too — the data being right is not enough if the code stops reading it. A real per-school `ICON_OVERRIDES` logo is still the proper fix; legible initials are just strictly better than a globe while that work is outstanding.
...
- **🚩 15 schools still show the generic placeholder globe in the modal (and likely the card too, since they share the same broken domain) — need real `ICON_OVERRIDES` logos, same process as v45.11–v45.12.** `glendale_cc_az` (Gauchos), `neosho_county_cc` (Panthers), `iowa_lakes_cc` (Lakers), `blinn_college` (Buccaneers), `coastal_bend_cc` (Cougars), `angelina_college` (Roadrunners), `eastern_arizona` (Gila Monsters), `central_wyoming` (Rustlers), `laramie_county_cc` (Golden Eagles), `western_nebraska_cc` (Cougars), `trinidad_state` (Trojans), `jefferson_college_mo` (Vikings), `harcum_college` (Bears), `central_georgia_tech` (Titans), `noc_enid` (Jets). Confirmed via hash: their `DOMAINS[]` and `domain` fields are identical, and Google's proxy for that one domain returns the byte-identical generic marker — there is no programmatic fallback left to try. A Wikipedia-infobox-image shortcut was tried and abandoned mid-v45.13 — most of these schools' Wikipedia thumbnails are campus photos or signage, not usable at a 30px icon size. Source each via Google Images search + ≥2-source Tier-1 cross-check (official social account, MascotDB.com, apparel vendor), same as the v45.12 CCC batch — do not ship a lower-effort placeholder just to close the gap. **The v45.13 entry hedged that the card was "likely" affected too; v45.14 settled it from the code — it is, and so is the Dashboard shortlist.** Both of those surfaces read only the school's own `domain`, which for these 15 is the same broken host the modal fails on, so all three surfaces land on the same globe. Fixing one `ICON_OVERRIDES` entry per school therefore fixes all three at once.
- **✅ RESOLVED v45.15 — the coloured-initials fallback was unreachable on every surface, because Google's proxy answers with a valid placeholder instead of an error (diagnosed v45.13, fixed v45.15).** `s2/favicons` returns **HTTP 404 with a decodable ~726B generic-globe PNG in the body**; a browser `<img>` only checks the bytes, so `onerror` never fires again and the terminal initials stage in `handleLogoError`, `handleModalLogoError` and the shortlist's inline chain is dead code for any school that reaches the proxy. v45.13 correctly identified this and then routed around it (adding earlier stages) rather than remediating it, so the 15 schools above render a meaningless globe where legible initials would be strictly better and cost nothing. **Fixed exactly that way, with no new research and no network access:** `PLACEHOLDER_ICON_DOMAINS` (js/app.js) holds the 15 hash-confirmed domains, and `handleLogoError`, `handleModalLogoError` and the shortlist's inline chain all skip the proxy for them and fall straight to initials. No size/`naturalWidth` heuristic was used — those would need a real browser to trust. Behaviour for every other school is unchanged (verified by simulating the full `onerror` cascade for both app.js chains and rendering the shortlist template for all four branches). The `ICONFALLBACK` validator check plus 4 negative tests keep it from rotting. **The 15 still need real logos** (item above) — this only upgrades their placeholder from a meaningless globe to legible initials.
- **🚩 Unswept mirror-image case: a school whose own `domain` favicon is broken but whose `DOMAINS[id]` athletics favicon works now renders in the modal and stays broken on the card and Dashboard shortlist.** Since v45.13 the modal is a strict superset of the other two chains (it alone tries `DOMAINS[id]`), so the surfaces can now disagree in that direction. **83 of 170 schools have distinct `domain` vs `DOMAINS[]` values**, so the population is real; nobody has checked how many actually fall in this class. Needs a real-browser sweep (RULE 0) — every fetch-based check is blocked from a sandbox.
...
`negtests/checks.json` is the committed suite — **12 cases across MAXAID (5), ICONFALLBACK (4), TIER, FIT and SCORES-SRC, all proven.** Add a case whenever you add a check. Note the **FIT case is a permanent regression test for v44.51**: it perturbs `housingPenalty` in `js/scores.js` and requires FIT to fire, so if anyone ever reintroduces a local mirror of the formula, that case goes silent and the suite fails.

**Read the suite's own tail line, not just `Issues: 0` — and never report a run you did not read (v45.14).** A session reported the suite as "8/8 proven" when it was printing `7/8 proven · 1 did not prove what they claimed`: the `MAXAID / missing field` case carried a **CRLF** (`\r\n`) in its `find` string while `data/conferences.json` is LF-only, so the patch was a silent no-op and `negtest.py` correctly reported **VOID / MUTATION-NOOP**. That is the exact v44.50 failure this script was written to make impossible, recurring one level up — the tool caught it and the *report* discarded the catch. The line-ending trap is the likely recurrence mode for any new case, since `negtest.py`'s own docstring example uses `\n`: **author `find` strings with `\n`, and treat any VOID as a failed run, not a passed one.**
