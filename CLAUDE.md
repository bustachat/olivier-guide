...
- **✅ RESOLVED v44.47 — Keiser's "Fort Lauderdale" references fixed; there were THREE, not two.** v40.6 corrected Keiser's location to **West Palm Beach** across 12 occurrences plus mapX/mapY but missed the `CONF_SECTIONS` NAIA section intro (Explore) and the NAIA tier intro (Coaches) in `js/app.js` — **and a third the original note had not found**, `data/aac.json`'s FAU `culture.olivierMatch`, which read *"adjacent to Fort Lauderdale (Keiser) and Miami"*. That one was wrong twice over: wrong city, **and wrong direction** — West Palm Beach is ~30 min *north* of Boca Raton while Fort Lauderdale is south. **Lesson: when closing a "two known instances" copy item, re-grep rather than trusting the count** — the note was written from a `js`/`html` glob and the data files were never swept. Every remaining "Fort Lauderdale" in the repo was then checked and all are legitimate: Nova Southeastern genuinely is in Fort Lauderdale, and Lynn/Keiser cite it as a nearby destination with correct distances (20 min / 45 min). Still true that **no validator can see this class of error** — `PROSE` covers counts, roster claims, scrape-season phrasing and phantom schools, not a wrong *city*.
